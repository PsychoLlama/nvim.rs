...
unsafe extern "C" fn matching_chars_iwhite(
    ...
    while k < 2 as ::core::ffi::c_int {
        ...
        let mut slen: size_t = if ((800 as ::core::ffi::c_int - 1 as ::core::ffi::c_int)
            as size_t) < line_len(s)
        {
        ...
        sp[k as usize] = s_mmfile {
            ptr: &raw mut *(&raw mut p as *mut [::core::ffi::c_char; 800])
                .offset(k as isize) as *mut ::core::ffi::c_char,
...
unsafe extern "C" fn matching_chars(
    ...
    let mut s1len: size_t = if ((800 as ::core::ffi::c_int - 1 as ::core::ffi::c_int)
        as size_t) < line_len(m1)
    {
        (800 as ::core::ffi::c_int - 1 as ::core::ffi::c_int) as size_t
    } else {
        line_len(m1)
    };
    let mut s2len: size_t = if ((800 as ::core::ffi::c_int - 1 as ::core::ffi::c_int)
        as size_t) < line_len(m2)
    {
        (800 as ::core::ffi::c_int - 1 as ::core::ffi::c_int) as size_t
    } else {
        line_len(m2)
    ...
    let mut matrix: [[::core::ffi::c_int; 800]; 2] = [
        [0 as ::core::ffi::c_int; 800],
        [0; 800],
    ];
    ...
    while i < s1len {
        icur = !icur;
        let mut e1: *mut ::core::ffi::c_int = &raw mut *(&raw mut matrix
            as *mut [::core::ffi::c_int; 800])
            .offset(icur as isize) as *mut ::core::ffi::c_int;
        let mut e2: *mut ::core::ffi::c_int = &raw mut *(&raw mut matrix
            as *mut [::core::ffi::c_int; 800])
            .offset(!icur as ::core::ffi::c_int as isize) as *mut ::core::ffi::c_int;
        let mut j: size_t = 0 as size_t;
        while j < s2len {
            if *e2.offset(j.wrapping_add(1 as size_t) as isize)
                > *e1.offset(j.wrapping_add(1 as size_t) as isize)
            {
                *e1.offset(j.wrapping_add(1 as size_t) as isize) = *e2
                    .offset(j.wrapping_add(1 as size_t) as isize);
            }
            if *e1.offset(j as isize) > *e1.offset(j.wrapping_add(1 as size_t) as isize)
            {
                *e1.offset(j.wrapping_add(1 as size_t) as isize) = *e1
                    .offset(j as isize);
            }
            if *s1.offset(i as isize) as ::core::ffi::c_int
                ...
                && *e2.offset(j as isize) + 1 as ::core::ffi::c_int
                    > *e1.offset(j.wrapping_add(1 as size_t) as isize)
            {
                *e1.offset(j.wrapping_add(1 as size_t) as isize) = *e2.offset(j as isize)
                    + 1 as ::core::ffi::c_int;
            }
            j = j.wrapping_add(1);
        }
...
unsafe extern "C" fn count_n_matched_chars(
    ...
    while i < n {
        ...
        while j < n {
            if !(**sp.offset(i as isize)).ptr.is_null()
                && !(**sp.offset(j as isize)).ptr.is_null()
            {
                matched += 1;
                matched_chars
                    += if iwhite as ::core::ffi::c_int != 0 {
                        matching_chars_iwhite(
                            *sp.offset(i as isize),
                            *sp.offset(j as isize),
                        )
                    } else {
...
#[no_mangle]
pub unsafe extern "C" fn fastforward_buf_to_lnum(
    mut s: mmfile_t,
    mut lnum: linenr_T,
) -> mmfile_t {
...
unsafe extern "C" fn try_possible_paths(
    ...
    if path_idx == npaths {
        if *choice > 0 as ::core::ffi::c_int {
            ...
            let mut current_lines: [*mut mmfile_t; 8] = [::core::ptr::null_mut::<
                mmfile_t,
            >(); 8];
            ...
            while k < ndiffs {
                ...
                current_lines[k as usize] = (&raw mut mm as *mut mmfile_t)
                    .offset(k as isize);
                k = k.wrapping_add(1);
            }
            ...
            let mut matched_chars: ::core::ffi::c_int = count_n_matched_chars(
                &raw mut current_lines as *mut *mut mmfile_t,
                ndiffs,
                iwhite,
            );
            let mut score: ::core::ffi::c_int = (*diffcmppath
                .offset(unwrapped_idx_from as isize))
                .df_lev_score + matched_chars;
            if score > (*diffcmppath.offset(unwrapped_idx_to as isize)).df_lev_score {
                ...
                (*diffcmppath.offset(unwrapped_idx_to as isize))
                    .df_decision[0 as ::core::ffi::c_int as usize] = diffcmppath
                    .offset(unwrapped_idx_from as isize);
                (*diffcmppath.offset(unwrapped_idx_to as isize))
                    .df_choice[0 as ::core::ffi::c_int as usize] = *choice;
                (*diffcmppath.offset(unwrapped_idx_to as isize)).df_lev_score = score;
            } else if score
                == (*diffcmppath.offset(unwrapped_idx_to as isize)).df_lev_score
            {
                let c2rust_fresh1 = (*diffcmppath.offset(unwrapped_idx_to as isize))
                    .df_path_n;
                ...
                (*diffcmppath.offset(unwrapped_idx_to as isize))
                    .df_decision[k_0 as usize] = diffcmppath
                    .offset(unwrapped_idx_from as isize);
                (*diffcmppath.offset(unwrapped_idx_to as isize))
                    .df_choice[k_0 as usize] = *choice;
            }
        }
        return;
    }
...
unsafe extern "C" fn unwrap_indexes(
    ...
    while k < ndiffs {
        num_unwrap_scalar = num_unwrap_scalar
            .wrapping_mul(
                (*diff_len.offset(k as isize) as size_t).wrapping_add(1 as size_t),
            );
        k = k.wrapping_add(1);
    }
    ...
    while k_0 < ndiffs {
        num_unwrap_scalar = num_unwrap_scalar
            .wrapping_div(
                (*diff_len.offset(k_0 as isize) as size_t).wrapping_add(1 as size_t),
            );
...
unsafe extern "C" fn populate_tensor(
    ...
    if ch_dim == ndiffs {
        ...
        (*diffcmppath.offset(unwrapper_idx_to as isize)).df_lev_score = -1
            as ::core::ffi::c_int;
        try_possible_paths(
            df_iters,
...
pub unsafe extern "C" fn linematch_nbuffers(
    ...
    while i < ndiffs {
        '_c2rust_label_0: {
            if *diff_len.offset(i as isize) >= 0 as ::core::ffi::c_int {} else {
                __assert_fail(
                    b"diff_len[i] >= 0\0".as_ptr() as *const ::core::ffi::c_char,
                    b"/home/overlord/projects/neovim/neovim/src/nvim/linematch.c\0"
        ...
        memsize = memsize
            .wrapping_mul(
                (*diff_len.offset(i as isize) + 1 as ::core::ffi::c_int) as size_t,
            );
        memsize_decisions = memsize_decisions
            .wrapping_add(*diff_len.offset(i as isize) as size_t);
        i = i.wrapping_add(1);
    }
    let mut diffcmppath: *mut diffcmppath_T = xmalloc(
        ::core::mem::size_of::<diffcmppath_T>().wrapping_mul(memsize),
    ...
    while i_0 < memsize {
        ...
        let mut j: size_t = 0 as size_t;
        while j < n {
            (*diffcmppath.offset(i_0 as isize)).df_choice_mem[j as usize] = -1
                as ::core::ffi::c_int;
            j = j.wrapping_add(1);
        }
    ...
    *decisions = xmalloc(
        ::core::mem::size_of::<::core::ffi::c_int>().wrapping_mul(memsize_decisions),
    ...
    while (*startNode).df_path_n > 0 as size_t {
        ...
        *(*decisions).offset(c2rust_fresh0 as isize) = (*startNode)
            .df_choice[j_0 as usize];
    ...
    while i_1 < n_optimal.wrapping_div(2 as size_t) {
        ...
        *(*decisions).offset(i_1 as isize) = *(*decisions)
            .offset(n_optimal.wrapping_sub(1 as size_t).wrapping_sub(i_1) as isize);
        *(*decisions)
            .offset(n_optimal.wrapping_sub(1 as size_t).wrapping_sub(i_1) as isize) = tmp;
...
unsafe extern "C" fn test_charmatch_paths(
    ...
    if (*node).df_choice_mem[lastdecision as usize] == -1 as ::core::ffi::c_int {
        if (*node).df_path_n == 0 as size_t {
            ...
            (*node).df_choice_mem[lastdecision as usize] = minimum_turns
                as ::core::ffi::c_int;
        }
    }
