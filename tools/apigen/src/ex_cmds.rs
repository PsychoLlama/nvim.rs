//! The Ex command table, from `crates/nvim/src/nvim/ex_cmds.lua`.
//!
//! Upstream's `src/gen/gen_ex_cmds.lua` (tag v0.12.4) turned that file into
//! two generated headers: the `cmdidx_T` enum, and `cmdnames[]` plus the two
...
/// `EX_A | EX_B | ...`, low bit first, over the single-bit locals the Lua
...
const TABLE_DOC: &str = r#"//! GENERATED by tools/apigen from `crate::src::nvim::ex_cmds.lua` -- the same
...
const ENUM_DOC: &str = r#"//! GENERATED by tools/apigen from `crate::src::nvim::ex_cmds.lua` -- the same
//! metadata upstream's `src/gen/gen_ex_cmds.lua` consumed. Do not edit; run
//! `just apigen` (`just apigen --check` fails on drift).
//!
//! Every `cmdidx_T` value, in enum order: an index into the `cmdnames` table
//! generated from the same file, which is why the two are emitted together.
//! c2rust rendered each `switch (cmdidx)` arm as a bare integer, and
//! thirty-one modules then re-declared whichever names they happened to need.
//!
//! `CMD_USER`/`CMD_USER_BUF` are the two negative members (a user command,
//! global or buffer-local); `CMD_SIZE` is the count, and doubles as the "not
//! a command" marker.
...
pub fn generate(lua_path: &Path) -> Result<(String, String), String> {
    ...
    for row in &rows {
        ...
        let (name, func, addr) = (&row.command, &row.func, &row.addr_type);
    ...
    for (i, row) in rows.iter().enumerate() {
        enums.push_str(&format!("pub const {}: CMD_index = {i};\n", row.enum_name));
    }
    enums.push_str(&format!("pub const CMD_SIZE: CMD_index = {n};\n"));
