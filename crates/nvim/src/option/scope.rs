...
pub(crate) unsafe fn get_varp_scope_from(
    ...
    if opt_flags.has(OptionSetFlags::LOCAL) && option_is_global_local(opt_idx) {
        ...
        return match opt_idx {
            kOptFormatprg => buf_var!(buffer, b_p_fp),
            kOptFsync => buf_var!(buffer, b_p_fs),
            kOptFindfunc => buf_var!(buffer, b_p_ffu),
            kOptErrorformat => buf_var!(buffer, b_p_efm),
            kOptGrepformat => buf_var!(buffer, b_p_gefm),
            kOptGrepprg => buf_var!(buffer, b_p_gp),
            kOptMakeprg => buf_var!(buffer, b_p_mp),
            kOptEqualprg => buf_var!(buffer, b_p_ep),
            kOptKeywordprg => buf_var!(buffer, b_p_kp),
            kOptPath => buf_var!(buffer, b_p_path),
            kOptAutocomplete => buf_var!(buffer, b_p_ac),
            kOptAutoread => buf_var!(buffer, b_p_ar),
            kOptTags => buf_var!(buffer, b_p_tags),
            kOptTagcase => buf_var!(buffer, b_p_tc),
            kOptSidescrolloff => win_var!(win, w_onebuf_opt.wo_siso),
            kOptScrolloff => win_var!(win, w_onebuf_opt.wo_so),
            kOptDefine => buf_var!(buffer, b_p_def),
            kOptInclude => buf_var!(buffer, b_p_inc),
            kOptCompleteopt => buf_var!(buffer, b_p_cot),
            kOptDictionary => buf_var!(buffer, b_p_dict),
            kOptDiffanchors => buf_var!(buffer, b_p_dia),
            kOptThesaurus => buf_var!(buffer, b_p_tsr),
            kOptThesaurusfunc => buf_var!(buffer, b_p_tsrfu),
            kOptTagfunc => buf_var!(buffer, b_p_tfu),
            ...
            kOptUndolevels => buf_var!(buffer, b_p_ul),
            kOptLispwords => buf_var!(buffer, b_p_lw),
            kOptBackupcopy => buf_var!(buffer, b_p_bkc),
            kOptMakeencoding => buf_var!(buffer, b_p_menc),
