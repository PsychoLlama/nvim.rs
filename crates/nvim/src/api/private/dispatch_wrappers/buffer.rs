//! Dispatch wrappers for `crate::api::buffer`, part 1 of 2.
//!
//! GENERATED by tools/apigen; see the parent module. Do not edit;
//! run `just apigen`.

#![deny(unsafe_op_in_unsafe_fn)]

use super::*;

pub unsafe fn handle_nvim__buf_stats(
    channel_id: uint64_t,
    args: Array,
    arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim__buf_stats",
        c"nvim__buf_stats",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 1 {
        wrong_arity(error, 1, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim__buf_stats", c"Buffer");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim__buf_stats(arg_1, arena, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeDict, object_data { dict: rv })
}

pub unsafe fn handle_nvim_buf_attach(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_attach",
        c"nvim_buf_attach",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 3 {
        wrong_arity(error, 3, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_attach", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_boolean(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_attach", c"Boolean");
        return NIL;
    };
    let mut arg_3: KeyDict_buf_attach =
        match read_keydict(Some(KeyDict_buf_attach_get_field), args[2], error) {
            KeySetArg::Read(v) => v,
            KeySetArg::Refused => return NIL,
            KeySetArg::WrongType => {
                wrong_type(error, 3, c"nvim_buf_attach", c"Dict(buf_attach) *");
                return NIL;
            }
        };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_attach(channel_id, arg_1, arg_2, &raw mut arg_3, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeBoolean, object_data { boolean: rv })
}

pub unsafe fn handle_nvim_buf_del_keymap(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_del_keymap",
        c"nvim_buf_del_keymap",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 3 {
        wrong_arity(error, 3, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_del_keymap", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_string(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_del_keymap", c"String");
        return NIL;
    };
    let Some(arg_3) = as_string(args[2]) else {
        wrong_type(error, 3, c"nvim_buf_del_keymap", c"String");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    unsafe { nvim_buf_del_keymap(channel_id, arg_1, arg_2, arg_3, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    NIL
}

pub unsafe fn handle_nvim_buf_del_mark(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_del_mark",
        c"nvim_buf_del_mark",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 2 {
        wrong_arity(error, 2, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_del_mark", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_string(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_del_mark", c"String");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_del_mark(arg_1, arg_2, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeBoolean, object_data { boolean: rv })
}

pub unsafe fn handle_nvim_buf_del_var(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_del_var",
        c"nvim_buf_del_var",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 2 {
        wrong_arity(error, 2, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_del_var", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_string(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_del_var", c"String");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    unsafe { nvim_buf_del_var(arg_1, arg_2, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    NIL
}

pub unsafe fn handle_nvim_buf_delete(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_delete",
        c"nvim_buf_delete",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 2 {
        wrong_arity(error, 2, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_delete", c"Buffer");
        return NIL;
    };
    let mut arg_2: KeyDict_buf_delete =
        match read_keydict(Some(KeyDict_buf_delete_get_field), args[1], error) {
            KeySetArg::Read(v) => v,
            KeySetArg::Refused => return NIL,
            KeySetArg::WrongType => {
                wrong_type(error, 2, c"nvim_buf_delete", c"Dict(buf_delete) *");
                return NIL;
            }
        };
    // SAFETY: a wrapper runs on the main loop.
    if unsafe { text_locked() } {
        text_locked_error(error);
        return NIL;
    }
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    unsafe { nvim_buf_delete(arg_1, &raw mut arg_2, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    NIL
}

pub unsafe fn handle_nvim_buf_detach(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_detach",
        c"nvim_buf_detach",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 1 {
        wrong_arity(error, 1, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_detach", c"Buffer");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_detach(channel_id, arg_1, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeBoolean, object_data { boolean: rv })
}

pub unsafe fn handle_nvim_buf_get_changedtick(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_get_changedtick",
        c"nvim_buf_get_changedtick",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 1 {
        wrong_arity(error, 1, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_get_changedtick", c"Buffer");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_get_changedtick(arg_1, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeInteger, object_data { integer: rv })
}

pub unsafe fn handle_nvim_buf_get_keymap(
    channel_id: uint64_t,
    args: Array,
    arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_get_keymap",
        c"nvim_buf_get_keymap",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 2 {
        wrong_arity(error, 2, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_get_keymap", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_string(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_get_keymap", c"String");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_get_keymap(arg_1, arg_2, arena, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeArray, object_data { array: rv })
}

pub unsafe fn handle_nvim_buf_get_lines(
    channel_id: uint64_t,
    args: Array,
    arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_get_lines",
        c"nvim_buf_get_lines",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 4 {
        wrong_arity(error, 4, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_get_lines", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_integer(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_get_lines", c"Integer");
        return NIL;
    };
    let Some(arg_3) = as_integer(args[2]) else {
        wrong_type(error, 3, c"nvim_buf_get_lines", c"Integer");
        return NIL;
    };
    let Some(arg_4) = as_boolean(args[3]) else {
        wrong_type(error, 4, c"nvim_buf_get_lines", c"Boolean");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe {
        nvim_buf_get_lines(
            channel_id,
            arg_1,
            arg_2,
            arg_3,
            arg_4,
            arena,
            core::ptr::null_mut(),
            error,
        )
    };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeArray, object_data { array: rv })
}

pub unsafe fn handle_nvim_buf_get_mark(
    channel_id: uint64_t,
    args: Array,
    arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_get_mark",
        c"nvim_buf_get_mark",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 2 {
        wrong_arity(error, 2, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_get_mark", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_string(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_get_mark", c"String");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_get_mark(arg_1, arg_2, arena, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeArray, object_data { array: rv })
}

pub unsafe fn handle_nvim_buf_get_name(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_get_name",
        c"nvim_buf_get_name",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 1 {
        wrong_arity(error, 1, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_get_name", c"Buffer");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_get_name(arg_1, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeString, object_data { string: rv })
}

pub unsafe fn handle_nvim_buf_get_offset(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_get_offset",
        c"nvim_buf_get_offset",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 2 {
        wrong_arity(error, 2, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_get_offset", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_integer(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_get_offset", c"Integer");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_get_offset(arg_1, arg_2, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeInteger, object_data { integer: rv })
}

pub unsafe fn handle_nvim_buf_get_text(
    channel_id: uint64_t,
    args: Array,
    arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_get_text",
        c"nvim_buf_get_text",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 6 {
        wrong_arity(error, 6, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_get_text", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_integer(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_get_text", c"Integer");
        return NIL;
    };
    let Some(arg_3) = as_integer(args[2]) else {
        wrong_type(error, 3, c"nvim_buf_get_text", c"Integer");
        return NIL;
    };
    let Some(arg_4) = as_integer(args[3]) else {
        wrong_type(error, 4, c"nvim_buf_get_text", c"Integer");
        return NIL;
    };
    let Some(arg_5) = as_integer(args[4]) else {
        wrong_type(error, 5, c"nvim_buf_get_text", c"Integer");
        return NIL;
    };
    let mut arg_6: KeyDict_empty = match read_keydict(Some(KeyDict_empty_get_field), args[5], error)
    {
        KeySetArg::Read(v) => v,
        KeySetArg::Refused => return NIL,
        KeySetArg::WrongType => {
            wrong_type(error, 6, c"nvim_buf_get_text", c"Dict(empty) *");
            return NIL;
        }
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe {
        nvim_buf_get_text(
            channel_id,
            arg_1,
            arg_2,
            arg_3,
            arg_4,
            arg_5,
            &raw mut arg_6,
            arena,
            core::ptr::null_mut(),
            error,
        )
    };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeArray, object_data { array: rv })
}

pub unsafe fn handle_nvim_buf_get_var(
    channel_id: uint64_t,
    args: Array,
    arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_get_var",
        c"nvim_buf_get_var",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 2 {
        wrong_arity(error, 2, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_get_var", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_string(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_get_var", c"String");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_get_var(arg_1, arg_2, arena, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    rv
}

pub unsafe fn handle_nvim_buf_is_loaded(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_is_loaded",
        c"nvim_buf_is_loaded",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 1 {
        wrong_arity(error, 1, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_is_loaded", c"Buffer");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_is_loaded(arg_1) };
    obj(kObjectTypeBoolean, object_data { boolean: rv })
}

pub unsafe fn handle_nvim_buf_is_valid(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_is_valid",
        c"nvim_buf_is_valid",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 1 {
        wrong_arity(error, 1, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_is_valid", c"Buffer");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_is_valid(arg_1) };
    obj(kObjectTypeBoolean, object_data { boolean: rv })
}

pub unsafe fn handle_nvim_buf_line_count(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_line_count",
        c"nvim_buf_line_count",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 1 {
        wrong_arity(error, 1, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_line_count", c"Buffer");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_line_count(arg_1, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeInteger, object_data { integer: rv })
}

pub unsafe fn handle_nvim_buf_set_keymap(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_set_keymap",
        c"nvim_buf_set_keymap",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 5 {
        wrong_arity(error, 5, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_set_keymap", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_string(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_set_keymap", c"String");
        return NIL;
    };
    let Some(arg_3) = as_string(args[2]) else {
        wrong_type(error, 3, c"nvim_buf_set_keymap", c"String");
        return NIL;
    };
    let Some(arg_4) = as_string(args[3]) else {
        wrong_type(error, 4, c"nvim_buf_set_keymap", c"String");
        return NIL;
    };
    let mut arg_5: KeyDict_keymap =
        match read_keydict(Some(KeyDict_keymap_get_field), args[4], error) {
            KeySetArg::Read(v) => v,
            KeySetArg::Refused => return NIL,
            KeySetArg::WrongType => {
                wrong_type(error, 5, c"nvim_buf_set_keymap", c"Dict(keymap) *");
                return NIL;
            }
        };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    unsafe {
        nvim_buf_set_keymap(
            channel_id,
            arg_1,
            arg_2,
            arg_3,
            arg_4,
            &raw mut arg_5,
            error,
        )
    };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    NIL
}

pub unsafe fn handle_nvim_buf_set_lines(
    channel_id: uint64_t,
    args: Array,
    arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_set_lines",
        c"nvim_buf_set_lines",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 5 {
        wrong_arity(error, 5, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_set_lines", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_integer(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_set_lines", c"Integer");
        return NIL;
    };
    let Some(arg_3) = as_integer(args[2]) else {
        wrong_type(error, 3, c"nvim_buf_set_lines", c"Integer");
        return NIL;
    };
    let Some(arg_4) = as_boolean(args[3]) else {
        wrong_type(error, 4, c"nvim_buf_set_lines", c"Boolean");
        return NIL;
    };
    let Some(arg_5) = as_array(args[4]) else {
        wrong_type(error, 5, c"nvim_buf_set_lines", c"ArrayOf(String)");
        return NIL;
    };
    // SAFETY: a wrapper runs on the main loop.
    if textlock.get() != 0 || unsafe { expr_map_locked() } {
        expr_map_locked_error(error);
        return NIL;
    }
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    unsafe { nvim_buf_set_lines(channel_id, arg_1, arg_2, arg_3, arg_4, arg_5, arena, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    NIL
}

pub unsafe fn handle_nvim_buf_set_mark(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_set_mark",
        c"nvim_buf_set_mark",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 5 {
        wrong_arity(error, 5, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_set_mark", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_string(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_set_mark", c"String");
        return NIL;
    };
    let Some(arg_3) = as_integer(args[2]) else {
        wrong_type(error, 3, c"nvim_buf_set_mark", c"Integer");
        return NIL;
    };
    let Some(arg_4) = as_integer(args[3]) else {
        wrong_type(error, 4, c"nvim_buf_set_mark", c"Integer");
        return NIL;
    };
    let mut arg_5: KeyDict_empty = match read_keydict(Some(KeyDict_empty_get_field), args[4], error)
    {
        KeySetArg::Read(v) => v,
        KeySetArg::Refused => return NIL,
        KeySetArg::WrongType => {
            wrong_type(error, 5, c"nvim_buf_set_mark", c"Dict(empty) *");
            return NIL;
        }
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    let rv = unsafe { nvim_buf_set_mark(arg_1, arg_2, arg_3, arg_4, &raw mut arg_5, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    obj(kObjectTypeBoolean, object_data { boolean: rv })
}

pub unsafe fn handle_nvim_buf_set_name(
    channel_id: uint64_t,
    args: Array,
    _arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_set_name",
        c"nvim_buf_set_name",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 2 {
        wrong_arity(error, 2, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_set_name", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_string(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_set_name", c"String");
        return NIL;
    };
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    unsafe { nvim_buf_set_name(arg_1, arg_2, error) };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    NIL
}

pub unsafe fn handle_nvim_buf_set_text(
    channel_id: uint64_t,
    args: Array,
    arena: *mut Arena,
    error: *mut Error,
) -> Object {
    // SAFETY: the dispatcher hands over an argument array of `size` initialized
    // objects and an `Error` slot that is live and ours alone until we return;
    // both outlive the call.
    let (args, error) = unsafe { (args_slice(&args), &mut *error) };
    log_invoke(
        c"handle_nvim_buf_set_text",
        c"nvim_buf_set_text",
        line!() as c_int,
        channel_id,
    );
    if args.len() != 6 {
        wrong_arity(error, 6, args.len());
        return NIL;
    }
    let Some(arg_1) = as_handle(args[0], kObjectTypeBuffer) else {
        wrong_type(error, 1, c"nvim_buf_set_text", c"Buffer");
        return NIL;
    };
    let Some(arg_2) = as_integer(args[1]) else {
        wrong_type(error, 2, c"nvim_buf_set_text", c"Integer");
        return NIL;
    };
    let Some(arg_3) = as_integer(args[2]) else {
        wrong_type(error, 3, c"nvim_buf_set_text", c"Integer");
        return NIL;
    };
    let Some(arg_4) = as_integer(args[3]) else {
        wrong_type(error, 4, c"nvim_buf_set_text", c"Integer");
        return NIL;
    };
    let Some(arg_5) = as_integer(args[4]) else {
        wrong_type(error, 5, c"nvim_buf_set_text", c"Integer");
        return NIL;
    };
    let Some(arg_6) = as_array(args[5]) else {
        wrong_type(error, 6, c"nvim_buf_set_text", c"ArrayOf(String)");
        return NIL;
    };
    // SAFETY: a wrapper runs on the main loop.
    if textlock.get() != 0 || unsafe { expr_map_locked() } {
        expr_map_locked_error(error);
        return NIL;
    }
    // SAFETY: each argument was checked against the type the signature declares;
    // `arena` and `error` are the dispatcher's own.
    unsafe {
        nvim_buf_set_text(
            channel_id, arg_1, arg_2, arg_3, arg_4, arg_5, arg_6, arena, error,
        )
    };
    if error.type_0 != kErrorTypeNone {
        return NIL;
    }
    NIL
}
