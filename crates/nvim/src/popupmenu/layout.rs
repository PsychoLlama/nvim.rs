...
pub(crate) unsafe fn pum_compute_size() {
    ...
    for item in unsafe { pum_items() } {
        if !item.pum_text.is_null() {
            base = base.max(unsafe { vim_strsize(item.pum_text) });
        }
        if !item.pum_kind.is_null() {
            kind = kind.max(unsafe { vim_strsize(item.pum_kind) } + 1);
        }
        if !item.pum_extra.is_null() {
            extra = extra.max(unsafe { vim_strsize(item.pum_extra) } + 1);
        }
    }
...
pub(crate) unsafe fn pum_position_at_mouse(min_width: c_int) {
    ...
    if grid > 1 {
        ...
        if !wp.is_null() {
            ...
            pum_win_row_offset.set(unsafe { (*wp).w_winrow });
            pum_win_col_offset.set(unsafe { (*wp).w_wincol });

            if unsafe { (*wp).w_view_height } > 0 || unsafe { (*wp).w_view_width } > 0 {
                // The user asked for a different grid size; let the menu
                // extend to it.
                let (winrow, wincol) = (unsafe { (*wp).w_winrow }, unsafe { (*wp).w_wincol });
                max_row = (Rows.get() - winrow).max(winrow + unsafe { (*wp).w_view_height });
                max_col = (Columns.get() - wincol).max(wincol + unsafe { (*wp).w_view_width });
            }
        }
    }
