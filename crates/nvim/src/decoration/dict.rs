...
pub unsafe fn decor_to_dict_legacy(
    ...
    if flags & kSHConceal as c_int != 0 {
        ...
        let value = unsafe { Object::string(cstr_to_string(buf.as_ptr())) };
        put(dict, c"conceal", value);
    }
    ...
    if !sh_hl.url.is_null() {
        unsafe { put(dict, c"url", Object::string(cstr_to_string(sh_hl.url))) };
    }

    if !virt_text.is_null() {
        unsafe { put_virt_text(dict, &*virt_text, hl_name) };
        priority = i32::from(unsafe { (*virt_text).priority });
    }

    if !virt_lines.is_null() {
        unsafe { put_virt_lines(dict, &*virt_lines, hl_name) };
...
unsafe fn put_virt_text(dict: &mut ApiDict, vt: &DecorVirtText, hl_name: bool) {
    ...
    put(dict, c"virt_text_repeat_linebreak", value);
    if vt.pos == kVPosWinCol {
        put(dict, c"virt_text_win_col", Object::integer(vt.col.into()));
    }
    let pos = VIRT_TEXT_POS_STR[vt.pos as usize];
    let value = Object::string(String_0::from_cstr(pos));
    put(dict, c"virt_text_pos", value);
}
...
unsafe fn put_virt_lines(dict: &mut ApiDict, vt: &DecorVirtText, hl_name: bool) {
    ...
    put(dict, c"virt_lines_above", value);
    let value = Object::boolean(line_flags & kVLLeftcol as c_int != 0);
    put(dict, c"virt_lines_leftcol", value);
    let overflow = if line_flags & kVLScroll as c_int != 0 {
        c"scroll"
    } else {
        c"trunc"
    };
    let value = Object::string(String_0::from_cstr(overflow));
    put(dict, c"virt_lines_overflow", value);
}
...
unsafe fn put_sign(dict: &mut ApiDict, sh: &mut DecorSignHighlight, hl_name: bool) {
    if sh.text[0] != 0 {
        ...
        // SAFETY: the string the decoration owns, live for the copy.
        let value = unsafe { Object::string(cstr_to_string(buf.as_ptr())) };
        put(dict, c"sign_text", value);
    }

    if !sh.sign_name.is_null() {
        // SAFETY: the string the decoration owns, live for the copy.
        let value = unsafe { Object::string(cstr_to_string(sh.sign_name)) };
        put(dict, c"sign_name", value);
    }
