    ...
    unsafe fn next_pointer(&mut self) -> *const c_void {
        unsafe {
            if self.reads_typvals() {
                ...
                self.position();
                self.ap.next_arg::<*mut c_void>() as *const c_void
            }
        }
    }
...
unsafe fn parse_conversion(
    ...
    unsafe {
        ...
        *p = p.add(1); // step over the '%'
        ...
        if *ptype as u8 == b'$' {
            let digstart = *p;
            pos_arg = get_unsigned_int(digstart, p, tvs_present).ok_or(())? as c_int;
            *p = p.add(1); // step over the '$'
        }
        ...
        if **p as u8 == b'*' {
            let digstart = p.add(1);
            *p = p.add(1);
            let j = star_argument(args, p, digstart)?;
            if j >= 0 {
            ...
            let digstart = *p;
            c.min_field_width = get_unsigned_int(digstart, p, tvs_present).ok_or(())? as size_t;
        }
        ...
        if **p as u8 == b'.' {
            *p = p.add(1);
            ...
            if ascii_isdigit(**p as c_int) {
                let digstart = *p;
                c.precision = get_unsigned_int(digstart, p, tvs_present).ok_or(())? as size_t;
            } else if **p as u8 == b'*' {
        ...
        if matches!(**p as u8, b'h' | b'l' | b'z') {
            c.length_modifier = **p as u8;
            *p = p.add(1);
            if c.length_modifier == b'l' && **p as u8 == b'l' {
        ...
        match c.fmt_spec {
            ...
            _ => {
                ...
                if c.fmt_spec == b'S' {
                    ...
                    let mut end = str_arg;
                    while *end != 0 {
                        let cell = utf_ptr2cells(end) as size_t;
                    ...
                    str_arg_l = end.offset_from(str_arg) as size_t;
...
unsafe fn render_integer(c: &mut Conversion, args: &mut Args, tmp: &mut [c_char; TMP]) -> Body {
    unsafe {
        ...
        } else if c.alternate_form
            && arg_sign != 0
            && matches!(c.fmt_spec, b'x' | b'X' | b'b' | b'B')
        {
...
unsafe fn render_float(c: &mut Conversion, args: &mut Args, tmp: &mut [c_char; TMP]) -> Body {
    unsafe {
        ...
        if f.is_infinite() || (matches!(c.fmt_spec, b'f' | b'F') && abs_f > 1.0e307) {
            ...
            c.zero_padding = false;
            return Body::Tmp(strlen(tmp.as_ptr()));
        }
        if f.is_nan() {
            ...
            memmove(tmp.as_mut_ptr().cast(), nan.as_ptr().cast(), 4);
            c.zero_padding = false;
            return Body::Tmp(3);
        }
        ...
        let mut str_arg_l = snprintf(tmp.as_mut_ptr(), TMP, format.as_ptr(), f) as size_t;
        debug_assert!(str_arg_l < TMP);

        if remove_trailing_zeroes {
            str_arg_l = trim_float(c, tmp, str_arg_l);
        } else {
            str_arg_l = trim_exponent_width(c, tmp, str_arg_l);
        }
...
unsafe fn delete_byte(at: *mut c_char, len: size_t) -> size_t {
    unsafe {
        memmove(at.cast(), at.add(1).cast(), strlen(at.add(1)) + 1);
        len - 1
    }
}
...
unsafe fn trim_float(c: &Conversion, tmp: &mut [c_char; TMP], mut len: size_t) -> size_t {
    unsafe {
        ...
        if matches!(c.fmt_spec, b'f' | b'F') {
            ...
            if *tp.add(1) as u8 == b'+' {
                len = delete_byte(tp.add(1), len);
            }
            // Leading zeros of the exponent, past its sign.
            let mut i = if *tp.add(1) as u8 == b'-' { 2 } else { 1 };
            while *tp.add(i) as u8 == b'0' {
                len = delete_byte(tp.add(i), len);
            }
            tp = tp.sub(1);
        }
        ...
        if !c.precision_specified {
            ...
            // the point.
            while tp > tmp.as_mut_ptr().add(2) && *tp as u8 == b'0' && *tp.sub(1) as u8 != b'.' {
                len = delete_byte(tp, len);
                tp = tp.sub(1);
            }
        }
        len
...
unsafe fn trim_exponent_width(c: &Conversion, tmp: &mut [c_char; TMP], len: size_t) -> size_t {
    unsafe {
        ...
        if !tp.is_null()
            && matches!(*tp.add(1) as u8, b'+' | b'-')
            && *tp.add(2) as u8 == b'0'
            && ascii_isdigit(*tp.add(3) as c_int)
            && ascii_isdigit(*tp.add(4) as c_int)
        {
            return delete_byte(tp.add(2), len);
        }
        len
    }
}
    ...
    unsafe {
        ...
        let split = if c.zeros_to_pad == 0 {
            ...
            if c.zero_insertion_ind > 0 {
                sink.copy(body, c.zero_insertion_ind);
            }
        ...
        if len > split {
            sink.copy(body.add(split), len - split);
        }
        ...
        'error: {
            while *p != 0 {
                if *p as u8 != b'%' {
                    ...
                    let n = xstrchrnul(p.add(1), b'%' as c_char).offset_from(p) as size_t;
                    sink.copy(p, n);
                    p = p.add(n);
                    continue;
                }

                let Ok(mut c) = parse_conversion(&mut args, &mut p, tvs_present) else {
                    break 'error;
                };
                ...
                if *p != 0 {
                    p = p.add(1); // step over the conversion character
                }
                emit_conversion(&mut sink, &c, body, len);
                xfree(tofree.cast());
            }
            ...
            if tvs_present && (*tvs.offset(unused as isize)).v_type != VAR_UNKNOWN {
                emsg(gettext(E_TOO_MANY_ARGS.as_ptr()));
            }
        }
