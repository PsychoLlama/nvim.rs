...
pub unsafe fn ex_messages(eap: *mut exarg_T) {
    ...
    let mut p = if unsafe { (*eap).skip } != 0 {
        msg_hist_temp.get()
    } else {
        msg_hist_first.get()
    };
    let mut skip = if unsafe { (*eap).addr_count } != 0 {
        msg_hist_len.get() - unsafe { (*eap).line2 as c_int }
    } else {
        0
    };
    ...
    while !p.is_null() {
        ...
        let temporary = unsafe { (*p).temp } && unsafe { (*eap).skip } == 0;
    ...
    if entries.size > 0 {
        ui_call_msg_history_show(entries, unsafe { (*eap).skip } != 0);
