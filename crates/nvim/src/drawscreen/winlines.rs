...
/// Draw window `window`, and answer the `w_botline` it had on entry.
///
...
pub(crate) unsafe fn draw_window_lines(
    ...
    'restart: loop {
        ...
        window.w_last_cursorline = window.w_cursorline;
        window.w_last_cursor_lnum_rnu = if window.w_onebuf_opt.wo_rnu != 0 {
            window.w_cursor.lnum
        } else {
            0
        };
        window.w_lines_valid = window.w_lines_valid.max(w.idx);
        window.w_display_tick = display_tick.get();

        // Tell the syntax machinery where parsing stopped.
        if w.syntax_last_parsed != 0 && unsafe { syntax_present(window.raw()) } {
            unsafe { syntax_end_parsing(window.raw(), w.syntax_last_parsed + 1) };
        }

        let old_botline = window.w_botline;
        window.w_empty_rows = 0;
...
unsafe fn line_needs_drawing(window: Win, buffer: *mut Buffer, rg: &Regions, w: &Walk) -> bool {
    ...
    if w.row < rg.top_end
        ...
        || rg.top_to_mod
        || w.idx >= window.w_lines_valid
        || w.row + unsafe { *window.w_lines.add(w.idx as usize) }.wl_size as c_int > rg.bot_start
    {
        return true;
    }
            ...
            || (w.lnum >= rg.mod_top
                ...
                && (w.lnum < rg.mod_bot
                    ...
                    || (w.did_update == DidUpdate::Line
                        && unsafe { syntax_present(window.raw()) }
                        && ((foldmethod_is_syntax(unsafe { Win::new(window.raw()) }) && has_any_folding(unsafe { Win::new(window.raw()) }) != 0)
    ...
    w.lnum == window.w_cursorline || w.lnum == window.w_last_cursorline
}
...
unsafe fn draw_one_line(
    ...
    let foldinfo = if window.w_onebuf_opt.wo_cul != 0 && w.lnum == window.w_cursor.lnum {
        cursorline_fi
    } else {
        fold_info(unsafe { Win::new(window.raw()) }, w.lnum)
    };

    // A concealed line with no filler lines takes no rows at all.
    let concealed = unsafe { decor_conceal_line(window.raw(), w.lnum - 1, false) };
    if concealed && unsafe { win_get_fill(Win::new(window.raw()), w.lnum) } == 0 {
    ...
    if foldinfo.fi_lines == 0
        ...
        && unsafe { win_get_fill(Win::new(window.raw()), w.lnum) } == 0
    {
        // This line is not going to fit. Draw nothing here; the "@" lines
        // below take the rest.
        w.row = window.w_view_height + 1;
    } else {
        unsafe { prepare_search_hl(window.raw(), SearchHl::current().raw(), w.lnum) };
    ...
    if w.row > window.w_view_height {
        ...
        if dollar_vcol.get() == -1 || !is_curline {
            unsafe { (*wl).wl_size = plines_win(Win::new(window.raw()), w.lnum, true) as uint16_t };
        }
        w.idx += 1;
        return false;
    }
...
unsafe fn scroll_for_changed_lines(window: Win, rg: &mut Regions, w: &mut Walk) {
    ...
    let mut old_rows = 0;
    let mut i = w.idx;
    while i < window.w_lines_valid {
        let wl = unsafe { window.w_lines.add(i as usize) };
        ...
        if unsafe { (*wl).wl_lnum } == window.w_cursor.lnum {
        ...
        if unsafe { (*wl).wl_valid } && unsafe { (*wl).wl_lastlnum } + 1 == rg.mod_bot {
            ...
            i += 1;
            while i < window.w_lines_valid && !unsafe { *window.w_lines.add(i as usize) }.wl_valid {
                old_rows += unsafe { *window.w_lines.add(i as usize) }.wl_size as c_int;
                i += 1;
            }
            break;
        }
    ...
    while l < rg.mod_bot {
            ...
            && l == window.w_cursor.lnum
        {
            ...
            let (n, l_last) = unsafe { plines_correct_topline(window.raw(), l, true) };
            (l, new_rows) = (l_last, new_rows + n);
            j += c_int::from(n > 0); // do not count concealed lines
        }
        if new_rows > window.w_view_height - w.row - 2 {
    ...
    if xtra_rows < 0 {
        ...
        if w.row - xtra_rows >= window.w_view_height - 2 {
            ...
            unsafe { win_scroll_lines(window.raw(), w.row, xtra_rows) };
        ...
        if w.row + xtra_rows >= window.w_view_height - 2 {
            ...
            unsafe { win_scroll_lines(window.raw(), w.row + old_rows, xtra_rows) };
            rg.bot_scroll_start = 0;
...
unsafe fn move_line_entries(
    ...
    if j < i {
        ...
        loop {
            if i >= window.w_lines_valid {
                window.w_lines_valid = j;
                break;
            }
            unsafe { *window.w_lines.add(j as usize) = *window.w_lines.add(i as usize) };
            // Stop at a line that will not fit.
            if at_row + unsafe { *window.w_lines.add(j as usize) }.wl_size as c_int
                > window.w_view_height
            {
                window.w_lines_valid = j + 1;
                break;
            }
            at_row += unsafe { *window.w_lines.add(j as usize) }.wl_size as c_int;
            j += 1;
            i += 1;
        }
        ...
        // Downwards.
        j -= i;
        window.w_lines_valid = (window.w_lines_valid + j).min(window.w_view_height);
        let mut at = window.w_lines_valid;
        while at - j >= w.idx {
            unsafe { *window.w_lines.add(at as usize) = *window.w_lines.add((at - j) as usize) };
            at -= 1;
        }
        // The entries for the inserted lines are invalid now, but `wl_size`
        // is read above, so it is zeroed rather than left alone.
        while at >= w.idx {
            let wl = unsafe { window.w_lines.add(at as usize) };
            unsafe { (*wl).wl_size = 0 };
...
unsafe fn skip_one_line(
    ...
    if numbers_moved {
        let info = if window.w_onebuf_opt.wo_cul != 0 && w.lnum == window.w_cursor.lnum {
            cursorline_fi
        } else {
            fold_info(unsafe { Win::new(window.raw()) }, w.lnum)
        };
    ...
    w.lnum = unsafe { *window.w_lines.add((w.idx - 1) as usize) }.wl_lastlnum + 1;
...
unsafe fn draw_unfinished_last_line(mut window: Win, w: &Walk) {
    ...
    let at_attr =
        unsafe { hl_combine_attr(win_bg_attr(window.raw()), win_hl_attr(window.raw(), HLF_AT)) };

    if w.lnum == window.w_topline {
        // A single line that does not fit. Do not overwrite it -- it can
        // still be edited.
        window.w_botline = w.lnum + 1;
        return;
    }

    if unsafe { win_get_fill(Win::new(window.raw()), w.lnum) } >= window.w_view_height - w.srow {
    ...
    if dy_flags.get() & kOptDyFlagTruncate != 0 {
        ...
        grid_line_fill(
            (window.w_view_width - width).max(0),
            window.w_view_width,
            window.w_p_fcs_chars.lastline,
            at_attr,
        );
    ...
    set_empty_rows(unsafe { Win::new(window.raw()) }, w.srow);
    window.w_botline = w.lnum;
}
...
unsafe fn draw_end_of_buffer(window: Win, buffer: *mut Buffer, rg: &Regions, w: &Walk) {
    ...
    set_empty_rows(unsafe { Win::new(window.raw()) }, w.row);
}
...
pub unsafe fn win_scroll_lines(window: *mut Window, row: c_int, line_count: c_int) {
...
pub unsafe fn win_draw_end(
    ...
    for row in startrow..endrow {
        ...
        if draw_margin {
            ...
            if scwidth > 0 {
                n = grid_line_fill(
                    ...
                    schar_from_ascii(b' '),
                    unsafe { win_hl_attr(window.raw(), HLF_SC) },
                );
            }
        ...
        let attr = unsafe { win_hl_attr(window.raw(), hl) };
        if n < view_width {
            grid_line_put_schar(n, c1, attr);
            n += 1;
        }
        grid_line_clear_end(n, view_width, unsafe { win_bg_attr(window.raw()) }, attr);
