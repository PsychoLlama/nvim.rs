//! Lua bindings for `crate::api::command`.
//!
//! GENERATED by tools/apigen; see the parent module. Do not edit;
//! run `just apigen`.

#![deny(unsafe_op_in_unsafe_fn)]
// Unsafe perimeter: the `lua/` row in docs/perimeter.md.
#![allow(unsafe_code)]
#![deny(
    clippy::cast_lossless,
    clippy::cast_possible_truncation,
    clippy::cast_possible_wrap,
    clippy::cast_sign_loss,
    clippy::ptr_as_ptr
)]

use super::*;

/// The Lua binding for `nvim_buf_create_user_command`, as a `lua_CFunction`.
///
/// # Safety
/// LuaJIT's contract: `lstate` is the running Lua state, with this
/// binding's arguments on top of its stack and nothing of this frame's
/// below them. The ABI is `C-unwind` because a refused argument ends in
/// `lua_error`, which unwinds through this frame rather than returning.
pub unsafe extern "C-unwind" fn nlua_api_nvim_buf_create_user_command(
    lstate: *mut lua_State,
) -> c_int {
    /// Pop the arguments, call the API function, hand the result back.
    /// Each argument that owns Lua references arms a guard, so every way
    /// out releases exactly what was converted, in declaration order.
    ///
    /// # Safety
    /// The dispatcher's contract, which is what every `unsafe` below rests
    /// on: `lstate` is the running Lua state with this binding's arguments
    /// on top, and `call` is the binding's own.
    unsafe fn convert(lstate: *mut lua_State, call: &mut Call) -> Result<(), Error> {
        let Call { arena, err_param } = call;
        let mut arg_4 = KeyDictArg::<KeyDict_user_command>::unset();
        // SAFETY: as above.
        unsafe { pop_keydict(lstate, &mut arg_4, arena, err_param) }?;
        // SAFETY: as above.
        let arg_3 = unsafe { nlua_pop_object(lstate, true, arena) }
            .inspect_err(|_| *err_param = c"cmd".as_ptr().cast_mut())?;
        // SAFETY: as above.
        let arg_2 = unsafe { nlua_pop_string(lstate, arena) }
            .inspect_err(|_| *err_param = c"name".as_ptr().cast_mut())?;
        // SAFETY: as above.
        let arg_1 = unsafe { nlua_pop_handle(lstate, arena) }
            .inspect_err(|_| *err_param = c"buf".as_ptr().cast_mut())?;
        let _lstate = Restore::of(&active_lstate, lstate);
        // SAFETY: as above; the arguments are this binding's own.
        unsafe {
            nvim_buf_create_user_command(
                LUA_INTERNAL_CALL,
                arg_1,
                arg_2,
                arg_3,
                &raw mut arg_4.dict,
            )
        }?;
        Ok(())
    }
    // SAFETY: `lstate` is the state Lua called this binding on.
    unsafe { dispatch(lstate, c"nvim_buf_create_user_command", 4, 0, convert) }
}

/// The Lua binding for `nvim_buf_del_user_command`, as a `lua_CFunction`.
///
/// # Safety
/// LuaJIT's contract: `lstate` is the running Lua state, with this
/// binding's arguments on top of its stack and nothing of this frame's
/// below them. The ABI is `C-unwind` because a refused argument ends in
/// `lua_error`, which unwinds through this frame rather than returning.
pub unsafe extern "C-unwind" fn nlua_api_nvim_buf_del_user_command(
    lstate: *mut lua_State,
) -> c_int {
    /// Pop the arguments, call the API function, hand the result back.
    /// Each argument that owns Lua references arms a guard, so every way
    /// out releases exactly what was converted, in declaration order.
    ///
    /// # Safety
    /// The dispatcher's contract, which is what every `unsafe` below rests
    /// on: `lstate` is the running Lua state with this binding's arguments
    /// on top, and `call` is the binding's own.
    unsafe fn convert(lstate: *mut lua_State, call: &mut Call) -> Result<(), Error> {
        let Call { arena, err_param } = call;
        // SAFETY: as above.
        let arg_2 = unsafe { nlua_pop_string(lstate, arena) }
            .inspect_err(|_| *err_param = c"name".as_ptr().cast_mut())?;
        // SAFETY: as above.
        let arg_1 = unsafe { nlua_pop_handle(lstate, arena) }
            .inspect_err(|_| *err_param = c"buf".as_ptr().cast_mut())?;
        let _lstate = Restore::of(&active_lstate, lstate);
        // SAFETY: as above; the arguments are this binding's own.
        unsafe { nvim_buf_del_user_command(arg_1, arg_2) }?;
        Ok(())
    }
    // SAFETY: `lstate` is the state Lua called this binding on.
    unsafe { dispatch(lstate, c"nvim_buf_del_user_command", 2, 0, convert) }
}

/// The Lua binding for `nvim_buf_get_commands`, as a `lua_CFunction`.
///
/// # Safety
/// LuaJIT's contract: `lstate` is the running Lua state, with this
/// binding's arguments on top of its stack and nothing of this frame's
/// below them. The ABI is `C-unwind` because a refused argument ends in
/// `lua_error`, which unwinds through this frame rather than returning.
pub unsafe extern "C-unwind" fn nlua_api_nvim_buf_get_commands(lstate: *mut lua_State) -> c_int {
    /// Pop the arguments, call the API function, hand the result back.
    /// Each argument that owns Lua references arms a guard, so every way
    /// out releases exactly what was converted, in declaration order.
    ///
    /// # Safety
    /// The dispatcher's contract, which is what every `unsafe` below rests
    /// on: `lstate` is the running Lua state with this binding's arguments
    /// on top, and `call` is the binding's own.
    unsafe fn convert(lstate: *mut lua_State, call: &mut Call) -> Result<(), Error> {
        let Call { arena, err_param } = call;
        let mut arg_2 = KeyDictArg::<KeyDict_get_commands>::unset();
        // SAFETY: as above.
        unsafe { pop_keydict(lstate, &mut arg_2, arena, err_param) }?;
        // SAFETY: as above.
        let arg_1 = unsafe { nlua_pop_handle(lstate, arena) }
            .inspect_err(|_| *err_param = c"buf".as_ptr().cast_mut())?;
        let _lstate = Restore::of(&active_lstate, lstate);
        // SAFETY: as above; the arguments are this binding's own.
        let mut ret = unsafe { nvim_buf_get_commands(arg_1, &raw mut arg_2.dict) }?;
        // SAFETY: as above.
        unsafe { nlua_push_dict(lstate, &mut ret, PUSH_SPECIAL) };
        Ok(())
    }
    // SAFETY: `lstate` is the state Lua called this binding on.
    unsafe { dispatch(lstate, c"nvim_buf_get_commands", 2, 1, convert) }
}

/// The Lua binding for `nvim_cmd`, as a `lua_CFunction`.
///
/// # Safety
/// LuaJIT's contract: `lstate` is the running Lua state, with this
/// binding's arguments on top of its stack and nothing of this frame's
/// below them. The ABI is `C-unwind` because a refused argument ends in
/// `lua_error`, which unwinds through this frame rather than returning.
pub unsafe extern "C-unwind" fn nlua_api_nvim_cmd(lstate: *mut lua_State) -> c_int {
    /// Pop the arguments, call the API function, hand the result back.
    /// Each argument that owns Lua references arms a guard, so every way
    /// out releases exactly what was converted, in declaration order.
    ///
    /// # Safety
    /// The dispatcher's contract, which is what every `unsafe` below rests
    /// on: `lstate` is the running Lua state with this binding's arguments
    /// on top, and `call` is the binding's own.
    unsafe fn convert(lstate: *mut lua_State, call: &mut Call) -> Result<(), Error> {
        let Call { arena, err_param } = call;
        let mut arg_2 = KeyDictArg::<KeyDict_cmd_opts>::unset();
        // SAFETY: as above.
        unsafe { pop_keydict(lstate, &mut arg_2, arena, err_param) }?;
        let mut arg_1 = KeyDictArg::<KeyDict_cmd>::unset();
        // SAFETY: as above.
        unsafe { pop_keydict(lstate, &mut arg_1, arena, err_param) }?;
        let _lstate = Restore::of(&active_lstate, lstate);
        // SAFETY: as above; the arguments are this binding's own.
        let ret = unsafe { nvim_cmd(LUA_INTERNAL_CALL, &raw mut arg_1.dict, &raw mut arg_2.dict) }?;
        // SAFETY: as above.
        unsafe { nlua_push_string(lstate, &ret, PUSH_SPECIAL) };
        Ok(())
    }
    // SAFETY: `lstate` is the state Lua called this binding on.
    unsafe { dispatch(lstate, c"nvim_cmd", 2, 1, convert) }
}

/// The Lua binding for `nvim_create_user_command`, as a `lua_CFunction`.
///
/// # Safety
/// LuaJIT's contract: `lstate` is the running Lua state, with this
/// binding's arguments on top of its stack and nothing of this frame's
/// below them. The ABI is `C-unwind` because a refused argument ends in
/// `lua_error`, which unwinds through this frame rather than returning.
pub unsafe extern "C-unwind" fn nlua_api_nvim_create_user_command(lstate: *mut lua_State) -> c_int {
    /// Pop the arguments, call the API function, hand the result back.
    /// Each argument that owns Lua references arms a guard, so every way
    /// out releases exactly what was converted, in declaration order.
    ///
    /// # Safety
    /// The dispatcher's contract, which is what every `unsafe` below rests
    /// on: `lstate` is the running Lua state with this binding's arguments
    /// on top, and `call` is the binding's own.
    unsafe fn convert(lstate: *mut lua_State, call: &mut Call) -> Result<(), Error> {
        let Call { arena, err_param } = call;
        let mut arg_3 = KeyDictArg::<KeyDict_user_command>::unset();
        // SAFETY: as above.
        unsafe { pop_keydict(lstate, &mut arg_3, arena, err_param) }?;
        // SAFETY: as above.
        let arg_2 = unsafe { nlua_pop_object(lstate, true, arena) }
            .inspect_err(|_| *err_param = c"cmd".as_ptr().cast_mut())?;
        // SAFETY: as above.
        let arg_1 = unsafe { nlua_pop_string(lstate, arena) }
            .inspect_err(|_| *err_param = c"name".as_ptr().cast_mut())?;
        let _lstate = Restore::of(&active_lstate, lstate);
        // SAFETY: as above; the arguments are this binding's own.
        unsafe { nvim_create_user_command(LUA_INTERNAL_CALL, arg_1, arg_2, &raw mut arg_3.dict) }?;
        Ok(())
    }
    // SAFETY: `lstate` is the state Lua called this binding on.
    unsafe { dispatch(lstate, c"nvim_create_user_command", 3, 0, convert) }
}

/// The Lua binding for `nvim_del_user_command`, as a `lua_CFunction`.
///
/// # Safety
/// LuaJIT's contract: `lstate` is the running Lua state, with this
/// binding's arguments on top of its stack and nothing of this frame's
/// below them. The ABI is `C-unwind` because a refused argument ends in
/// `lua_error`, which unwinds through this frame rather than returning.
pub unsafe extern "C-unwind" fn nlua_api_nvim_del_user_command(lstate: *mut lua_State) -> c_int {
    /// Pop the arguments, call the API function, hand the result back.
    /// Each argument that owns Lua references arms a guard, so every way
    /// out releases exactly what was converted, in declaration order.
    ///
    /// # Safety
    /// The dispatcher's contract, which is what every `unsafe` below rests
    /// on: `lstate` is the running Lua state with this binding's arguments
    /// on top, and `call` is the binding's own.
    unsafe fn convert(lstate: *mut lua_State, call: &mut Call) -> Result<(), Error> {
        let Call { arena, err_param } = call;
        // SAFETY: as above.
        let arg_1 = unsafe { nlua_pop_string(lstate, arena) }
            .inspect_err(|_| *err_param = c"name".as_ptr().cast_mut())?;
        let _lstate = Restore::of(&active_lstate, lstate);
        // SAFETY: as above; the arguments are this binding's own.
        unsafe { nvim_del_user_command(arg_1) }?;
        Ok(())
    }
    // SAFETY: `lstate` is the state Lua called this binding on.
    unsafe { dispatch(lstate, c"nvim_del_user_command", 1, 0, convert) }
}

/// The Lua binding for `nvim_get_commands`, as a `lua_CFunction`.
///
/// # Safety
/// LuaJIT's contract: `lstate` is the running Lua state, with this
/// binding's arguments on top of its stack and nothing of this frame's
/// below them. The ABI is `C-unwind` because a refused argument ends in
/// `lua_error`, which unwinds through this frame rather than returning.
pub unsafe extern "C-unwind" fn nlua_api_nvim_get_commands(lstate: *mut lua_State) -> c_int {
    /// Pop the arguments, call the API function, hand the result back.
    /// Each argument that owns Lua references arms a guard, so every way
    /// out releases exactly what was converted, in declaration order.
    ///
    /// # Safety
    /// The dispatcher's contract, which is what every `unsafe` below rests
    /// on: `lstate` is the running Lua state with this binding's arguments
    /// on top, and `call` is the binding's own.
    unsafe fn convert(lstate: *mut lua_State, call: &mut Call) -> Result<(), Error> {
        let Call { arena, err_param } = call;
        let mut arg_1 = KeyDictArg::<KeyDict_get_commands>::unset();
        // SAFETY: as above.
        unsafe { pop_keydict(lstate, &mut arg_1, arena, err_param) }?;
        let _lstate = Restore::of(&active_lstate, lstate);
        // SAFETY: as above; the arguments are this binding's own.
        let mut ret = unsafe { nvim_get_commands(&raw mut arg_1.dict) }?;
        // SAFETY: as above.
        unsafe { nlua_push_dict(lstate, &mut ret, PUSH_SPECIAL) };
        Ok(())
    }
    // SAFETY: `lstate` is the state Lua called this binding on.
    unsafe { dispatch(lstate, c"nvim_get_commands", 1, 1, convert) }
}

/// The Lua binding for `nvim_parse_cmd`, as a `lua_CFunction`.
///
/// # Safety
/// LuaJIT's contract: `lstate` is the running Lua state, with this
/// binding's arguments on top of its stack and nothing of this frame's
/// below them. The ABI is `C-unwind` because a refused argument ends in
/// `lua_error`, which unwinds through this frame rather than returning.
pub unsafe extern "C-unwind" fn nlua_api_nvim_parse_cmd(lstate: *mut lua_State) -> c_int {
    /// Pop the arguments, call the API function, hand the result back.
    /// Each argument that owns Lua references arms a guard, so every way
    /// out releases exactly what was converted, in declaration order.
    ///
    /// # Safety
    /// The dispatcher's contract, which is what every `unsafe` below rests
    /// on: `lstate` is the running Lua state with this binding's arguments
    /// on top, and `call` is the binding's own.
    unsafe fn convert(lstate: *mut lua_State, call: &mut Call) -> Result<(), Error> {
        let Call { arena, err_param } = call;
        let mut arg_2 = KeyDictArg::<KeyDict_empty>::unset();
        // SAFETY: as above.
        unsafe { pop_keydict(lstate, &mut arg_2, arena, err_param) }?;
        // SAFETY: as above.
        let arg_1 = unsafe { nlua_pop_string(lstate, arena) }
            .inspect_err(|_| *err_param = c"str".as_ptr().cast_mut())?;
        let _lstate = Restore::of(&active_lstate, lstate);
        // SAFETY: as above; the arguments are this binding's own.
        let mut ret = unsafe { nvim_parse_cmd(arg_1, &raw mut arg_2.dict, arena) }?;
        // SAFETY: as above.
        unsafe { push_keydict(lstate, &raw mut ret) };
        Ok(())
    }
    // SAFETY: `lstate` is the state Lua called this binding on.
    unsafe { dispatch_fast(lstate, 2, 1, convert) }
}
