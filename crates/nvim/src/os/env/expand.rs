...
/// [`expand_env`] into an owned buffer of `capacity` bytes: the expansion, cut to
...
pub fn expand_env_into(src: &CStr, capacity: usize) -> Vec<u8> {
    let mut buf = vec![0 as c_char; capacity];
    let bound = c_int::try_from(capacity - 1).expect("a destination buffer smaller than INT_MAX");
    // SAFETY: `src` is NUL-terminated and `expand_env` only reads it; `buf`
    // holds `capacity` chars and the bound passed is one less, as upstream's is.
