...
pub unsafe fn vim_getenv(name: *const c_char) -> *mut c_char {
    ...
    unsafe {
        ...
        if vim_path.is_null() {
            // Uninitialized (as it is this early in startup) is not the same as
            // empty: it rules 'helpfile' out, and the exe-relative path decides.
            let from_helpfile = !P_HF.is_uninit() && !p_hf(|hf| has_char(hf, '$' as c_int));
            if from_helpfile {
