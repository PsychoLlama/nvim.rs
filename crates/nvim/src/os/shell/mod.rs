...
use crate::kvec::Kvec;
use crate::main::{State, do_profiling, emsg_silent, main_loop};
...
use crate::types::{
    LineNr, NUL, ProfTime, READBIN, StringBuilder, VarNumber, Vv, size_t, stream_read_cb,
};
...
pub unsafe fn call_shell(cmd: *mut c_char, opts: ShellOpts, extra_shell_arg: *mut c_char) -> c_int {
    ...
    unsafe {
        ...
        set_vim_var_nr(Vv::ShellError, retval as VarNumber);
        if do_profiling.get() == PROF_YES {
