...
unsafe fn decor_spell_nav_col(
    ...
    if *decor_lnum != lnum {
        unsafe { decor_redraw_reset(wp, state) };
        unsafe { decor_providers_invoke_spell(wp, lnum as c_int - 1, col, lnum as c_int - 1, -1) };
        unsafe { decor_redraw_line(wp, lnum as c_int - 1, state) };
        *decor_lnum = lnum;
    }
    unsafe { decor_redraw_col(wp, col, 0, false, state, MAXCOL as c_int) };
    state.spell
}
...
pub unsafe fn spell_move_to(
    ...
    while !got_int.get() {
        ...
        let mut len = unsafe { ml_get_buf_len((*wp).w_buffer, lnum) } as size_t;
        ...
        if capcol == 0 {
            ...
            line = unsafe { ml_get_buf((*wp).w_buffer, lnum) };
        }
        ...
        while p < endp {
            ...
            attr = HLF_COUNT;
            len = unsafe { spell_check(wp, p, &raw mut attr, &raw mut capcol, false) };
                    ...
                    || (behaviour == SMT_RARE && attr == HLF_SPR))
            {
                ...
                if past_cursor {
                    let no_plain_buffer =
                        unsafe { (*(*wp).w_s).b_p_spo_flags } & kOptSpoFlagNoplainbuffer != 0;
                    let mut can_spell = !no_plain_buffer;
                    let decor_says =
                        unsafe { decor_spell_nav_col(wp, lnum, &mut decor_lnum, col, decor) };
                    if decor_says == Some(true) {
                        can_spell = true;
                    } else if decor_says == Some(false) {
                        can_spell = false;
                    } else if has_syntax {
                        can_spell = unsafe { can_syn_spell(wp, lnum, col) };
                    }
        ...
        if lnum == unsafe { (*wp).w_cursor.lnum } && wrapped {
            break;
        }
        ...
        if dir == BACKWARD {
            if lnum > 1 {
                ...
                lnum = unsafe { (*(*wp).w_buffer).b_ml.ml_line_count };
                wrapped = true;
