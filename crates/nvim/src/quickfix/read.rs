...
impl Reader {
    ...
    unsafe fn open(
        ...
        if !enc.is_null() && unsafe { *enc } != 0 {
            let _ = p_enc(|value| unsafe {
                convert_setup(&raw mut reader.vc, enc, value.as_ptr().cast_mut())
            });
        }
        if !efile.is_null() {
    ...
    fn read_buffer(&mut self) -> Status {
        ...
        let text = lines.line_cstr(lnum, 0);
        self.len = self.fit(text.count_bytes());
        // SAFETY: the reader's own buffer, sized by `fit`, and a
        // NUL-terminated line.
        unsafe { xstrlcpy(self.line(), text.as_ptr(), self.len + 1) };
