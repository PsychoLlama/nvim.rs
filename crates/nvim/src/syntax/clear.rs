...
pub(crate) unsafe fn syntax_clear(block: *mut synblock_T) {
    unsafe { (*block).b_syn_error = false }; // clear previous error
    unsafe { (*block).b_syn_slow = false }; // clear previous timeout
    unsafe { (*block).b_syn_ic = 0 }; // Use case, by default
    unsafe { (*block).b_syn_foldlevel = SYNFLD_START };
    unsafe { (*block).b_syn_spell = SYNSPL_DEFAULT }; // default spell checking
    unsafe { (*block).b_syn_containedin = 0 };
    unsafe { (*block).b_syn_conceal = 0 };

    unsafe { clear_keywtab(&raw mut (*block).b_keywtab) };
    unsafe { clear_keywtab(&raw mut (*block).b_keywtab_ic) };

    // Last to first: `syn_clear_pattern` looks at the entry before it.
    let mut i = unsafe { (*block).b_syn_patterns.ga_len };
    while i > 0 {
        i -= 1;
        unsafe { syn_clear_pattern(block, i) };
    }
    unsafe { ga_clear(&raw mut (*block).b_syn_patterns) };

    let mut i = unsafe { (*block).b_syn_clusters.ga_len };
    while i > 0 {
        i -= 1;
        unsafe { syn_clear_cluster(block, i) };
    }
    unsafe { ga_clear(&raw mut (*block).b_syn_clusters) };
    unsafe { (*block).b_spell_cluster_id = 0 };
    unsafe { (*block).b_nospell_cluster_id = 0 };

    unsafe { (*block).b_syn_sync_flags = 0 };
    unsafe { (*block).b_syn_sync_minlines = 0 };
    unsafe { (*block).b_syn_sync_maxlines = 0 };
    unsafe { (*block).b_syn_sync_linebreaks = 0 };

    unsafe { vim_regfree((*block).b_syn_linecont_prog) };
    unsafe { (*block).b_syn_linecont_prog = ::core::ptr::null_mut() };
    unsafe { xfree((*block).b_syn_linecont_pat as *mut c_void) };
    unsafe { (*block).b_syn_linecont_pat = ::core::ptr::null_mut() };
    unsafe { (*block).b_syn_folditems = 0 };
    unsafe { clear_string_option(&raw mut (*block).b_syn_isk) };

    unsafe { syn_stack_free_all(block) };
...
unsafe fn syntax_sync_clear() {
    ...
    unsafe { syn_stack_free_all(block) }; // Need to recompute all syntax.
}
...
pub(crate) unsafe fn syn_remove_pattern(block: *mut synblock_T, idx: c_int) {
    ...
    if unsafe { (*spp).sp_flags }.has(SynFlags::FOLD) {
        unsafe { (*block).b_syn_folditems -= 1 };
    }
    unsafe { syn_clear_pattern(block, idx) };
    let after = unsafe { (*block).b_syn_patterns.ga_len } - idx - 1;
    unsafe { ::core::ptr::copy(spp.add(1), spp, after as usize) };
    unsafe { (*block).b_syn_patterns.ga_len -= 1 };
}
...
pub(crate) unsafe fn syn_cmd_clear(eap: *mut exarg_T, syncing: c_int) {
    ...
    // ":syntax clear" at all.
    if unsafe { (*cur_syn_block()).b_syn_topgrp } != 0 {
        return;
    }
    ...
    if ends_excmd(unsafe { *arg } as c_int) != 0 {
        ...
        if syncing != 0 {
            ...
            unsafe { syntax_clear(cur_syn_block()) };
            if unsafe { cur_syn_block() } == unsafe { &raw mut (*(*curwin.get()).w_buffer).b_s } {
        ...
        while ends_excmd(unsafe { *arg } as c_int) == 0 {
            ...
            if unsafe { *arg } as c_int == '@' as c_int {
                ...
                let scl = unsafe { &mut (*cur_cluster(id - SYNID_CLUSTER)).scl_list };
...
unsafe fn syn_clear_one(id: c_int, syncing: bool) {
    ...
    if !syncing {
        unsafe { syn_clear_keyword(id, &raw mut (*cur_syn_block()).b_keywtab) };
        unsafe { syn_clear_keyword(id, &raw mut (*cur_syn_block()).b_keywtab_ic) };
    }

    let mut idx = unsafe { cur_pattern_count() };
    while idx > 0 {
        idx -= 1;
