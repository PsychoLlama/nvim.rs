...
impl ZeroBlock {
    ...
    pub(crate) unsafe fn read(path: *const c_char) -> Result<Self, NoBlock> {
        ...
        let fd = unsafe { os_open(path, O_RDONLY, 0) };
        if fd < 0 {
            return Err(NoBlock::CannotOpen);
        }
        let mut b0: Self = unsafe { core::mem::zeroed() };
        let got = unsafe { read_eintr(fd, (&raw mut b0).cast(), size_of::<Self>()) };
        unsafe { close(fd) };
...
pub(crate) unsafe fn ml_upd_block0(buf: *mut buf_T, what: upd_block0_T) {
    ...
    if !ml_check_b0_id(unsafe { &*b0p }) {
        ...
        unsafe { set_b0_dir_flag(b0p, buf) };
    }
...
pub(crate) unsafe fn set_b0_fname(b0p: *mut ZeroBlock, buf: *mut buf_T) {
    ...
    if b.b_ffname.is_null() {
        ...
        // after the tilde.
        // SAFETY: block zero is the page the caller holds, and the three
        // callees below are path helpers that cannot reach it. The borrow
        ...
        if unsafe { os_fileinfo((*buf).b_ffname, &raw mut file_info) } {
            ...
            unsafe { buf_store_file_info(Buf::new(buf), &raw mut file_info) };
            b.b_mtime_read = b.b_mtime;
            b.b_mtime_read_ns = b.b_mtime_ns;
        } else {
            unsafe { b0_store_number(0, &mut (*b0p).b0_mtime) };
            unsafe { b0_store_number(0, &mut (*b0p).b0_ino) };
            b.b_mtime = 0;
            b.b_mtime_ns = 0;
            b.b_mtime_read = 0;
            b.b_mtime_read_ns = 0;
...
pub(crate) unsafe fn add_b0_fenc(b0p: *mut ZeroBlock, buf: *mut buf_T) {
    ...
    let name = unsafe { &mut (*b0p).b0_fname };
    let fits = unsafe { cstr::bytes_at(name.as_ptr()) }.len() + n < size;
    if fits {
        let at = size - n;
        unsafe { core::ptr::copy_nonoverlapping(fenc, name[at..].as_mut_ptr(), n) };
        name[at - 1] = NUL as c_char;
    }
    unsafe { (*b0p).set_flag(B0_HAS_FENC, fits) };
}
...
pub(crate) fn swapfile_proc_running(b0: &ZeroBlock, swap_fname: *const c_char) -> c_int {
    ...
    // are locals.
    let mut st: FileInfo = unsafe { core::mem::zeroed() };
    let mut uptime: c_double = 0.;
    if unsafe { os_fileinfo(swap_fname, &raw mut st) }
        && unsafe { uv_uptime(&raw mut uptime) } == 0
...
pub(crate) unsafe fn swapfile_unchanged(fname: *const c_char) -> bool {
    ...
    let Ok(mut b0) = (unsafe { ZeroBlock::read(fname) }) else {
        return false;
    };
    ...
    if b0.b0_hname[0] as c_int == NUL {
        ...
        unsafe { os_get_hostname(hostname.as_mut_ptr(), B0_HNAME_SIZE as size_t) };
        hostname[B0_HNAME_SIZE as usize - 1] = NUL as c_char;
        // In case of corruption.
        b0.b0_hname[B0_HNAME_SIZE as usize - 1] = NUL as c_char;
        if unsafe { strcasecmp(b0.b0_hname.as_ptr(), hostname.as_ptr()) } != 0 {
            ret = false;
        }
    }
...
pub(crate) unsafe fn swapfile_is_for_other_file(buf: *mut buf_T, fname: *mut c_char) -> bool {
    ...
    if let Ok(mut b0) = unsafe { ZeroBlock::read(fname) } {
        proc_running.set(swapfile_proc_running(&b0, fname));
        ...
        if b0.flags() & B0_SAME_DIR == 0
            || unsafe { path_fnamecmp(path_tail((*buf).b_ffname), path_tail(b0.b0_fname.as_ptr())) }
                != 0
            || !unsafe { same_directory(fname, (*buf).b_ffname) }
        {
            ...
            differ = unsafe { files_differ((*buf).b_ffname, name, ino) };
        }
    }
    differ
}
...
pub(crate) unsafe fn files_differ(
    ...
    let mut file_info: FileInfo = unsafe { core::mem::zeroed() };
    let ino_c: uint64_t = if unsafe { os_fileinfo(fname_c, &raw mut file_info) } {
        unsafe { os_fileinfo_inode(&raw mut file_info) }
    } else {
        0
    };
    let ino_s: uint64_t = if unsafe { os_fileinfo(fname_s, &raw mut file_info) } {
        unsafe { os_fileinfo_inode(&raw mut file_info) }
    } else {
...
pub unsafe fn ml_setflags(buf: *mut buf_T) {
    ...
    let _ = unsafe { mf_sync(mfp, MFS_ZERO as c_int) };
}
