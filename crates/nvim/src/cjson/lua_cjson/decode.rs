...
impl<'a> Parser<'a> {
    ...
    fn append_unicode_escape(&mut self) -> bool {
        ...
        let width = match codepoint {
            ...
            0x0080..=0x07FF => {
                utf8[0] = (codepoint >> 6) as u8 | 0xC0;
                utf8[1] = (codepoint & 0x3F) as u8 | 0x80;
                2
            }
            0x0800..=0xFFFF => {
                utf8[0] = (codepoint >> 12) as u8 | 0xE0;
                utf8[1] = ((codepoint >> 6) & 0x3F) as u8 | 0x80;
                utf8[2] = (codepoint & 0x3F) as u8 | 0x80;
                3
            }
            _ => {
                utf8[0] = (codepoint >> 18) as u8 | 0xF0;
                utf8[1] = ((codepoint >> 12) & 0x3F) as u8 | 0x80;
                utf8[2] = ((codepoint >> 6) & 0x3F) as u8 | 0x80;
                utf8[3] = (codepoint & 0x3F) as u8 | 0x80;
                4
            }
        };
    ...
    fn descend(&mut self, l: *mut lua_State, slots: c_int) {
        ...
        let (depth, at) = (self.depth, self.at as c_int);
        drop(core::mem::take(&mut self.scratch));
    ...
    unsafe fn push_scratch(&self, l: *mut lua_State) {
        unsafe {
            lua_pushlstring(
                l,
                self.scratch.as_ptr().cast::<c_char>(),
                self.scratch.len(),
            );
        }
    }
    ...
    unsafe fn parse_object(&mut self, l: *mut lua_State) {
        // .., table, key, value
        self.descend(l, 3);
        // SAFETY: `l` is live; every branch below balances the stack.
        unsafe {
            lua_newtable(l);
    ...
    unsafe fn push_value(&mut self, l: *mut lua_State, token: &Token, use_luanil: bool) {
        // SAFETY: `l` is live and has the slot `descend` reserved.
        unsafe {
            match *token {
                Token::Str => self.push_scratch(l),
                Token::Number(number) => lua_pushnumber(l, number),
                Token::Integer(number) => lua_pushinteger(l, number),
                Token::Boolean(value) => lua_pushboolean(l, c_int::from(value)),
                Token::ObjBegin => self.parse_object(l),
                Token::ArrBegin => self.parse_array(l),
                Token::Null => {
                    if use_luanil {
                        lua_pushnil(l);
                    } else {
...
fn scan_integer(text: &[u8]) -> (lua_Integer, usize) {
    ...
    (value as lua_Integer, signed + digits)
}
...
unsafe fn read_options(l: *mut lua_State) -> Options {
    ...
    // SAFETY: `l` is live; every push below is popped.
    unsafe {
        match lua_gettop(l) {
            1 => return options,
            2 => {}
        ...
        lua_getfield(l, 2, c"skip_comments".as_ptr());
        options.skip_comments = lua_toboolean(l, -1) != 0;
        lua_pop(l, 1);

        // `luanil` is the only option that is itself a table, and an absent
        // one ends the read: the two flags inside stay false.
        ...
        lua_getfield(l, -1, c"object".as_ptr());
        options.luanil_object = lua_toboolean(l, -1) != 0;
        lua_pop(l, 1);

        lua_getfield(l, -1, c"array".as_ptr());
        options.luanil_array = lua_toboolean(l, -1) != 0;
...
pub unsafe extern "C-unwind" fn decode(l: *mut lua_State) -> c_int {
    ...
    // value `push_value` left on the stack.
    unsafe {
        let token = parser.next_token();
        let luanil = parser.luanil_object;
        parser.push_value(l, &token, luanil);
