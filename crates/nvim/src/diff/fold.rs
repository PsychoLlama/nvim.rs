...
///
/// # Safety
/// `args` and `result` must be the evaluator's live cells.
pub unsafe fn f_diff_filler(args: *mut TypVal, result: *mut TypVal, _fptr: EvalFuncData) {
    ...
    unsafe { (*result).write_number(VarNumber::from(fill)) };
}
