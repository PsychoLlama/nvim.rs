...
use crate::src::nvim::mapping::langmap_init;
use crate::src::nvim::mbyte::enc_locale;
use crate::src::nvim::memory::{xfree, xmalloc, xmemdupz, xrealloc, xstrdup};
...
use crate::src::nvim::optionstr::{check_buf_options, free_string_option};
use crate::src::nvim::os::env::{os_env_exists, os_getenv, vim_getenv};
...
fn set_option_default(opt_idx: OptIndex, opt_flags: c_int) {
    ...
    unsafe {
        ...
        *insecure_flag(curwin.get(), opt_idx, opt_flags) &= !(kOptFlagInsecure as uint32_t);
        if both {
            *insecure_flag(curwin.get(), opt_idx, OPT_LOCAL) &= !(kOptFlagInsecure as uint32_t);
        }
    }
}
