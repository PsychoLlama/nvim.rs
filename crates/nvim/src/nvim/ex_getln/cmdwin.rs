...
pub(crate) unsafe fn open_cmdwin() -> ::core::ffi::c_int {
    unsafe {
        ...
        // "scratch" buffer.
        set_option_value_give_err(kOptBufhidden, static_optval(c"wipe"), OPT_LOCAL);
        ...
        if histtype == HIST_CMD || histtype == HIST_DEBUG {
            ...
            set_option_value_give_err(kOptFiletype, static_optval(c"vim"), OPT_LOCAL);
        }
        (*curbuf.get()).b_ro_locked -= 1;
        ...
        let save_count = crate::src::nvim::clipboard::save_batch_count();
