...
unsafe fn tag_strnicmp(s1: *const c_char, s2: *const c_char, len: usize) -> c_int {
    ...
    unsafe {
        for at in 0..len {
            let a = (*s1.add(at) as u8).to_ascii_uppercase();
            let b = (*s2.add(at) as u8).to_ascii_uppercase();
            if a != b {
...
impl FindTags {
        ...
        unsafe {
            match self.state {
                ...
                Reading::SkipBack => {
                    ...
                    if sinfo.curr_offset < 0 {
                        sinfo.curr_offset = 0;
                        vim_ignored.set(fseeko(self.fp, 0, SEEK_SET));
            ...
            if !eof && sinfo.curr_offset != 0 {
                ...
                if sinfo.curr_offset == sinfo.high_offset {
                    ...
                    vim_ignored.set(fseeko(self.fp, sinfo.low_offset, SEEK_SET));
    ...
    fn hdr_parse(&mut self) -> bool {
        ...
        unsafe {
            ...
            if strncmp(line, c"!_TAG_".as_ptr().cast_mut(), 6) != 0 {
                return true;
            }
            if strncmp(line, c"!_TAG_FILE_SORTED\t".as_ptr().cast_mut(), 18) == 0 {
                self.tag_file_sorted = *line.add(18) as u8 as c_int;
            }
            if strncmp(line, c"!_TAG_FILE_ENCODING\t".as_ptr().cast_mut(), 20) == 0 {
                ...
                let mut end = 20;
                while *line.add(end) as u8 > b' ' && (*line.add(end) as u8) < 127 {
                    end += 1;
                }
                *line.add(end) = 0;
                convert_setup(&raw mut self.vimconv, line.add(20), p_enc.get());
            }
    ...
    pub(crate) fn start_state(&mut self, margs: &mut MatchArgs, sinfo: &mut SearchInfo) -> bool {
        ...
        unsafe {
            ...
            if strncmp(self.lbuf.as_ptr(), c"!_TAG_".as_ptr(), 6) <= 0
            ...
            if fseeko(self.fp, 0, SEEK_END) != 0 {
    ...
    pub(crate) fn parse_line(
        ...
        unsafe {
            ...
            *tagp = core::mem::zeroed();
            ...
            let mut cmplen = tagp.tagname_end.offset_from(tagp.tagname) as c_int;
            ...
            match self.state {
                Reading::Binary => {
                    ...
                    let signed = *tagp.tagname as c_int;
                    let sort_key = if !margs.sortic {
                        *tagp.tagname as u8 as c_int
                    ...
                    let mut tagcmp = if margs.sortic {
                        tag_strnicmp(tagp.tagname, head, cmplen)
                    } else {
                        strncmp(tagp.tagname, head, cmplen)
                    };
                ...
                Reading::SkipBack => {
                    if mb_strnicmp(tagp.tagname, head, cmplen) != 0 {
                        self.state = Reading::StepForward;
                    } else {
            ...
            tagp.fname = tagp.tagname_end.add(1);
            tagp.fname_end = vim_strchr(tagp.fname, TAB);
            if tagp.fname_end.is_null() {
                return TagMatch::Fail;
            }
            tagp.command = tagp.fname_end.add(1);
            TagMatch::Success
        }
    }
        ...
        unsafe {
            ...
            let mut matched = self.orgpat.len == cmplen && {
                if self.orgpat.regmatch.rm_ic {
                    let same = mb_strnicmp(tagp.tagname, pat, len) == 0;
                    if same {
                        margs.match_no_ic = strncmp(tagp.tagname, pat, len) == 0;
                    }
                    same
                } else {
                    strncmp(tagp.tagname, pat, len) == 0
                }
            };
            ...
            if !matched && !self.orgpat.regmatch.regprog.is_null() {
                ...
                *tagp.tagname_end = 0;
                matched = vim_regexec(&raw mut self.orgpat.regmatch, tagp.tagname, 0);
                if matched {
                    margs.matchoff =
                        self.orgpat.regmatch.startp[0].offset_from(tagp.tagname) as c_int;
                    if self.orgpat.regmatch.rm_ic {
                        // Ask again with case, to find out how good the
                        // match is.
                        self.orgpat.regmatch.rm_ic = false;
                        margs.match_no_ic =
                            vim_regexec(&raw mut self.orgpat.regmatch, tagp.tagname, 0);
