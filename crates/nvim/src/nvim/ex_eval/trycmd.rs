...
use crate::src::nvim::eval::eval_to_string_skip;
use crate::src::nvim::eval::userfunc::do_return;
use crate::src::nvim::ex_docmd::{ends_excmd, find_nextcmd};
...
use crate::src::nvim::regexp::{RE_MAGIC, RE_STRING, skip_regexp_err, vim_regcomp, vim_regexec_nl};
use crate::src::nvim::runtime::do_finish;
use crate::src::nvim::types::{cleanup_T, cstack_T, eslist_T, exarg_T, except_T, regmatch_T};
...
pub unsafe fn ex_throw(eap: *mut exarg_T) {
    ...
    unsafe {
        ...
        if throw_exception(value.cast(), ET_USER, ptr::null_mut()) == FAIL {
            xfree(value.cast());
        } else {
            do_throw((*eap).cstack);
        }
...
pub unsafe fn do_throw(cstack: *mut cstack_T) {
    ...
    // SAFETY: module contract.
    unsafe {
        let idx = cleanup_conditionals(cstack, 0, false);
        if idx >= 0 {
            let flags = &raw mut (*cstack).cs_flags[idx as usize];
            ...
            if *flags & CSF_CAUGHT == 0 {
                if *flags & CSF_ACTIVE != 0 {
                    *flags |= CSF_THROWN;
                } else {
            ...
            *flags &= !CSF_ACTIVE;
            (*cstack).cs_pend.csp_ex[idx as usize] = current_exception.get().cast();
        }
    }
    did_throw.set(true);
}
...
pub unsafe fn ex_try(eap: *mut exarg_T) {
    ...
    unsafe {
        ...
        (*cstack).cs_flags[idx] = CSF_TRY;
        (*cstack).cs_pending[idx] = CSTP_NONE as c_char;

        if check_skip(cstack) {
            return;
        }
        ...
        if emsg_silent.get() != 0 {
            let elem: *mut eslist_T = xmalloc(size_of::<eslist_T>()).cast();
...
pub unsafe fn ex_catch(eap: *mut exarg_T) {
    ...
    unsafe {
        ...
        if (*cstack).cs_trylevel <= 0 || (*cstack).cs_idx < 0 {
            ...
            idx = (*cstack).cs_idx;
            while idx > 0 && (*cstack).cs_flags[idx as usize] & CSF_TRY == 0 {
                idx -= 1;
            }
        ...
        if ends_excmd(*(*eap).arg as c_int) != 0 {
            ...
            pat = (*eap).arg.add(1);
            end = skip_regexp_err(pat, *(*eap).arg as c_int, true as c_int);
            if end.is_null() {
                give_up = true;
            }
        }
        ...
        if !give_up {
            ...
            // throw.
            if !did_throw.get() || (*cstack).cs_flags[idx as usize] & CSF_TRUE == 0 {
                skip = true;
            }
                ...
                && (*cstack).cs_flags[idx as usize] & CSF_CAUGHT == 0
            {
                ...
                if !dbg_check_skipped(eap) || !do_intthrow(cstack) {
                    caught = pattern_catches(pat, end);
                }
            }
            ...
            if caught {
                ...
                got_int.set(false);
                did_throw.set(false);
                catch_exception((*cstack).cs_pend.csp_ex[idx as usize].cast::<except_T>());
                ...
                if (*cstack).cs_pend.csp_ex[(*cstack).cs_idx as usize].cast::<except_T>()
...
unsafe fn pattern_catches(pat: *mut c_char, end: *mut c_char) -> bool {
    ...
    unsafe {
        ...
        got_int.set(false);
        let caught = vim_regexec_nl(&raw mut regmatch, (*current_exception.get()).value, 0);
...
pub unsafe fn ex_finally(eap: *mut exarg_T) {
    ...
    unsafe {
        ...
        if (*cstack).cs_flags[(*cstack).cs_idx as usize] & CSF_TRUE == 0 {
            return;
        }
        ...
        if pending == CSTP_ERROR || did_emsg.get() != 0 || got_int.get() || did_throw.get() {
            ...
            if (*cstack).cs_pending[top] == CSTP_RETURN as c_char {
                report_discard_pending(CSTP_RETURN, (*cstack).cs_pend.csp_rv[top]);
                discard_pending_return((*cstack).cs_pend.csp_rv[top]);
            }
            ...
            (*cstack).cs_pending[top] = pending as c_char;
            ...
            if did_throw.get()
                && (*cstack).cs_pend.csp_ex[top].cast::<except_T>() != current_exception.get()
            {
...
pub unsafe fn ex_endtry(eap: *mut exarg_T) {
    ...
    unsafe {
            ...
            && got_int.get()
        {
            ...
            rethrow = did_throw.get() && (*cstack).cs_flags[idx as usize] & CSF_FINALLY == 0;
        }
        ...
        if !skip {
            ...
            if pending == CSTP_RETURN as c_char {
                ...
                current_exception.set((*cstack).cs_pend.csp_ex[idx as usize].cast());
            }
        }
        ...
        if !skip {
            ...
            match pending as c_int {
                ...
                CSTP_FINISH => do_finish(eap, false),
...
pub unsafe fn enter_cleanup(csp: *mut cleanup_T) {
    ...
    unsafe {
        ...
        report_make_pending(CSTP_NONE, (*csp).exception.cast());
    }
}
    ...
    unsafe {
        ...
        if aborting() || need_rethrow.get() {
            if pending & CSTP_THROW != 0 {
                ...
                report_discard_pending(pending, ptr::null_mut());
            }
