...
unsafe fn free_entry(kp: *mut keyentry_T) {
    unsafe {
        xfree((*kp).next_list as *mut c_void);
        xfree((*kp).k_syn.cont_in_list as *mut c_void);
        xfree(kp as *mut c_void);
    }
}
    ...
    unsafe {
        ...
        while todo > 0 {
            ...
            while !kp.is_null() {
                ...
                let kp_next = (*kp).ke_next;
                if kp_prev.is_null() {
                    if kp_next.is_null() {
                        hash_remove(ht, hi);
                    } else {
                        (*hi).hi_key = entry_to_key(kp_next);
                    }
                } else {
...
unsafe fn add_keyword(name: *mut c_char, namelen: size_t, def: &KeywordDef) {
    unsafe {
        ...
        let (name_ic, name_iclen) = if ignore_case {
            let folded = str_foldcase(
                name,
                namelen as c_int,
                &raw mut name_folded as *mut c_char,
                MAXKEYWLEN + 1,
            );
            (folded, strlen(folded))
        } else {
            (name, namelen)
        };
        ...
        // is sized for it.
        let kp = xmalloc(KEYWORD_OFFSET + name_iclen + 1) as *mut keyentry_T;
        let key = entry_to_key(kp);
        strcpy(key, name_ic);
        (*kp).k_syn.id = def.id as int16_t;
        ...
        let ht = if ignore_case {
            &raw mut (*cur_syn_block()).b_keywtab_ic
        } else {
            &raw mut (*cur_syn_block()).b_keywtab
        };
        let hi = hash_lookup(ht, key, strlen(key), hash);
        if (*hi).is_kept() {
            // The keyword already has entries: prepend to the chain.
            (*kp).ke_next = key_to_entry((*hi).hi_key);
            (*hi).hi_key = key;
        } else {
            (*kp).ke_next = ::core::ptr::null_mut();
...
unsafe fn add_keyword_variants(mut kw: *mut c_char, def: &KeywordDef) -> Option<*mut c_char> {
    unsafe {
        let mut kwlen;
        let mut p = vim_strchr(kw, '[' as c_int);
        loop {
            if p.is_null() {
                kwlen = strlen(kw);
            } else {
            ...
            let l = utfc_ptr2len(p.add(1));
            memmove(p as *mut c_void, p.add(1) as *const c_void, l as size_t);
            p = p.add(l as usize);
        }
        Some(kw.add(kwlen + 1))
    }
    ...
    unsafe {
        ...
        if !rest.is_null() {
            let syn_id = if (*eap).skip != 0 {
                ...
                syn_check_group(arg, group_name_end.offset_from(arg) as size_t)
            };
            if syn_id != 0 {
                ...
                while !rest.is_null() && ends_excmd(*rest as c_int) == 0 {
                    ...
                    while *rest as c_int != NUL && !ascii_iswhite(*rest as c_int) {
                        if *rest as c_int == '\\' as c_int && *rest.add(1) as c_int != NUL {
                            rest = rest.add(1);
                        }
                        *p = *rest;
                        p = p.add(1);
                ...
                if (*eap).skip == 0 {
                    ...
                    while cnt > 0 {
                        cnt -= 1;
                        match add_keyword_variants(kw, &def) {
                            Some(next) => kw = next,
                            None => break,
                        }
                    }
