...
use crate::src::nvim::ex_eval::{enter_cleanup, leave_cleanup};
...
use crate::src::nvim::types::{
    ...
    OptInt, bufref_T, cleanup_T, exarg_T, int64_t, linenr_T, size_t, win_T,
};
use crate::src::nvim::window::{
    check_can_set_curbuf_forceit, last_window, swbuf_goto_win_with_buf, win_close, win_locked,
    win_split,
};
use crate::src::nvim::winlayer::{buffers, windows};
...
fn unload_buffer(buf: Buf, action: c_int, flags: c_int, update_jumplist: &mut bool) -> Unloaded {
    ...
    while buf.raw() == curbuf.get()
        ...
        && (is_autocmd_window(lastwin.get()) || !is_last_window(cur_win()))
    {
