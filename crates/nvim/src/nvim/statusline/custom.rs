...
use crate::src::nvim::grid::{schar_from_ascii, schar_get};
use crate::src::nvim::highlight_group::{
    HLF_MSG, HLF_TPF, HLF_WBR, HLF_WBRNC, syn_id2attr, syn_name2id_len,
};
...
use crate::src::nvim::options::{kOptRulerformat, kOptStatusline, kOptTabline, kOptWinbar};
...
use crate::src::nvim::types::{
    Array, Integer, Object, OptIndex, OptInt, String_0, colnr_T, hlf_T, int64_t, schar_T, ssize_t,
    tabpage_T, win_T,
};
use crate::src::nvim::ui::{ui_call_msg_ruler, ui_has};
use crate::src::nvim::window::lastwin_nofloating;
...
fn screen_canvas() -> Canvas {
    ...
    unsafe { Canvas::new(default_grid.ptr()) }
}
...
impl Target {
    ...
    unsafe fn of(wp: *mut win_T, draw_winbar: bool, draw_ruler: bool) -> Option<(Target, Source)> {
        ...
        let own = || unsafe { Canvas::new(&raw mut (*wp).w_grid_alloc) };
        ...
        if draw_winbar {
            ...
            canvas = unsafe { Canvas::adjust(&raw mut (*wp).w_grid, &mut row, &mut col) };
            if row < 0 {
                return None;
            }
        ...
        let source = if draw_ruler {
            ...
            if !in_status_line {
                ...
                canvas = unsafe { Canvas::adjust(msg_grid_adj.ptr(), &mut row, &mut col) };
