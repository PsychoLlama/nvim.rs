...
use crate::src::nvim::os::libc::{strlen, strncmp};
use crate::src::nvim::spellsuggest::SCORE_REP;
use crate::src::nvim::spellsuggest::walk::{State, Walk};
use crate::src::nvim::types::{fromto_T, garray_T};
use core::ffi::c_int;
use core::ptr;
...
impl Walk {
    ...
    pub(super) unsafe fn rep_ini(&mut self) {
        // SAFETY: the first-byte index is a 256-entry table indexed by an
        // unsigned byte of the bad word.
        unsafe {
            ...
            self.stack[level].child = if self.soundfold {
                (*self.slang).sl_repsal_first[first_byte]
            } else {
                (*(*self.lp).lp_replang).sl_rep_first[first_byte]
            };
    ...
    pub(super) unsafe fn rep(&mut self) {
        // SAFETY: the item index stays inside the garray, and both sides
        // of every item are NUL-terminated strings the language owns.
        unsafe {
            let level = self.depth as usize;
            let p = self.fword_ptr(self.stack[level].bad_idx as usize);
            let gap = self.rep_items();

            while (self.stack[level].child as c_int) < (*gap).ga_len {
                let item =
                ...
                if *(*item).ft_from != *p {
                    ...
                    self.stack[level].child = (*gap).ga_len as i16;
                    break;
                }
                ...
                let from_len = strlen((*item).ft_from) as c_int;
                let to_len = strlen((*item).ft_to) as c_int;
                if from_len != to_len {
                    move_tail(p, from_len, to_len);
                    self.repextra += to_len - from_len;
                }
                ptr::copy((*item).ft_to, p, to_len as usize);
            ...
            if self.stack[level].child as c_int >= (*gap).ga_len
                && self.stack[level].state == State::Rep
            {
                self.stack[level].state = State::Final;
            }
    ...
    pub(super) unsafe fn rep_undo(&mut self) {
        ...
        // applied, whose two sides are NUL-terminated strings.
        unsafe {
            let level = self.depth as usize;
            let gap = self.rep_items();
            let item =
            ...
            let p = self.fword_ptr(self.stack[level].bad_idx as usize);
            if from_len != to_len {
                move_tail(p, to_len, from_len);
                self.repextra -= to_len - from_len;
            }
            ptr::copy((*item).ft_from, p, from_len as usize);
    ...
    unsafe fn rep_items(&self) -> *mut garray_T {
        ...
        unsafe {
            if self.soundfold {
                &raw mut (*self.slang).sl_repsal
            } else {
                &raw mut (*(*self.lp).lp_replang).sl_rep
            }
        }
    }
}
...
unsafe fn move_tail(p: *mut core::ffi::c_char, from_len: c_int, to_len: c_int) {
    ...
    unsafe {
        let src = p.offset(from_len as isize);
        ptr::copy(src, p.offset(to_len as isize), strlen(src) as usize + 1);
    }
}
