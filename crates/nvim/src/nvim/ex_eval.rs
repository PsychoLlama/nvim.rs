...
pub(crate) mod flag {
    use super::c_int;
    use crate::src::nvim::types::{estack_arg_T, except_type_T};
...
pub unsafe fn ex_else(eap: *mut exarg_T) {
    ...
    unsafe {
        ...
        if !skip && dbg_check_skipped(eap) && got_int.get() {
            do_intthrow(cstack);
            skip = true;
        }

        if (*eap).cmdidx != CMD_elseif {
            (*cstack).cs_flags[idx] |= CSF_ELSE;
            return;
        }
        ...
        if skip && *(*eap).arg != b'"' as c_char && ends_excmd(*(*eap).arg as c_int) != 0 {
            semsg_c!(message(&e_invexpr2), (*eap).arg);
        } else {
...
pub unsafe fn ex_while(eap: *mut exarg_T) {
    ...
    unsafe {
        ...
        // initialising.
        let jumped_back = (*cstack).cs_lflags & CSL_HAD_LOOP != 0;
        if !jumped_back {
            (*cstack).cs_idx += 1;
        ...
        let idx = (*cstack).cs_idx as usize;
        let is_while = (*eap).cmdidx == CMD_while;
        (*cstack).cs_flags[idx] = if is_while { CSF_WHILE } else { CSF_FOR };

        let skip = check_skip(cstack);
        let mut error = false;
        let result = if is_while {
            eval_to_bool((*eap).arg, &raw mut error, eap, skip, false)
        } else {
            for_next_item(eap, cstack, idx, jumped_back, skip, &mut error)
        };

        if !skip && !error && result {
            (*cstack).cs_flags[idx] |= CSF_ACTIVE | CSF_TRUE;
...
unsafe fn for_next_item(
    ...
    unsafe {
        ...
        let fi = if jumped_back {
            ...
            *error = false;
            (*cstack).cs_forinfo[idx]
        } else {
            let fi = eval_for_line((*eap).arg, error, eap, &raw mut evalarg);
            (*cstack).cs_forinfo[idx] = fi;
            fi
        };

        // Use the element at the start of the list and advance.
        let result = !*error && !fi.is_null() && !skip && next_for_item(fi, (*eap).arg);
        if !result {
            free_for_info(fi);
            (*cstack).cs_forinfo[idx] = ptr::null_mut();
        }
        clear_evalarg(&raw mut evalarg, eap);
        result
    }
...
pub unsafe fn ex_continue(eap: *mut exarg_T) {
    ...
    unsafe {
        ...
        let idx = cleanup_conditionals(cstack, CSF_WHILE | CSF_FOR, false);
        debug_assert!(idx >= 0, "idx >= 0");
        if (*cstack).cs_flags[idx as usize] & (CSF_WHILE | CSF_FOR) != 0 {
            rewind_conditionals(cstack, idx, CSF_TRY, &raw mut (*cstack).cs_trylevel);
            ...
            (*cstack).cs_pending[idx as usize] = CSTP_CONTINUE as c_char;
            report_make_pending(CSTP_CONTINUE, ptr::null_mut());
        }
    }
}
...
pub unsafe fn ex_break(eap: *mut exarg_T) {
    ...
    unsafe {
        ...
        let idx = cleanup_conditionals(cstack, CSF_WHILE | CSF_FOR, true);
        if idx >= 0 && (*cstack).cs_flags[idx as usize] & (CSF_WHILE | CSF_FOR) == 0 {
            (*cstack).cs_pending[idx as usize] = CSTP_BREAK as c_char;
            report_make_pending(CSTP_BREAK, ptr::null_mut());
        }
    }
}
...
pub unsafe fn ex_endwhile(eap: *mut exarg_T) {
    ...
    unsafe {
        ...
        let mut fl = (*cstack).cs_flags[(*cstack).cs_idx as usize];
        if fl & csf == 0 {
        ...
        while idx >= 0 {
            if (*cstack).cs_flags[idx as usize] & CSF_TRY != 0 {
                ...
                if (*cstack).cs_flags[idx as usize] & CSF_FINALLY == 0 {
                    if (*cstack).cs_flags[idx as usize] & CSF_ACTIVE != 0
                        && (*cstack).cs_flags[idx as usize] & CSF_CAUGHT != 0
                        && (*cstack).cs_flags[idx as usize] & CSF_FINISHED == 0
                    {
                        exception::finish_exception((*cstack).cs_pend.csp_ex[idx as usize].cast());
                        (*cstack).cs_flags[idx as usize] |= CSF_FINISHED;
                    }
                    ...
                    // throw.
                    if (*cstack).cs_flags[idx as usize] & CSF_TRUE != 0 {
            ...
            (*cstack).cs_flags[idx as usize] &= !CSF_ACTIVE;
            ...
            if (*cstack).cs_flags[idx as usize] & CSF_TRY != 0
                && (*cstack).cs_flags[idx as usize] & CSF_SILENT != 0
            {
                let elem: *mut eslist_T = (*cstack).cs_emsg_silent_list;
                (*cstack).cs_emsg_silent_list = (*elem).next;
                emsg_silent.set((*elem).saved_emsg_silent);
                xfree(elem.cast());
                (*cstack).cs_flags[idx as usize] &= !CSF_SILENT;
            }
            if stop {
                break;
            }
...
unsafe fn discard_finally_pending(cstack: *mut cstack_T, idx: c_int) {
    ...
    unsafe {
        ...
        let pending = (*cstack).cs_pending[idx as usize] as c_int;
        match pending {
            CSTP_NONE => {}
            CSTP_CONTINUE | CSTP_BREAK | CSTP_FINISH => {
                exception::report_discard_pending(pending, ptr::null_mut());
                (*cstack).cs_pending[idx as usize] = CSTP_NONE as c_char;
            }
            CSTP_RETURN => {
                ...
                discard_pending_return((*cstack).cs_pend.csp_rv[idx as usize]);
                (*cstack).cs_pending[idx as usize] = CSTP_NONE as c_char;
            }
            _ => {
                if (*cstack).cs_flags[idx as usize] & CSF_FINALLY == 0 {
                ...
                if pending & CSTP_THROW != 0 && !(*cstack).cs_pend.csp_ex[idx as usize].is_null() {
...
pub unsafe fn rewind_conditionals(
    ...
    unsafe {
        while (*cstack).cs_idx > idx {
            let top = (*cstack).cs_idx as usize;
            if (*cstack).cs_flags[top] & cond_type != 0 {
                *cond_level -= 1;
            }
            if (*cstack).cs_flags[top] & CSF_FOR != 0 {
                free_for_info((*cstack).cs_forinfo[top]);
            }
            (*cstack).cs_idx -= 1;
        }
    }
...
pub unsafe fn has_loop_cmd(p: *mut c_char) -> bool {
    ...
    unsafe {
        let mut p = p;
        loop {
            while *p == b' ' as c_char || *p == b'\t' as c_char || *p == b':' as c_char {
                p = p.add(1);
            }
        ...
        (*p == b'w' as c_char && *p.add(1) == b'h' as c_char)
            || (*p == b'f' as c_char && *p.add(1) == b'o' as c_char && *p.add(2) == b'r' as c_char)
    }
}
