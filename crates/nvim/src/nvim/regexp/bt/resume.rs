...
pub(crate) fn resume(stack: &mut RegStack, scan: &mut *mut uint8_t, status: &mut c_int) {
            ...
            match stack.top().rs_state {
                ...
                RS_BRANCH => {
                    if *status == RA_MATCH {
                        ...
                        if *status != RA_BREAK {
                            reg_restore(&stack.top().rs_un.regsave, backpos.ptr());
                ...
                RS_BRCPLX_MORE => {
                    if *status == RA_NOMATCH {
                        let rp = stack.top();
                        reg_restore(&rp.rs_un.regsave, backpos.ptr());
                ...
                RS_BEHIND1 => behind_start(stack, scan, status),
                RS_BEHIND2 => behind_step(stack, scan, status),
                RS_STAR_LONG | RS_STAR_SHORT => star(stack, scan, status),
                _ => {}
            }
...
/// # Safety
/// As `resume`.
unsafe fn behind_start(stack: &mut RegStack, scan: &mut *mut uint8_t, status: &mut c_int) {
    ...
    unsafe {
        ...
        reg_save(&mut bp.save_after, backpos.ptr());
...
/// # Safety
/// As `resume`.
unsafe fn behind_step(stack: &mut RegStack, scan: &mut *mut uint8_t, status: &mut c_int) {
    ...
    unsafe {
        ...
        if *status == RA_MATCH && reg_save_equal(&*behind_pos.ptr()) {
            ...
            if rp.rs_no as c_int == BEHIND {
                reg_restore(&bp.save_after, backpos.ptr());
            } else {
...
/// # Safety
/// As `resume`.
unsafe fn star(stack: &mut RegStack, scan: &mut *mut uint8_t, status: &mut c_int) {
