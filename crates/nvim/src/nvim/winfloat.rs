...
use crate::src::nvim::mouse::{MousePos, find_win_inner};
...
use crate::src::nvim::optionstr::{clear_string_option, free_string_option};
use crate::src::nvim::os::libc::{qsort, strlen};
...
pub unsafe extern "C" fn win_set_minimal_style(wp: *mut win_T) {
...
pub unsafe extern "C" fn win_border_height(wp: *mut win_T) -> c_int {
    // SAFETY: the caller's promise -- a live window.
    border_height(unsafe { Win::new(wp) })
}

pub unsafe extern "C" fn win_border_width(wp: *mut win_T) -> c_int {
    // SAFETY: the caller's promise -- a live window.
...
pub unsafe extern "C" fn win_config_float(wp: *mut win_T, fconfig: WinConfig) {
    // SAFETY: the caller's promise -- a live window.
...
pub unsafe extern "C" fn win_float_remove(bang: bool, mut count: c_int) {
...
pub unsafe extern "C" fn win_check_anchored_floats(win: *mut win_T) {
    // SAFETY: the caller's promise -- a live window.
...
pub unsafe extern "C" fn win_float_create_preview(enter: bool, new_buf: bool) -> *mut win_T {
