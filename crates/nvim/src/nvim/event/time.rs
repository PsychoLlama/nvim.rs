...
pub use crate::src::nvim::types::{
    ...
    Loop, LuaRef, MultiQueue, Proc, ProcType, QUEUE, RStream, ScopeType, Stream, TimeWatcher,
    ...
    pthread_rwlock_t, queue, rstream, size_t, ssize_t, stream, stream_close_cb, stream_read_cb,
    stream_uv as C2Rust_Unnamed_12, stream_write_cb, time_cb, time_watcher, uint8_t, uint64_t,
...
pub unsafe extern "C" fn time_watcher_init(
    ...
    uv_timer_init(&raw mut (*loop_0).uv, &raw mut (*watcher).uv);
...
pub unsafe extern "C" fn time_watcher_start(
    ...
    uv_timer_start(
        &raw mut (*watcher).uv,
        Some(time_watcher_cb as unsafe extern "C" fn(*mut uv_timer_t) -> ()),
        timeout,
        repeat,
    );
}
