...
unsafe fn wildmenu_process_key_menunames(cclp: Cc, key: c_int, xp: *mut expand_T) -> c_int {
    ...
    loop {
        ...
        let unescaped = unsafe { *buf.offset((j - 1) as isize) } != b'\\' as c_char;
        if unsafe { *buf.offset(j as isize) } == b' ' as c_char && unescaped {
...
unsafe fn wildmenu_process_key_filenames(cclp: Cc, key: c_int, xp: *mut expand_T) -> c_int {
    ...
    if key == K_DOWN && unsafe { cstr::prefix_eq(xp.xp_pattern, UPSEG_TAIL.as_ptr(), 3) } {
        ...
        loop {
            ...
            j -= unsafe { utf_head_off(buf, buf.offset(j as isize)) };
