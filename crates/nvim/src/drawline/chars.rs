...
impl Cells {
    ...
    pub(super) unsafe fn char_from_extra(&mut self, wlv: &mut WinLineVars, window: Win) {
            ...
            || (wlv.extra_todo == 1 && wlv.extra_last != NUL as ScreenChar)
        {
            ...
            self.char_len = unsafe { utfc_ptr2len(wlv.extra_text) };
            self.cell_char = unsafe { utfc_ptr2schar(wlv.extra_text, &raw mut self.char_code) };
            // `char_len` is 0 at the end-of-line NUL.
            if self.char_len > wlv.extra_todo || self.char_len == 0 {
                self.char_len = 1;
            }

            if wlv.col >= self.view_width - 1 && unsafe { schar_cells(self.cell_char) } == 2 {
                ...
                if wlv.cursorline_attr != 0 {
                    self.overflow_attr = if wlv.line_attr_lowprio != 0 {
                        ...
                        unsafe { hl_combine_attr(self.overflow_attr, wlv.cursorline_attr) }
                    };
                }
            } else {
                wlv.extra_todo -= self.char_len;
                wlv.extra_text = unsafe { wlv.extra_text.offset(self.char_len as isize) };
            }
        ...
        if self.extra_todo_next <= 0 {
            ...
            if self.area_attr == 0 && unsafe { *self.ptr } as ::core::ffi::c_int != NUL {
    ...
    pub(super) unsafe fn char_from_buffer(
        ...
        let mut c0 = unsafe { *self.ptr } as uint8_t as ::core::ffi::c_int;
        if c0 == NUL {
            // No more cells to skip.
            wlv.skip_cells = 0;
        }

        self.char_len = unsafe { utfc_ptr2len(self.ptr) };
        self.cell_char = unsafe { utfc_ptr2schar(self.ptr, &raw mut self.char_code) };
        ...
        if wlv.col >= self.view_width - 1 && unsafe { schar_cells(self.cell_char) } == 2 {
            ...
            self.ptr = unsafe { self.ptr.offset(self.char_len as isize - 1) };
        }
    ...
    ///
    /// # Safety
    /// `window` must be live, `f` must hold the caller's frame and `prev_ptr` must
    /// point into the line.
    pub(super) unsafe fn slow_path(
        ...
        let no_plain_buffer =
            unsafe { (*window.w_s).b_p_spo_flags } & kOptSpoFlagNoplainbuffer as uint32_t != 0;
        ...
        if self.has_decor && at > 0 {
            ...
            self.decor_attr = unsafe { hl_combine_attr(self.decor_attr, self.extmark_attr) };
        ...
        unsafe { self.spell_at(wlv, window, f, *prev_ptr, can_spell) };
        if self.spell_attr != 0 {
            self.attr_base = unsafe { hl_combine_attr(self.attr_base, self.spell_attr) };
            wlv.char_attr = unsafe { hl_combine_attr(self.attr_base, self.attr_pri) };
        }
    ...
    pub(super) unsafe fn spell_at(
        ...
        if !unsafe { (*f.spv).spv_has_spell } || at < self.word_end || at <= self.cur_checked_col {
            return;
        }
        self.spell_attr = 0;
        ...
        // space follows.
        if self.cell_char == 0
            || unsafe { *skipwhite(prev_ptr) } as ::core::ffi::c_int == NUL
            || !can_spell
        {
            return;
        }
        ...
        let p = if prev_at - self.nextlinecol as isize >= 0 {
            unsafe { f.nextline.offset(prev_at - self.nextlinecol as isize) }
        } else {
            prev_ptr
        };
        ...
        if spell_hlf != HLF_COUNT
            ...
            && window.w_cursor.lnum == wlv.lnum
            && window.w_cursor.col >= prev_at as ColNr
            && window.w_cursor.col < self.word_end
        {
            spell_hlf = HLF_COUNT;
        ...
        if spell_hlf == HLF_COUNT
            && p != prev_ptr
            && unsafe { p.offset_from(f.nextline) } + len as isize > self.nextline_idx as isize
        {
            // The good word continues at the start of the next line.
            unsafe { (*f.spv).spv_checked_lnum = wlv.lnum + 1 };
            unsafe {
        ...
        if unsafe { (*f.spv).spv_cap_col } > 0 {
            if p != prev_ptr
                && unsafe { p.offset_from(f.nextline) } + unsafe { (*f.spv).spv_cap_col } as isize
                    >= self.nextline_idx as isize
            {
                // The word on the next line must start with a capital.
                unsafe { (*f.spv).spv_capcol_lnum = wlv.lnum + 1 };
                unsafe {
    ...
    pub(super) unsafe fn linebreak(
        ...
        let mb_off = unsafe { utf_head_off(self.line, self.ptr.offset(-1)) };
        let p = unsafe { self.ptr.offset(-(mb_off as isize + 1)) };
    ...
    ///
    /// # Safety
    /// `window` must be live and `prev_ptr` point into the line.
        ...
        if window.w_onebuf_opt.wo_list != 0 {
            self.in_multispace = self.char_code == ' ' as ::core::ffi::c_int
                && (unsafe { *self.ptr } as ::core::ffi::c_int == ' ' as ::core::ffi::c_int
                    || (prev_ptr > self.line
                        && unsafe { *prev_ptr.offset(-1) } as ::core::ffi::c_int
                            == ' ' as ::core::ffi::c_int));
