...
use crate::mbyte::{
    mb_charlen, mb_off_next, mb_ptr2char_adv, mb_string2cells, str_char_at, utf_head_off,
    utf_ptr2cells, utf_ptr2char_info, utfc_next, utfc_ptr2len, utfc_ptr2schar,
};
...
pub unsafe fn win_line(
    ...
    let mut fold_buf: [::core::ffi::c_char; FOLD_TEXT_LEN as usize] = [0; FOLD_TEXT_LEN as usize];
...
unsafe fn wlv_put_linebuf(
        ...
        && !(window.w_onebuf_opt.wo_list != 0 && window.w_p_lcs_chars.prec != 0)
    {
        ...
        for _ in 0..3 {
            ...
            if off + 1 < window.w_view_width && line.chars()[off as usize + 1] == NUL as ScreenChar
            {
