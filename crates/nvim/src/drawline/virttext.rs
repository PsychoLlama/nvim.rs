...
pub(crate) unsafe fn line_putchar(
    ...
    if is_tab {
        ...
        dest[0] = unsafe { utfc_ptr2schar(p, &raw mut u8c) };
        if cells > 1 {
...
pub(crate) unsafe fn draw_virt_text(
    ...
    let max_col = window.w_view_width;
    ...
    for i in 0..end {
        ...
        if unsafe { (*item).start_row } != state.row || !unsafe { decor_virt_pos(item) } {
            continue;
        }
        ...
        if unsafe { (*item).draw_col } == -1 {
            let placed = match unsafe { decor_virt_pos_kind(item) } {
                kVPosEndOfLineRightAlign if do_eol => {
                    let mut eol_offset = 0;
                    if total_eol_right_width == 0 {
                        for j in i..end {
                            let ahead = decor_range_at(state, j);
                            if unsafe { (*ahead).start_row } != state.row
                                || !unsafe { decor_virt_pos(ahead) }
                                || unsafe { (*ahead).draw_col } != -1
                            {
                                continue;
                            }
                            if unsafe { decor_virt_pos_kind(ahead) } == kVPosEndOfLineRightAlign {
                ...
                kVPosRightAlign => {
                    right_pos -= unsafe { (*vt).width };
                    Some(right_pos)
                }
                kVPosEndOfLine if do_eol => Some(state.eol_col),
                kVPosWinCol => Some((col_off + unsafe { (*vt).col }).max(0)),
            ...
            if let Some(col) = placed {
                ...
                unsafe {
                    (*item).draw_col = if col < 0 || col >= window.w_view_width {
                        INT_MIN
                    } else {
                        col
                    }
        ...
        if !vt.is_null() {
            ...
            if do_eol
                && (unsafe { (*vt).pos } == kVPosEndOfLine
                    || unsafe { (*vt).pos } == kVPosEndOfLineRightAlign)
            {
        ...
        // a wrapped line.
        if vt.is_null()
            || unsafe { (*vt).flags } as ::core::ffi::c_int
...
pub(crate) unsafe fn draw_virt_text_item(
    ...
    while col < max_col {
        if skip_cells >= 0 && unsafe { *virt_str } as ::core::ffi::c_int == NUL {
            if virt_pos >= vt.size {
                break;
            }
            virt_attr = 0;
            virt_str = unsafe { next_virt_text_chunk(vt, &raw mut virt_pos, &raw mut virt_attr) };
            if virt_str.is_null() {
                break;
            }
        }
        ...
        while skip_cells > 0 && unsafe { *virt_str } as ::core::ffi::c_int != NUL {
            ...
            skip_cells -= cells;
            vcol += cells;
            virt_str = unsafe { virt_str.add(c_len as usize) };
        }
        ...
        if unsafe { *draw_str } as ::core::ffi::c_int == NUL {
            continue;
        }
...
impl WinLineVars {
    ...
    pub(crate) unsafe fn handle_inline_virtual_text(&mut self, v: ptrdiff_t, selected: bool) {
        ...
        while self.extra_todo == 0 {
            ...
            let mut attr = 0;
            let text = unsafe {
                next_virt_text_chunk(self.virt_inline, &raw mut self.virt_inline_i, &raw mut attr)
            };
            if text.is_null() {
                continue;
            }
            ...
            if self.skip_cells > 0 {
                ...
                let width = unsafe { mb_string2cells(self.extra_text) } as ::core::ffi::c_int;
                ...
                while remaining > 0 {
                    let cells = unsafe { utf_ptr2cells(self.extra_text) };
                    if cells > remaining {
                        break;
                    }
                    let c_len = unsafe { utfc_ptr2len(self.extra_text) };
                    remaining -= cells;
                    self.extra_text = unsafe { self.extra_text.add(c_len as usize) };
