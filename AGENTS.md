...
- The ratchet (`just ratchet --check`) constrains per-file unchecked lines (code inside `unsafe` regions), `static mut`/`#[no_mangle]`/variadic counts, `GlobalCell` raw escape-hatch uses (`.ptr()`/`.as_raw()`), file line counts, and internal exports to only shrink.
