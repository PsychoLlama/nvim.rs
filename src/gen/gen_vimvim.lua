...
local ex_cmds_file = arg[4]
local vvars_file = arg[5]
...
w('\n\n' .. vimfun_start)
-- Upstream decoded funcs_data.mpack here: the sorted builtin-function
-- names, including the vimscript-callable nvim_* API wrappers.
-- getcompletion over the running binary yields the same sorted list;
-- entries carry a trailing '(' or '()', and the name-shaped filter drops
-- runtime script functions (none under -u NONE, but belt and braces).
local funcs = {}
for _, c in ipairs(vim.fn.getcompletion('', 'function')) do
  local name = c:gsub('%(%)?$', '')
