...
extern int rs_ins_compl_add(char *str, int len, char *fname, char **cptext, int cptext_allocated, void *user_data, int cdir, int flags_arg, int adup, const int *user_hl, int score);
extern int rs_ins_compl_add_infercase(char *str, int len, int icase, const char *fname, int dir, int cont_s_ipos, int score);
int nvim_ins_compl_add_tv_impl(void *tv, int dir, int fast);
void nvim_ins_compl_add_list_impl(void *list); void nvim_ins_compl_add_dict_impl(void *dict);
void nvim_expand_by_function_full_impl(int type, char *base, void *cb); void *nvim_get_callback_if_cpt_func_impl(const char *p, int idx);
#define CTRL_X_WANT_IDENT 0x100
enum {
  CTRL_X_NORMAL = 0, CTRL_X_NOT_DEFINED_YET = 1, CTRL_X_SCROLL = 2, CTRL_X_WHOLE_LINE = 3,
  CTRL_X_FILES = 4, CTRL_X_TAGS = (5 + CTRL_X_WANT_IDENT), CTRL_X_PATH_PATTERNS = (6 + CTRL_X_WANT_IDENT),
  CTRL_X_PATH_DEFINES = (7 + CTRL_X_WANT_IDENT), CTRL_X_FINISHED = 8,
  CTRL_X_DICTIONARY = (9 + CTRL_X_WANT_IDENT), CTRL_X_THESAURUS = (10 + CTRL_X_WANT_IDENT),
  CTRL_X_CMDLINE = 11, CTRL_X_FUNCTION = 12, CTRL_X_OMNI = 13, CTRL_X_SPELL = 14,
  CTRL_X_LOCAL_MSG = 15, CTRL_X_EVAL = 16, CTRL_X_CMDLINE_CTRL_X = 17,
  CTRL_X_BUFNAMES = 18, CTRL_X_REGISTER = 19,
};
static char *ctrl_x_msgs[] = {
  N_(" Keyword completion (^N^P)"), N_(" ^X mode (^]^D^E^F^I^K^L^N^O^P^Rs^U^V^Y)"),
  NULL, N_(" Whole line completion (^L^N^P)"), N_(" File name completion (^F^N^P)"),
  N_(" Tag completion (^]^N^P)"), N_(" Path pattern completion (^N^P)"),
  N_(" Definition completion (^D^N^P)"), NULL, N_(" Dictionary completion (^K^N^P)"),
  N_(" Thesaurus completion (^T^N^P)"), N_(" Command-line completion (^V^N^P)"),
  N_(" User defined completion (^U^N^P)"), N_(" Omni completion (^O^N^P)"),
  N_(" Spelling suggestion (^S^N^P)"), N_(" Keyword Local completion (^N^P)"),
  NULL, N_(" Command-line completion (^V^N^P)"), NULL, N_(" Register completion (^N^P)"),
};
typedef struct compl_S compl_T;
struct compl_S {
  compl_T *cp_next; compl_T *cp_prev; compl_T *cp_match_next;
  String cp_str; char *(cp_text[CPT_COUNT]); typval_T cp_user_data; char *cp_fname;
  int cp_flags; int cp_number; int cp_score; bool cp_in_match_array;
  int cp_user_abbr_hlattr; int cp_user_kind_hlattr; int cp_cpt_source_idx;
};
typedef struct {
  char *e_cpt_copy; char *e_cpt; buf_T *ins_buf; pos_T *cur_match_pos; pos_T prev_match_pos;
  bool set_match_pos; pos_T first_match_pos; pos_T last_match_pos; bool found_all;
  char *dict; int dict_f; Callback *func_cb;
} ins_compl_next_state_T;
ins_compl_next_state_T ins_compl_st; bool ins_compl_st_cleared = false;
#define LOOKBACK_LINE_COUNT     1000
#include "insexpand_shim.c.generated.h"
typedef enum {
  CP_ORIGINAL_TEXT = 1, CP_FREE_FNAME = 2, CP_CONT_S_IPOS = 4,
  CP_EQUAL = 8, CP_ICASE = 16, CP_FAST = 32,
} cp_flags_T;
static const char e_compldel[] = N_("E840: Completion function deleted text");
compl_T *compl_first_match = NULL; compl_T *compl_curr_match = NULL; compl_T *compl_shown_match = NULL; compl_T *compl_old_match = NULL;
compl_T **compl_best_matches = NULL; int compl_num_bests = 0; bool compl_enter_selects = false;
String compl_leader = STRING_INIT; bool compl_get_longest = false; bool compl_used_match;
bool compl_was_interrupted = false; bool compl_interrupted = false; bool compl_started = false;
int ctrl_x_mode = CTRL_X_NORMAL; int compl_matches = 0;
String compl_pattern = STRING_INIT; String cpt_compl_pattern = STRING_INIT;
Direction compl_direction = FORWARD; Direction compl_shows_dir = FORWARD; pos_T compl_startpos;
int compl_length = 0; linenr_T compl_lnum = 0; colnr_T compl_col = 0; colnr_T compl_ins_end_col = 0;
String compl_orig_text = STRING_INIT; static extmark_undo_vec_t compl_orig_extmarks;
int compl_cont_mode = 0; expand_T compl_xp; win_T *compl_curr_win = NULL; buf_T *compl_curr_buf = NULL;
bool compl_autocomplete = false; uint64_t compl_timeout_ms = 80;
bool compl_time_slice_expired = false; bool compl_from_nonkeyword = false;
int compl_cont_status = 0; bool compl_opt_refresh_always = false; size_t spell_bad_len = 0; int compl_selected_item = -1;
typedef struct cpt_source_T { bool cs_refresh_always; int cs_startcol; int cs_max_matches; uint64_t compl_start_tv; char cs_flag; } cpt_source_T;
cpt_source_T *cpt_sources_array; int cpt_sources_count; int cpt_sources_index = -1; pumitem_T *compl_match_array = NULL; int compl_match_arraysize;
...
int ins_compl_add_infercase(char *str_arg, int len, bool icase, char *fname, Direction dir, bool cont_s_ipos, int score)
...
static dict_T *ins_compl_dict_alloc(compl_T *match)
{
  ...
  tv_dict_add_str(dict, S_LEN("word"), match->cp_str.data); tv_dict_add_str(dict, S_LEN("abbr"), match->cp_text[CPT_ABBR]);
  tv_dict_add_str(dict, S_LEN("menu"), match->cp_text[CPT_MENU]); tv_dict_add_str(dict, S_LEN("kind"), match->cp_text[CPT_KIND]); tv_dict_add_str(dict, S_LEN("info"), match->cp_text[CPT_INFO]);
  if (match->cp_user_data.v_type == VAR_UNKNOWN) { tv_dict_add_str(dict, S_LEN("user_data"), ""); } else { tv_dict_add_tv(dict, S_LEN("user_data"), &match->cp_user_data); }
  return dict;
}
...
void nvim_trigger_complete_changed_guarded(int cur)
{
  ...
  if (recursive) { return; }
  save_v_event_T save_v_event; dict_T *v_event = get_v_event(&save_v_event);
  ...
  recursive = true; textlock++; apply_autocmds(EVENT_COMPLETECHANGED, NULL, NULL, false, curbuf); textlock--; recursive = false;
  restore_v_event(v_event, &save_v_event);
}
static Callback cfu_cb; static Callback ofu_cb; static Callback tsrfu_cb; static Callback *cpt_cb; static int cpt_cb_count;
...
int set_cpt_callbacks(optset_T *args)
{
  ...
  clear_cpt_callbacks(&curbuf->b_p_cpt_cb, curbuf->b_p_cpt_count); curbuf->b_p_cpt_count = 0;
  int count = rs_get_cpt_sources_count();
  if (count == 0) { return OK; }
  curbuf->b_p_cpt_cb = xcalloc((size_t)count, sizeof(Callback)); curbuf->b_p_cpt_count = count;
  char buf[LSIZE]; int idx = 0;
  for (char *p = curbuf->b_p_cpt; *p != NUL;) {
    while (*p == ',' || *p == ' ') { p++; }
    if (*p != NUL) {
      size_t slen = copy_option_part(&p, buf, LSIZE, ",");
      if (slen > 0 && buf[0] == 'F' && buf[1] != NUL) { char *caret = vim_strchr(buf, '^'); if (caret != NULL) { *caret = NUL; } if (option_set_callback_func(buf + 1, &curbuf->b_p_cpt_cb[idx]) != OK) { curbuf->b_p_cpt_cb[idx].type = kCallbackNone; } }
      idx++;
    }
  }
...
bool set_ref_in_cpt_callbacks(Callback *callbacks, int count, int copyID)
  { if (callbacks == NULL) { return false; } bool abort = false; for (int i = 0; i < count; i++) { abort = abort || rs_set_ref_in_callback(&callbacks[i], copyID, NULL, NULL); } return abort; }
int nvim_set_ref_in_insexpand_funcs_impl(int copyID)
  { bool ab = rs_set_ref_in_callback(&cfu_cb, copyID, NULL, NULL) || rs_set_ref_in_callback(&ofu_cb, copyID, NULL, NULL) || rs_set_ref_in_callback(&tsrfu_cb, copyID, NULL, NULL) || set_ref_in_cpt_callbacks(cpt_cb, cpt_cb_count, copyID); return ab ? 1 : 0; }
...
{
  const int save_State = State; typval_T args[3]; args[0].v_type = VAR_NUMBER; args[1].v_type = VAR_STRING; args[2].v_type = VAR_UNKNOWN;
  args[0].vval.v_number = 1; args[1].vval.v_string = ""; pos_T pos = curwin->w_cursor; textlock++;
  colnr_T col = (colnr_T)callback_call_retnr((Callback *)cb_opaque, 2, args);
  textlock--; State = save_State; curwin->w_cursor = pos; check_cursor(curwin); validate_cursor(curwin);
...
{
  Callback *cb = (Callback *)cb_opaque; list_T *matchlist = NULL; dict_T *matchdict = NULL; typval_T rettv; const int save_State = State;
  assert(curbuf != NULL);
  if (cb == NULL) {
  ...
  typval_T args[3]; args[0].v_type = VAR_NUMBER; args[1].v_type = VAR_STRING; args[2].v_type = VAR_UNKNOWN;
  args[0].vval.v_number = 0; args[1].vval.v_string = base != NULL ? base : "";
  pos_T pos = curwin->w_cursor; textlock++;
  ...
  textlock--; curwin->w_cursor = pos; check_cursor(curwin); validate_cursor(curwin);
  if (!equalpos(curwin->w_cursor, pos)) { emsg(_(e_compldel)); goto theend; }
  if (matchlist != NULL) { nvim_ins_compl_add_list_impl(matchlist); } else if (matchdict != NULL) { nvim_ins_compl_add_dict_impl(matchdict); }
theend:
  State = save_State; if (matchdict != NULL) { tv_dict_unref(matchdict); } if (matchlist != NULL) { tv_list_unref(matchlist); }
}
int nvim_ins_compl_add_tv_impl(void *tv_opaque, int dir, int fast)
{
  typval_T *tv = (typval_T *)tv_opaque; const char *word; bool dup = false, empty = false;
  int flags = fast ? CP_FAST : 0; char *(cptext[CPT_COUNT]); int user_hl[2] = { -1, -1 };
  typval_T user_data; user_data.v_type = VAR_UNKNOWN;
  if (tv->v_type == VAR_DICT && tv->vval.v_dict != NULL) {
    word = tv_dict_get_string(tv->vval.v_dict, "word", false);
    cptext[CPT_ABBR] = tv_dict_get_string(tv->vval.v_dict, "abbr", true); cptext[CPT_MENU] = tv_dict_get_string(tv->vval.v_dict, "menu", true);
    cptext[CPT_KIND] = tv_dict_get_string(tv->vval.v_dict, "kind", true); cptext[CPT_INFO] = tv_dict_get_string(tv->vval.v_dict, "info", true);
    ...
    dup = (bool)tv_dict_get_number(tv->vval.v_dict, "dup"); empty = (bool)tv_dict_get_number(tv->vval.v_dict, "empty");
    if (tv_dict_get_string(tv->vval.v_dict, "equal", false) != NULL && tv_dict_get_number(tv->vval.v_dict, "equal")) { flags |= CP_EQUAL; }
  } else { word = tv_get_string_chk(tv); CLEAR_FIELD(cptext); }
  if (word == NULL || (!empty && *word == NUL)) { free_cptext(cptext); tv_clear(&user_data); return FAIL; }
  int status = rs_ins_compl_add((char *)word, -1, NULL, cptext, 1, &user_data, dir, flags, dup ? 1 : 0, user_hl, FUZZY_SCORE_NONE);
  if (status != OK) { tv_clear(&user_data); }
  return status;
}
void nvim_ins_compl_add_list_impl(void *list_opaque)
{
  list_T *list = (list_T *)list_opaque; Direction dir = compl_direction;
  TV_LIST_ITER(list, li, { if (nvim_ins_compl_add_tv_impl(TV_LIST_ITEM_TV(li), (int)dir, 1) == OK) { dir = FORWARD; } else if (did_emsg) { break; } });
}
void nvim_ins_compl_add_dict_impl(void *dict_opaque)
{
  dict_T *dict = (dict_T *)dict_opaque; compl_opt_refresh_always = false;
  dictitem_T *di_refresh = tv_dict_find(dict, S_LEN("refresh"));
  if (di_refresh != NULL && di_refresh->di_tv.v_type == VAR_STRING) { const char *v = di_refresh->di_tv.vval.v_string; if (v != NULL && strcmp(v, "always") == 0) { compl_opt_refresh_always = true; } }
  dictitem_T *di_words = tv_dict_find(dict, S_LEN("words"));
  if (di_words != NULL && di_words->di_tv.v_type == VAR_LIST) { nvim_ins_compl_add_list_impl(di_words->di_tv.vval.v_list); }
}
void *nvim_get_callback_if_cpt_func_impl(const char *p, int idx)
{
  if (*p == 'o') { return &curbuf->b_ofu_cb; }
