...
/// Get global mark index. Rust implementation.
static int global_mark_index(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_global_index(name);
}

/// Get local mark index. Rust implementation.
static int local_mark_index(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_local_index(name);
}

/// Check if mark is valid named mark. Rust implementation.
static bool is_valid_named_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_valid_named(name);
}

/// Check if mark is a file mark. Rust implementation.
static bool is_file_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_file_mark(name);
}

/// Check if mark is a jump mark. Rust implementation.
static bool is_jump_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_jump_mark(name);
}

/// Check if mark is special. Rust implementation.
static bool is_special_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_special(name);
}

/// Check if mark is visual mark. Rust implementation.
static bool is_visual_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_visual(name);
}

/// Check if mark is last cursor mark. Rust implementation.
static bool is_last_cursor_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_last_cursor(name);
}

/// Check if mark is last insert mark. Rust implementation.
static bool is_last_insert_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_last_insert(name);
}

/// Check if mark is last change mark. Rust implementation.
static bool is_last_change_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_last_change(name);
}

/// Check if mark is sentence mark. Rust implementation.
static bool is_sentence_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_sentence(name);
}

/// Check if mark is user settable. Rust implementation.
static bool is_user_settable_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_user_settable(name);
}

/// Check if mark is persistent (saved in ShaDa). Rust implementation.
static bool is_persistent_mark(int name)
  FUNC_ATTR_CONST
{
  return rs_mark_is_persistent(name);
}

/// Compare positions: a < b. Rust implementation.
static int pos_less_than(pos_T a, pos_T b)
  FUNC_ATTR_CONST
{
  return rs_lt(a, b);
}
...
/// Compare positions: a == b. Rust implementation.
static int pos_equal(pos_T a, pos_T b)
  FUNC_ATTR_CONST
{
  return rs_equalpos(a, b);
}

/// Compare positions: a <= b. Rust implementation.
static int pos_less_or_equal(pos_T a, pos_T b)
  FUNC_ATTR_CONST
{
  return rs_ltoreq(a, b);
}

/// Check if position is empty. Rust implementation.
static int pos_is_empty(pos_T a)
  FUNC_ATTR_CONST
{
  return rs_empty_pos(a);
}

/// Clear a position. Rust implementation.
static void clear_pos(pos_T *a)
{
  rs_clearpos(a);
}

/// Check if position is valid. Rust implementation.
static int pos_is_valid(pos_T pos)
  FUNC_ATTR_CONST
{
  return rs_pos_is_valid(pos);
}

/// Check if position is in range. Rust implementation.
static int pos_in_range(pos_T pos, int line_count)
  FUNC_ATTR_CONST
{
  return rs_pos_in_range(pos, line_count);
}

/// Compare two positions. Rust implementation.
static int compare_pos(pos_T a, pos_T b)
  FUNC_ATTR_CONST
{
  return rs_pos_compare(a, b);
}

/// Copy a position. Rust implementation.
static void copy_pos(pos_T *dst, const pos_T *src)
{
  rs_pos_copy(dst, src);
}

/// Get line number from position. Rust implementation.
static int get_pos_lnum(pos_T pos)
  FUNC_ATTR_CONST
{
  return rs_pos_get_lnum(pos);
}

/// Get column from position. Rust implementation.
static int get_pos_col(pos_T pos)
  FUNC_ATTR_CONST
{
  return rs_pos_get_col(pos);
}

/// Get column addition from position. Rust implementation.
static int get_pos_coladd(pos_T pos)
  FUNC_ATTR_CONST
{
  return rs_pos_get_coladd(pos);
}

/// Set line number in position. Rust implementation.
static void set_pos_lnum(pos_T *pos, int lnum)
{
  rs_pos_set_lnum(pos, lnum);
}

/// Set column in position. Rust implementation.
static void set_pos_col(pos_T *pos, int col)
{
  rs_pos_set_col(pos, col);
}

/// Set column addition in position. Rust implementation.
static void set_pos_coladd(pos_T *pos, int coladd)
{
  rs_pos_set_coladd(pos, coladd);
}

/// Create a new position. Rust implementation.
static pos_T make_pos(int lnum, int col, int coladd)
{
  return rs_pos_new(lnum, col, coladd);
}

/// Create a zero position. Rust implementation.
static pos_T zero_pos(void)
{
  return rs_pos_zero();
}

/// Adjust line number. Rust implementation.
static void adjust_pos_line(pos_T *pos, int delta)
{
  rs_pos_adjust_line(pos, delta);
}

/// Adjust column. Rust implementation.
static void adjust_pos_col(pos_T *pos, int delta)
{
  rs_pos_adjust_col(pos, delta);
}

/// Clamp position to range. Rust implementation.
static void clamp_pos(pos_T *pos, int max_lnum, int max_col)
{
  rs_pos_clamp(pos, max_lnum, max_col);
}

/// Get line distance between positions. Rust implementation.
static int line_distance(pos_T a, pos_T b)
  FUNC_ATTR_CONST
{
  return rs_pos_line_distance(a, b);
}

/// Check if positions are on same line. Rust implementation.
static int same_line(pos_T a, pos_T b)
  FUNC_ATTR_CONST
{
  return rs_pos_same_line(a, b);
}

/// Order two positions (swap if needed). Rust implementation.
static void order_pos(pos_T *a, pos_T *b)
{
  rs_pos_order(a, b);
}

/// Get mark name string. Rust implementation.
static void mark_name_str(int name, char *buf, size_t buf_len)
{
  rs_mark_get_name(name, buf, buf_len);
}

/// Get mark category string. Rust implementation.
static const char *mark_category(int name)
{
  return rs_mark_get_category(name);
}
