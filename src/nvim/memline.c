...
// Constants and flags
extern int rs_ml_get_empty_flag(void);
extern int rs_ml_get_line_dirty_flag(void);
extern int rs_ml_get_locked_dirty_flag(void);
extern int rs_ml_get_locked_pos_flag(void);
extern int rs_ml_get_allocated_flag(void);
extern int rs_ml_is_simple_action(int action);
extern uint16_t rs_ml_get_data_id(void);
extern uint16_t rs_ml_get_ptr_id(void);
extern uint32_t rs_ml_get_db_index_mask(void);
extern uint32_t rs_ml_get_db_marked(void);
extern uint8_t rs_ml_get_block0_id0(void);
extern uint8_t rs_ml_get_block0_id1(void);
extern int64_t rs_ml_get_b0_magic_long(void);
extern int32_t rs_ml_get_b0_magic_int(void);
extern int16_t rs_ml_get_b0_magic_short(void);
extern uint8_t rs_ml_get_b0_magic_char(void);
extern uint8_t rs_ml_get_b0_dirty(void);
extern uint8_t rs_ml_get_b0_ff_mask(void);
extern uint8_t rs_ml_get_b0_same_dir(void);
extern uint8_t rs_ml_get_b0_has_fenc(void);
extern int rs_ml_get_find_action(void);
extern int rs_ml_get_insert_action(void);
extern int rs_ml_get_delete_action(void);
extern int rs_ml_get_flush_action(void);
extern int rs_ml_get_chnk_addline(void);
extern int rs_ml_get_chnk_delline(void);
extern int rs_ml_get_chnk_updline(void);
extern int rs_ml_get_del_message_flag(void);
extern int rs_ml_get_append_new_flag(void);
extern int rs_ml_get_append_mark_flag(void);
extern size_t rs_ml_get_b0_fname_size_org(void);
extern size_t rs_ml_get_b0_fname_size_nocrypt(void);
extern size_t rs_ml_get_b0_fname_size_crypt(void);
extern size_t rs_ml_get_b0_uname_size(void);
extern size_t rs_ml_get_b0_hname_size(void);
extern size_t rs_ml_get_stack_incr(void);
extern size_t rs_ml_get_index_size(void);

// Access functions
extern colnr_T rs_ml_get_len(linenr_T lnum);
extern colnr_T rs_ml_get_buf_len(void *buf, linenr_T lnum);
extern colnr_T rs_ml_get_pos_len(void *pos);
extern char *rs_ml_get_pos(const void *pos);
extern int rs_gchar_pos(void *pos);
extern int rs_ml_line_cached(void *buf, linenr_T lnum);
extern linenr_T rs_ml_get_cached_lnum(void *buf);
extern colnr_T rs_ml_get_cached_len(void *buf);
extern char *rs_ml_get_cached_ptr(void *buf);
extern linenr_T rs_ml_get_line_count(void *buf);
extern int rs_ml_valid_lnum(void *buf, linenr_T lnum);
extern int rs_line_empty(linenr_T lnum);
extern int rs_line_empty_buf(void *buf, linenr_T lnum);
extern int rs_ml_line_alloced_buf(void *buf);

// Modify functions
extern int rs_ml_append_new_flag(void);
extern int rs_ml_append_mark_flag(void);
extern int rs_ml_del_message_flag(void);
extern int rs_ml_has_dirty_line(void *buf);
extern void rs_ml_clear_cached_line(void *buf);
extern int rs_ml_can_modify(void *buf, linenr_T lnum);
extern int rs_ml_can_append(void *buf, linenr_T lnum);
extern int rs_ml_is_empty(void *buf);

// Navigation functions
extern int rs_inc(void *lp);
extern int rs_incl(void *lp);
extern int rs_dec(void *lp);
extern int rs_decl(void *lp);
extern int rs_pos_valid(const void *pos);
extern int rs_pos_at_start(const void *pos);
extern int rs_pos_at_end(const void *pos);
extern int rs_ml_get_used_chunks(void *buf);

// Swap file functions
extern int rs_ml_has_swap(void *buf);
extern int rs_ml_needs_sync(void *buf);
extern int rs_ub_fname(void);
extern int rs_ub_same_dir(void);

// Recovery functions
extern int rs_ml_check_b0_id(const void *b0);
extern int rs_ml_check_b0_strings(const void *b0);
extern int rs_b0_magic_wrong(const void *b0);
extern void rs_long_to_char(int64_t n, char *s);
extern int64_t rs_char_to_long(const char *s);

// Phase 6: Wrapper functions for Rust implementations

/// Get ML_EMPTY flag. Rust implementation.
static int ml_flag_empty(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_empty_flag();
}

/// Get ML_LINE_DIRTY flag. Rust implementation.
static int ml_flag_line_dirty(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_line_dirty_flag();
}

/// Get ML_LOCKED_DIRTY flag. Rust implementation.
static int ml_flag_locked_dirty(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_locked_dirty_flag();
}

/// Get ML_LOCKED_POS flag. Rust implementation.
static int ml_flag_locked_pos(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_locked_pos_flag();
}

/// Get ML_ALLOCATED flag. Rust implementation.
static int ml_flag_allocated(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_allocated_flag();
}

/// Check if action is simple. Rust implementation.
static bool ml_action_is_simple(int action)
  FUNC_ATTR_PURE
{
  return rs_ml_is_simple_action(action) != 0;
}

/// Get DATA_ID constant. Rust implementation.
static uint16_t ml_const_data_id(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_data_id();
}

/// Get PTR_ID constant. Rust implementation.
static uint16_t ml_const_ptr_id(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_ptr_id();
}

/// Get DB_INDEX_MASK constant. Rust implementation.
static uint32_t ml_const_db_index_mask(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_db_index_mask();
}

/// Get DB_MARKED constant. Rust implementation.
static uint32_t ml_const_db_marked(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_db_marked();
}

/// Get BLOCK0_ID0 constant. Rust implementation.
static uint8_t ml_const_block0_id0(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_block0_id0();
}

/// Get BLOCK0_ID1 constant. Rust implementation.
static uint8_t ml_const_block0_id1(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_block0_id1();
}

/// Get B0_MAGIC_LONG constant. Rust implementation.
static int64_t ml_const_b0_magic_long(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_b0_magic_long();
}

/// Get B0_MAGIC_INT constant. Rust implementation.
static int32_t ml_const_b0_magic_int(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_b0_magic_int();
}

/// Get B0_MAGIC_SHORT constant. Rust implementation.
static int16_t ml_const_b0_magic_short(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_b0_magic_short();
}

/// Get B0_MAGIC_CHAR constant. Rust implementation.
static uint8_t ml_const_b0_magic_char(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_b0_magic_char();
}

/// Get B0_DIRTY constant. Rust implementation.
static uint8_t ml_const_b0_dirty(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_b0_dirty();
}

/// Get B0_FF_MASK constant. Rust implementation.
static uint8_t ml_const_b0_ff_mask(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_b0_ff_mask();
}

/// Get B0_SAME_DIR constant. Rust implementation.
static uint8_t ml_const_b0_same_dir(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_b0_same_dir();
}

/// Get B0_HAS_FENC constant. Rust implementation.
static uint8_t ml_const_b0_has_fenc(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_b0_has_fenc();
}

/// Get ML_FIND constant. Rust implementation.
static int ml_const_find(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_find_action();
}

/// Get ML_INSERT constant. Rust implementation.
static int ml_const_insert(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_insert_action();
}

/// Get ML_DELETE constant. Rust implementation.
static int ml_const_delete(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_delete_action();
}

/// Get ML_FLUSH constant. Rust implementation.
static int ml_const_flush(void)
  FUNC_ATTR_PURE
{
  return rs_ml_get_flush_action();
}

/// Get line length. Rust implementation.
static colnr_T ml_line_len(linenr_T lnum)
  FUNC_ATTR_PURE
{
  return rs_ml_get_len(lnum);
}

/// Get line length for buffer. Rust implementation.
static colnr_T ml_line_len_buf(buf_T *buf, linenr_T lnum)
  FUNC_ATTR_PURE
{
  return rs_ml_get_buf_len(buf, lnum);
}

/// Check if line is cached. Rust implementation.
static bool ml_line_is_cached(buf_T *buf, linenr_T lnum)
  FUNC_ATTR_PURE
{
  return rs_ml_line_cached(buf, lnum) != 0;
}

/// Get cached line number. Rust implementation.
static linenr_T ml_cached_lnum(buf_T *buf)
  FUNC_ATTR_PURE
{
  return rs_ml_get_cached_lnum(buf);
}

/// Get cached line length. Rust implementation.
static colnr_T ml_cached_len(buf_T *buf)
  FUNC_ATTR_PURE
{
  return rs_ml_get_cached_len(buf);
}

/// Get cached line pointer. Rust implementation.
static char *ml_cached_ptr(buf_T *buf)
  FUNC_ATTR_PURE
{
  return rs_ml_get_cached_ptr(buf);
}

/// Get line count for buffer. Rust implementation.
static linenr_T ml_buf_line_count(buf_T *buf)
  FUNC_ATTR_PURE
{
  return rs_ml_get_line_count(buf);
}

/// Check if line number is valid. Rust implementation.
static bool ml_lnum_valid(buf_T *buf, linenr_T lnum)
  FUNC_ATTR_PURE
{
  return rs_ml_valid_lnum(buf, lnum) != 0;
}

/// Check if line is empty. Rust implementation.
static bool ml_is_line_empty(linenr_T lnum)
  FUNC_ATTR_PURE
{
  return rs_line_empty(lnum) != 0;
}

/// Check if line is empty in buffer. Rust implementation.
static bool ml_is_line_empty_buf(buf_T *buf, linenr_T lnum)
  FUNC_ATTR_PURE
{
  return rs_line_empty_buf(buf, lnum) != 0;
}

/// Check if cached line is allocated. Rust implementation.
static bool ml_cached_alloced(buf_T *buf)
  FUNC_ATTR_PURE
{
  return rs_ml_line_alloced_buf(buf) != 0;
}

/// Check if buffer has dirty line. Rust implementation.
static bool ml_buf_has_dirty(buf_T *buf)
  FUNC_ATTR_PURE
{
  return rs_ml_has_dirty_line(buf) != 0;
}

/// Clear cached line. Rust implementation.
static void ml_clear_cached(buf_T *buf)
{
  rs_ml_clear_cached_line(buf);
}

/// Check if can modify line. Rust implementation.
static bool ml_buf_can_modify(buf_T *buf, linenr_T lnum)
  FUNC_ATTR_PURE
{
  return rs_ml_can_modify(buf, lnum) != 0;
}

/// Check if can append to buffer. Rust implementation.
static bool ml_buf_can_append(buf_T *buf, linenr_T lnum)
  FUNC_ATTR_PURE
{
  return rs_ml_can_append(buf, lnum) != 0;
}

/// Check if buffer is empty. Rust implementation.
static bool ml_buf_is_empty(buf_T *buf)
  FUNC_ATTR_PURE
{
  return rs_ml_is_empty(buf) != 0;
}

/// Increment position. Rust implementation.
static int pos_inc(pos_T *pos)
{
  return rs_inc(pos);
}

/// Increment position with line wrap. Rust implementation.
static int pos_incl(pos_T *pos)
{
  return rs_incl(pos);
}

/// Decrement position. Rust implementation.
static int pos_dec(pos_T *pos)
{
  return rs_dec(pos);
}

/// Decrement position with line wrap. Rust implementation.
static int pos_decl(pos_T *pos)
{
  return rs_decl(pos);
}

/// Check if position is valid. Rust implementation.
static bool pos_is_valid(const pos_T *pos)
  FUNC_ATTR_PURE
{
  return rs_pos_valid(pos) != 0;
}

/// Check if position is at start. Rust implementation.
static bool pos_is_at_start(const pos_T *pos)
  FUNC_ATTR_PURE
{
  return rs_pos_at_start(pos) != 0;
}

/// Check if position is at end. Rust implementation.
static bool pos_is_at_end(const pos_T *pos)
  FUNC_ATTR_PURE
{
  return rs_pos_at_end(pos) != 0;
}

/// Get used chunks count. Rust implementation.
static int ml_used_chunks(buf_T *buf)
  FUNC_ATTR_PURE
{
  return rs_ml_get_used_chunks(buf);
}

/// Check if buffer has swap file. Rust implementation.
static bool ml_buf_has_swap(buf_T *buf)
  FUNC_ATTR_PURE
{
  return rs_ml_has_swap(buf) != 0;
}

/// Check if buffer needs sync. Rust implementation.
static bool ml_buf_needs_sync(buf_T *buf)
  FUNC_ATTR_PURE
{
  return rs_ml_needs_sync(buf) != 0;
}

/// Get UB_FNAME constant. Rust implementation.
static int ml_const_ub_fname(void)
  FUNC_ATTR_PURE
{
  return rs_ub_fname();
}

/// Get UB_SAME_DIR constant. Rust implementation.
static int ml_const_ub_same_dir(void)
  FUNC_ATTR_PURE
{
  return rs_ub_same_dir();
}

/// Check block 0 ID. Rust implementation.
static bool ml_b0_id_valid(const ZeroBlock *b0)
  FUNC_ATTR_PURE
{
  return rs_ml_check_b0_id(b0) != 0;
}

/// Check block 0 strings. Rust implementation.
static bool ml_b0_strings_valid(const ZeroBlock *b0)
  FUNC_ATTR_PURE
{
  return rs_ml_check_b0_strings(b0) != 0;
}

/// Check if block 0 magic is wrong. Rust implementation.
static bool ml_b0_magic_is_wrong(const ZeroBlock *b0)
  FUNC_ATTR_PURE
{
  return rs_b0_magic_wrong(b0) != 0;
}

/// Convert long to char array. Rust implementation.
static void ml_long_to_char_impl(int64_t n, char *s)
{
  rs_long_to_char(n, s);
}

/// Convert char array to long. Rust implementation.
static int64_t ml_char_to_long_impl(const char *s)
  FUNC_ATTR_PURE
{
  return rs_char_to_long(s);
}
