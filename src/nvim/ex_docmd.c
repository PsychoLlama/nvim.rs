...
// =============================================================================
// Phase 5: do_cmdline Rust port accessor functions
// =============================================================================

// --- Global state getters/setters ---

/// Get trylevel.
int nvim_docmd_get_trylevel(void) { return trylevel; }
/// Set trylevel.
void nvim_docmd_set_trylevel(int val) { trylevel = val; }

/// Get force_abort.
int nvim_docmd_get_force_abort(void) { return force_abort ? 1 : 0; }
/// Set force_abort.
void nvim_docmd_set_force_abort(bool val) { force_abort = val; }

/// Set suppress_errthrow.
void nvim_docmd_set_suppress_errthrow(bool val) { suppress_errthrow = val; }

/// Get did_throw.
int nvim_docmd_get_did_throw(void) { return did_throw ? 1 : 0; }
/// Set did_throw.
void nvim_docmd_set_did_throw(bool val) { did_throw = val; }

/// Set did_emsg (int).
void nvim_docmd_set_did_emsg(int val) { did_emsg = val; }

/// Set got_int.
void nvim_docmd_set_got_int(bool val) { got_int = val; }

/// Get did_endif.
int nvim_docmd_get_did_endif(void) { return did_endif ? 1 : 0; }
/// Set did_endif.
void nvim_docmd_set_did_endif(bool val) { did_endif = val; }

/// Set KeyTyped.
void nvim_docmd_set_keytyped(bool val) { KeyTyped = val; }
/// Get KeyTyped.
int nvim_docmd_get_keytyped(void) { return KeyTyped ? 1 : 0; }

/// Get debug_tick.
int nvim_docmd_get_debug_tick(void) { return debug_tick; }
/// Get debug_break_level.
int nvim_docmd_get_debug_break_level(void) { return debug_break_level; }

/// Get do_profiling.
int nvim_docmd_get_do_profiling(void) { return do_profiling; }
/// PROF_YES constant value.
int nvim_docmd_PROF_YES(void) { return PROF_YES; }

/// Get p_mfd option value.
int nvim_docmd_get_p_mfd(void) { return (int)p_mfd; }

/// Get SOURCING_LNUM.
linenr_T nvim_docmd_get_sourcing_lnum(void) { return SOURCING_LNUM; }
/// Set SOURCING_LNUM.
void nvim_docmd_set_sourcing_lnum(linenr_T lnum) { SOURCING_LNUM = lnum; }
/// Get SOURCING_NAME.
char *nvim_docmd_get_sourcing_name(void) { return SOURCING_NAME; }

/// msg_didout getter/setter.
int nvim_docmd_get_msg_didout(void) { return msg_didout ? 1 : 0; }
void nvim_docmd_set_msg_didout(bool val) { msg_didout = val; }
/// msg_didany setter.
void nvim_docmd_set_msg_didany(bool val) { msg_didany = val; }
/// no_wait_return getter/setter.
int nvim_docmd_get_no_wait_return(void) { return no_wait_return; }
void nvim_docmd_inc_no_wait_return(void) { no_wait_return++; }
void nvim_docmd_dec_no_wait_return(void) { no_wait_return--; }
/// RedrawingDisabled getter/setter.
int nvim_docmd_get_redrawing_disabled(void) { return RedrawingDisabled; }
void nvim_docmd_inc_redrawing_disabled(void) { RedrawingDisabled++; }
void nvim_docmd_dec_redrawing_disabled(void) { RedrawingDisabled--; }
/// need_wait_return setter.
void nvim_docmd_set_need_wait_return(bool val) { need_wait_return = val; }
/// msg_scroll setter.
void nvim_docmd_set_msg_scroll(bool val) { msg_scroll = val; }

/// last_cmdline getter/setter.
char *nvim_docmd_get_last_cmdline(void) { return last_cmdline; }
void nvim_docmd_set_last_cmdline(char *val) { last_cmdline = val; }
/// new_last_cmdline getter/setter.
char *nvim_docmd_get_new_last_cmdline(void) { return new_last_cmdline; }
void nvim_docmd_set_new_last_cmdline(char *val) { new_last_cmdline = val; }
/// repeat_cmdline getter.
char *nvim_docmd_get_repeat_cmdline(void) { return repeat_cmdline; }
void nvim_docmd_set_repeat_cmdline(char *val) { repeat_cmdline = val; }

/// e_command_too_recursive string (already exists but accessible for Rust).
// nvim_get_e_command_too_recursive already defined above.

// --- did_emsg_syntax ---
// nvim_docmd_set_did_emsg_syntax already defined above.

// --- cstack field accessors ---

/// Get cs_looplevel.
int nvim_cstack_get_looplevel(const cstack_T *cs) { return cs->cs_looplevel; }
/// Set cs_looplevel.
void nvim_cstack_set_looplevel(cstack_T *cs, int val) { cs->cs_looplevel = val; }

/// Get cs_trylevel.
int nvim_cstack_get_trylevel(const cstack_T *cs) { return cs->cs_trylevel; }

/// Get cs_lflags.
int nvim_cstack_get_lflags(const cstack_T *cs) { return cs->cs_lflags; }
/// Set cs_lflags.
void nvim_cstack_set_lflags(cstack_T *cs, int val) { cs->cs_lflags = val; }

/// Get cs_flags[idx].
int nvim_cstack_get_flags_idx(const cstack_T *cs, int idx) { return cs->cs_flags[idx]; }
/// Set cs_flags[idx] bits (OR in val).
void nvim_cstack_or_flags_idx(cstack_T *cs, int idx, int val) { cs->cs_flags[idx] |= val; }

/// Get cs_line[idx].
int nvim_cstack_get_line_idx(const cstack_T *cs, int idx) { return cs->cs_line[idx]; }
/// Set cs_line[idx].
void nvim_cstack_set_line_idx(cstack_T *cs, int idx, int val) { cs->cs_line[idx] = val; }

/// Get cs_pending[idx] (as int).
int nvim_cstack_get_pending_idx(const cstack_T *cs, int idx) { return (int)cs->cs_pending[idx]; }

/// Set cs_idx.
void nvim_cstack_set_idx(cstack_T *cs, int val) { cs->cs_idx = val; }

/// Check if cs_emsg_silent_list is non-NULL and clean it up.
void nvim_cstack_cleanup_emsg_silent_list(cstack_T *cs)
{
  if (cs->cs_emsg_silent_list != NULL) {
    eslist_T *temp;
    for (eslist_T *elem = cs->cs_emsg_silent_list; elem != NULL; elem = temp) {
      temp = elem->next;
      xfree(elem);
    }
  }
...
// --- lines_ga (garray_T / wcmd_T) wrappers ---

/// Allocate a garray_T for wcmd_T with grow size 10.
garray_T *nvim_docmd_lines_ga_alloc(void)
{
  garray_T *ga = xmalloc(sizeof(garray_T));
  ga_init(ga, (int)sizeof(wcmd_T), 10);
  return ga;
}

/// Free lines_ga (deep clear and free the struct itself).
void nvim_docmd_lines_ga_free(garray_T *ga)
{
  GA_DEEP_CLEAR(ga, wcmd_T, FREE_WCMD);
  xfree(ga);
}

/// Get ga_len.
int nvim_docmd_ga_len(const garray_T *ga) { return ga->ga_len; }

/// Deep clear the wcmd_T growarray.
void nvim_docmd_lines_ga_deep_clear(garray_T *ga)
{
  GA_DEEP_CLEAR(ga, wcmd_T, FREE_WCMD);
}

/// Get wcmd line at index.
char *nvim_docmd_lines_ga_get_line(const garray_T *ga, int idx)
{
  return ((wcmd_T *)ga->ga_data)[idx].line;
}

/// Get wcmd lnum at index.
linenr_T nvim_docmd_lines_ga_get_lnum(const garray_T *ga, int idx)
{
  return ((wcmd_T *)ga->ga_data)[idx].lnum;
}

/// Store a line in the lines_ga (wraps store_loop_line).
void nvim_docmd_lines_ga_store(garray_T *ga, char *line)
{
  store_loop_line(ga, line);
}

/// Check GA_EMPTY.
int nvim_docmd_lines_ga_empty(const garray_T *ga) { return GA_EMPTY(ga) ? 1 : 0; }

// --- loop_cookie struct accessors ---

/// Allocate a loop_cookie.
struct loop_cookie *nvim_docmd_loop_cookie_alloc(void)
{
  return xcalloc(1, sizeof(struct loop_cookie));
}

/// Free a loop_cookie.
void nvim_docmd_loop_cookie_free(struct loop_cookie *lc) { xfree(lc); }

/// Set loop_cookie fields.
void nvim_docmd_loop_cookie_init(struct loop_cookie *lc, garray_T *lines_gap,
                                  int current_line, LineGetter fgetline,
                                  void *cookie, bool repeating)
{
  lc->lines_gap = lines_gap;
  lc->current_line = current_line;
  lc->lc_getline = fgetline;
  lc->cookie = cookie;
  lc->repeating = repeating ? 1 : 0;
}

/// Get loop_cookie current_line.
int nvim_docmd_loop_cookie_get_current_line(const struct loop_cookie *lc)
{
  return lc->current_line;
}

/// Set loop_cookie current_line.
void nvim_docmd_loop_cookie_set_current_line(struct loop_cookie *lc, int val)
{
  lc->current_line = val;
}

// get_loop_line function pointer value for comparison.
LineGetter nvim_docmd_get_loop_line_ptr(void) { return get_loop_line; }

// --- msg_list save/restore ---

/// Block used to save/restore msg_list across do_cmdline.
typedef struct {
  msglist_T **saved;
  msglist_T *priv;
} nvim_msg_list_save_T;

/// Save and reset msg_list; return opaque save block.
void *nvim_docmd_save_msg_list(void)
{
  nvim_msg_list_save_T *blk = xmalloc(sizeof(nvim_msg_list_save_T));
  blk->saved = msg_list;
  blk->priv = NULL;
  msg_list = &blk->priv;
  return blk;
...
/// Restore msg_list from a saved block (returned by nvim_docmd_save_msg_list).
void nvim_docmd_restore_msg_list(void *blk_opaque)
{
  nvim_msg_list_save_T *blk = (nvim_msg_list_save_T *)blk_opaque;
  msg_list = blk->saved;
  xfree(blk);
}

// --- Debug/profile function cookie accessors ---

/// func_name wrapper.
char *nvim_docmd_func_name(void *cookie) { return func_name(cookie); }
/// func_breakpoint wrapper. Returns pointer to linenr_T*.
linenr_T *nvim_docmd_func_breakpoint(void *cookie) { return func_breakpoint(cookie); }
/// func_dbg_tick wrapper. Returns pointer to int*.
...
/// source_breakpoint wrapper. Returns pointer to linenr_T*.
linenr_T *nvim_docmd_source_breakpoint(void *cookie) { return source_breakpoint(cookie); }
/// source_dbg_tick wrapper. Returns pointer to int*.
...
linenr_T nvim_docmd_dbg_find_breakpoint(bool is_source, char *fname, linenr_T lnum)
{
  return dbg_find_breakpoint(is_source, fname, lnum);
}
/// dbg_breakpoint wrapper.
void nvim_docmd_dbg_breakpoint(char *fname, linenr_T lnum) { dbg_breakpoint(fname, lnum); }
/// do_debug wrapper.
void nvim_docmd_do_debug(const char *msg) { do_debug((char *)msg); }
...
int nvim_docmd_has_loop_cmd(const char *p) { return has_loop_cmd(p) ? 1 : 0; }

/// kUICmdline constant.
int nvim_docmd_kUICmdline(void) { return kUICmdline; }

/// ui_has wrapper for kUICmdline.
int nvim_docmd_ui_has_cmdline(void) { return ui_has(kUICmdline) ? 1 : 0; }

/// ui_ext_cmdline_block_append wrapper.
void nvim_docmd_ui_ext_cmdline_block_append(size_t indent, const char *line)
{
  ui_ext_cmdline_block_append(indent, line);
}
...
void nvim_docmd_msg_verbose_cmd(linenr_T lnum, char *cmd) { msg_verbose_cmd(lnum, cmd); }

/// line_breakcheck wrapper.
void nvim_docmd_line_breakcheck(void) { line_breakcheck(); }

/// report_make_pending wrapper for ":finally" handling.
void nvim_docmd_report_make_pending_finally(int pending, bool did_throw_val, void *exception)
{
  report_make_pending(pending, did_throw_val ? exception : NULL);
}

/// msg_start wrapper.
void nvim_docmd_msg_start(void) { msg_start(); }
/// wait_return wrapper.
void nvim_docmd_wait_return(void) { wait_return(false); }
/// msg_didout_before_start helpers.
int nvim_docmd_get_msg_didout_val(void) { return msg_didout ? 1 : 0; }

/// p_verbose getter.
long nvim_docmd_get_p_verbose(void) { return p_verbose; }

/// rewind_conditionals wrapper.
void nvim_docmd_rewind_conditionals(cstack_T *cs, int idx, int cond_type)
{
  rewind_conditionals(cs, idx, cond_type, &cs->cs_looplevel);
}

/// cleanup_conditionals wrapper.
int nvim_docmd_cleanup_conditionals(cstack_T *cs)
{
  return cleanup_conditionals(cs, 0, true);
}

/// do_errthrow for do_cmdline end (takes cstack and is_func flag).
void nvim_docmd_do_errthrow_endfunction(cstack_T *cstack, bool is_func)
{
  do_errthrow(cstack, is_func ? "endfunction" : NULL);
}

/// e_endtry/endwhile/endfor/endif strings.
const char *nvim_docmd_get_e_endtry(void) { return _(e_endtry); }
const char *nvim_docmd_get_e_endwhile(void) { return _(e_endwhile); }
const char *nvim_docmd_get_e_endfor(void) { return _(e_endfor); }
const char *nvim_docmd_get_e_endif(void) { return _(e_endif); }

/// emsg wrapper.
void nvim_docmd_emsg(const char *msg) { emsg(msg); }

/// XFREE_CLEAR wrapper.
void nvim_docmd_xfree_clear_str(char **pp) { XFREE_CLEAR(*pp); }
// nvim_docmd_strmove: already defined above (line ~1829).

/// Check did_emsg_syntax.
int nvim_docmd_get_did_emsg_syntax(void) { return did_emsg_syntax ? 1 : 0; }
/// Clear did_emsg_syntax.
void nvim_docmd_clear_did_emsg_syntax(void) { did_emsg_syntax = false; }

/// Save/restore dbg_stuff via alloc'd block.
void *nvim_docmd_save_dbg_stuff_alloc(void)
{
  struct dbg_stuff *dsp = xmalloc(sizeof(struct dbg_stuff));
  save_dbg_stuff(dsp);
  return dsp;
}
void nvim_docmd_restore_dbg_stuff_free(void *dsp)
{
  restore_dbg_stuff((struct dbg_stuff *)dsp);
  xfree(dsp);
}

/// DOCMD_EXCRESET flag value.
int nvim_docmd_DOCMD_EXCRESET(void) { return DOCMD_EXCRESET; }
/// DOCMD_NOWAIT flag value.
int nvim_docmd_DOCMD_NOWAIT(void) { return DOCMD_NOWAIT; }
/// DOCMD_REPEAT flag value.
int nvim_docmd_DOCMD_REPEAT(void) { return DOCMD_REPEAT; }
/// DOCMD_KEEPLINE flag value.
int nvim_docmd_DOCMD_KEEPLINE(void) { return DOCMD_KEEPLINE; }
/// DOCMD_KEYTYPED flag value.
int nvim_docmd_DOCMD_KEYTYPED(void) { return DOCMD_KEYTYPED; }

/// Accessor: get e_command_too_recursive string.
const char *nvim_docmd_e_command_too_recursive(void)
{
  return _(e_command_too_recursive);
}

/// cstack_T size.
size_t nvim_docmd_sizeof_cstack(void) { return sizeof(cstack_T); }

// --- Additional loop_cookie field accessors ---

/// Get lc_getline from loop_cookie.
LineGetter nvim_docmd_loop_cookie_get_lc_getline(const struct loop_cookie *lc)
{
  return lc->lc_getline;
}

/// Get cookie from loop_cookie.
void *nvim_docmd_loop_cookie_get_cookie(const struct loop_cookie *lc)
{
  return lc->cookie;
}

/// Get current_exception pointer.
void *nvim_docmd_get_current_exception(void) { return (void *)current_exception; }

/// Get need_wait_return value.
int nvim_docmd_get_need_wait_return_val(void) { return need_wait_return ? 1 : 0; }

/// Check getline_equal(fgetline, cookie, getexline) with fgetline/cookie args.
int nvim_docmd_getline_is_getexline_fgetline(LineGetter fgetline, void *cookie)
{
  return getline_equal(fgetline, cookie, getexline) ? 1 : 0;
}

/// do_errthrow wrapper for do_cmdline (NULL eap, cstack and cmd_name directly).
void nvim_docmd_do_errthrow_cmdname(cstack_T *cstack, const char *cmd_name)
{
