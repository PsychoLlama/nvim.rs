...
void *nvim_timewatcher_get_data(TimeWatcher *tw) { return tw->data; }

/// Get the events queue from a TimeWatcher (accessor for Rust).
MultiQueue *nvim_timewatcher_get_events(TimeWatcher *tw) { return tw->events; }

/// Check if a TimeWatcher is blockable (accessor for Rust).
int nvim_timewatcher_is_blockable(TimeWatcher *tw) { return tw->blockable ? 1 : 0; }

/// Set the data pointer for a TimeWatcher (accessor for Rust).
void nvim_timewatcher_set_data(TimeWatcher *tw, void *data) { tw->data = data; }

/// Get the events queue from a TimeWatcher (accessor for Rust).
void nvim_timewatcher_set_events(TimeWatcher *tw, MultiQueue *events) { tw->events = events; }
