...
int nvim_drawscreen_get_resizing_autocmd(void) { return resizing_autocmd ? 1 : 0; }
int nvim_drawscreen_get_msg_grid_invalid(void) { return msg_grid_invalid ? 1 : 0; }
int nvim_default_grid_get_valid(void) { return default_grid.valid ? 1 : 0; }
void nvim_default_grid_set_valid(int val) { default_grid.valid = (val != 0); }

static void win_update(win_T *wp);  // forward declaration for win_update_wrapper
/// Wrapper for static win_update(), called from drawscreen_shim.c.
void nvim_win_update(win_T *wp) { win_update(wp); }
