...
/// Save p_cpo (returns old value) and set it to empty_string_option.
/// Used by do_searchpair in Rust; restore via nvim_restore_cpo.
char *nvim_eval_save_cpo(void) { char *s = p_cpo; p_cpo = empty_string_option; return s; }
...
// f_getmarklist: migrated to Rust (src/nvim-rs/eval/src/funcs/misc.rs)
// f_gettagstack: migrated to Rust (src/nvim-rs/window/src/viml.rs)
// f_prompt_getprompt: migrated to Rust (src/nvim-rs/eval/src/funcs/misc.rs)
// f_prompt_getinput: migrated to Rust (src/nvim-rs/eval/src/funcs/misc.rs)
// f_expandcmd: migrated to Rust (src/nvim-rs/eval/src/funcs/misc.rs)
// f_islocked: migrated to Rust (src/nvim-rs/eval/src/funcs/misc.rs)
// f_settagstack: migrated to Rust (src/nvim-rs/window/src/viml.rs)
...
// =============================================================================
// Phase 33: XDG helpers and accessors moved from funcs.c
// (Search helpers migrated to Rust: search_cmn.rs)
// =============================================================================
...
// nvim_eval_searchpair_cmn: migrated to Rust (search_cmn::do_searchpair_fn)
...
/// Error helpers for readfile/readblob (Phase 5+6 Rust migration).
void nvim_semsg_isadir2(const char *fname) { semsg(_(e_isadir2), fname); }
void nvim_semsg_notopen(const char *fname) { semsg(_(e_notopen), fname); }
void nvim_semsg_notread(const char *fname) { semsg(_(e_notread), fname); }
void nvim_semsg_notopen_empty(void) { semsg(_(e_notopen), _("<empty>")); }
...
// =============================================================================
// Phase 4: File finding/globbing functions (moved from eval/fs.c to allow
// Rust dispatching via #[export_name]).
// =============================================================================

static void nvim_findfilendir(typval_T *argvars, typval_T *rettv, int find_what)
{
  char *fresult = NULL;
...
void nvim_f_findfile(typval_T *argvars, typval_T *rettv, EvalFuncData fptr)
{
  nvim_findfilendir(argvars, rettv, FINDFILE_FILE);
}

void nvim_f_fnamemodify(typval_T *argvars, typval_T *rettv, EvalFuncData fptr)
{
  char *fbuf = NULL;
  size_t len = 0;
  char buf[NUMBUFLEN];
...
void nvim_f_glob(typval_T *argvars, typval_T *rettv, EvalFuncData fptr)
{
...
void nvim_f_globpath(typval_T *argvars, typval_T *rettv, EvalFuncData fptr)
{
