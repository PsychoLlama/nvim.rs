...
// Wrapper for Rust to call regmbc
void nvim_regmbc(int c)
{
  regmbc(c);
}
...
// Rust implementation for nfa_emit_equi_class
...
#ifdef REGEXP_DEBUG
// ============================================================================
// NFA Debug Functions - Thin C wrappers calling Rust implementations
// ============================================================================

// Rust FFI declarations for debug functions
extern void rs_nfa_set_code(int c, uint8_t *buf, size_t buf_len);
extern void rs_nfa_postfix_dump(const uint8_t *expr, int retval,
                                const int *post_start_ptr, const int *post_end_ptr);
extern void rs_nfa_print_state(FILE *debugf, nfa_state_T *state);
extern void rs_nfa_dump(nfa_regprog_T *prog);

static uint8_t code[50];

/// Set the code buffer with the name of an NFA opcode.
/// Calls the Rust implementation rs_nfa_set_code.
static void nfa_set_code(int c)
{
  rs_nfa_set_code(c, code, sizeof(code));
}

/// Print the postfix notation of the current regexp.
/// Calls the Rust implementation rs_nfa_postfix_dump.
static void nfa_postfix_dump(uint8_t *expr, int retval)
{
  rs_nfa_postfix_dump(expr, retval, post_start, post_ptr);
}

/// Print the NFA starting with a root node "state".
/// This is a thin wrapper that handles state marking before calling Rust.
...
static void nfa_print_state2(FILE *debugf, nfa_state_T *state, garray_T *indent)
{
  ...
  if (indent->ga_len >= 3) {
    ...
    strncpy((char *)save, (char *)&p[last], 2);  // NOLINT(runtime/printf)
    memcpy(&p[last], "+-", 2);
    fprintf(debugf, " %s", p);
    strncpy((char *)&p[last], (char *)save, 2);  // NOLINT(runtime/printf)
  } else {
    fprintf(debugf, " %s", p);
  }
...
// Note: The following code was migrated to Rust (rs_nfa_set_code).
// The original C implementation (~340 lines) has been deleted.
// See src/nvim-rs/regexp/src/nfa_states.rs for the Rust implementation.
