...
use crate::types::{HL_EXCLUDENL, HL_HAS_EOL};
...
unsafe fn syn_cmd_match_impl(eap: *mut c_void, syncing: c_int) {
    ...
    if !pat.is_null() && nvim_syn_vim_regcomp_had_eol() != 0 && (opt_flags & HL_EXCLUDENL) == 0 {
        // HL_HAS_EOL already set by compile_pattern for ITEM_END; for ITEM_START
        // (match) we set it here in the same way the old C code did.
