...
/// Check if an ID is a member of a cluster's ID list.
/// Handles ALLBUT (inverted) lists correctly.
#[no_mangle]
pub unsafe extern "C" fn rs_cluster_check_membership(
    cluster: SynClusterHandle,
    id: i16,
    ...
    let list = nvim_syncluster_get_list(cluster);
    if list.is_null() {
        return ClusterMembershipResult {
            is_member: 0,
            is_allbut: 0,
    ...
    let count = nvim_id_list_count(list);
    let is_allbut = nvim_id_list_first(list) == SYNID_ALLBUT as i16;

    // Search for the ID in the list
    let mut found = false;
    let start_idx = if is_allbut { 1 } else { 0 }; // Skip ALLBUT marker
    for i in start_idx..count {
        let item = nvim_id_list_get(list, i);
        if item == 0 {
            break;
        }
        if item == id {
    ...
    // With ALLBUT, membership is inverted
    let is_member = if is_allbut { !found } else { found };

    ClusterMembershipResult {
        is_member: if is_member { 1 } else { 0 },
        is_allbut: if is_allbut { 1 } else { 0 },
        list_count: count,
    }
}
...
/// Get the number of clusters in a synblock.
#[no_mangle]
pub unsafe extern "C" fn rs_synblock_cluster_count(block: SynBlockPtr) -> c_int {
    let handle = SynBlockHandle(block as *mut c_void);
    if handle.is_null() {
        return 0;
    }
    nvim_synblock_get_cluster_count(handle)
}

/// Check if a synblock has any clusters defined.
#[no_mangle]
pub unsafe extern "C" fn rs_synblock_has_clusters(block: SynBlockPtr) -> c_int {
    let handle = SynBlockHandle(block as *mut c_void);
    if handle.is_null() {
        return 0;
    }
    if nvim_synblock_get_cluster_count(handle) > 0 {
        1
    } else {
        0
    }
...
/// Check if a cluster is empty (no items in its list).
#[no_mangle]
pub unsafe extern "C" fn rs_cluster_is_empty(cluster: SynClusterHandle) -> c_int {
    if cluster.is_null() {
        return 1;
    }
    let list = nvim_syncluster_get_list(cluster);
    ...
    let first = nvim_id_list_first(list);
    if first == 0 {
        1
    } else {
        0
...
/// Get the number of items in a cluster's ID list.
/// For ALLBUT lists, this returns the count minus 1 (excluding the marker).
#[no_mangle]
pub unsafe extern "C" fn rs_cluster_item_count(cluster: SynClusterHandle) -> c_int {
    if cluster.is_null() {
        return 0;
    }
    let list = nvim_syncluster_get_list(cluster);
...
/// Check if a cluster uses ALLBUT (inverted membership).
#[no_mangle]
pub unsafe extern "C" fn rs_cluster_is_allbut(cluster: SynClusterHandle) -> c_int {
    if cluster.is_null() {
        return 0;
    }
    let list = nvim_syncluster_get_list(cluster);
    ...
    if nvim_id_list_first(list) == SYNID_ALLBUT as i16 {
        1
    } else {
        0
    }
...
/// Find a cluster by ID in a synblock.
/// Returns the cluster handle or null if not found.
#[no_mangle]
pub unsafe extern "C" fn rs_synblock_find_cluster_by_id(
    block: SynBlockPtr,
    cluster_id: c_int,
) -> SynClusterHandle {
    let handle = SynBlockHandle(block as *mut c_void);
    ...
    let count = nvim_synblock_get_cluster_count(handle);
    for i in 0..count {
        let cluster = nvim_synblock_get_cluster(handle, i);
        if !cluster.is_null() && nvim_syncluster_get_id(cluster) == cluster_id {
            return cluster;
        }
    }
    SynClusterHandle::null()
}

/// Get the index of a cluster in a synblock by its ID.
/// Returns -1 if not found.
#[no_mangle]
pub unsafe extern "C" fn rs_synblock_cluster_index_by_id(
    block: SynBlockPtr,
    cluster_id: c_int,
) -> c_int {
    let handle = SynBlockHandle(block as *mut c_void);
...
/// Parse a cluster operation string and return the operation code.
/// Returns -1 for invalid operation strings.
/// Recognizes: "=" for replace, "+=" for add, "-=" for subtract
#[no_mangle]
pub unsafe extern "C" fn rs_parse_cluster_op(op_str: *const c_char) -> c_int {
    if op_str.is_null() {
        return -1;
    }
...
/// Check if an ID list contains a specific ID (ignoring ALLBUT marker).
#[no_mangle]
pub unsafe extern "C" fn rs_id_list_contains(list: IdListHandle, id: i16) -> c_int {
    if list.is_null() {
        return 0;
    }
    let count = nvim_id_list_count(list);
    let is_allbut = nvim_id_list_first(list) == SYNID_ALLBUT as i16;
    let start = if is_allbut { 1 } else { 0 };
...
/// Count how many items in the list are cluster references.
#[no_mangle]
pub unsafe extern "C" fn rs_id_list_cluster_count(list: IdListHandle) -> c_int {
    if list.is_null() {
        return 0;
    }
    let count = nvim_id_list_count(list);
...
/// Count how many items in the list are direct group IDs (not clusters).
#[no_mangle]
pub unsafe extern "C" fn rs_id_list_group_count(list: IdListHandle) -> c_int {
    if list.is_null() {
        return 0;
    }
    let count = nvim_id_list_count(list);
    ...
    for i in start..count {
        ...
        if item > 0 && item < SYNID_CLUSTER as i16 {
            group_count += 1;
        }
    }
    group_count
...
#[no_mangle]
pub unsafe extern "C" fn rs_cluster_info(cluster: SynClusterHandle) -> ClusterInfo {
    if cluster.is_null() {
        return ClusterInfo {
            id: 0,
            item_count: 0,
    ...
    let id = nvim_syncluster_get_id(cluster);
    let list = nvim_syncluster_get_list(cluster);
    ...
    let first = nvim_id_list_first(list);
    let is_allbut = first == SYNID_ALLBUT as i16;
    let count = nvim_id_list_count(list);
    let item_count = if is_allbut && count > 0 {
        count - 1
    } else {
        count
    };
...
/// Get a summary of clusters in a synblock.
#[no_mangle]
pub unsafe extern "C" fn rs_synblock_cluster_summary(block: SynBlockPtr) -> SynblockClusterSummary {
    let handle = SynBlockHandle(block as *mut c_void);
    if handle.is_null() {
        return SynblockClusterSummary {
            total_count: 0,
            empty_count: 0,
            allbut_count: 0,
            has_spell: 0,
    ...
    let total_count = nvim_synblock_get_cluster_count(handle);
    let mut empty_count = 0;
    let mut allbut_count = 0;
    ...
    for i in 0..total_count {
        ...
        if first == 0 {
            empty_count += 1;
        } else if first == SYNID_ALLBUT as i16 {
            allbut_count += 1;
        }
    }

    let spell_id = nvim_synblock_get_spell_cluster_id(handle);
    let nospell_id = nvim_synblock_get_nospell_cluster_id(handle);

    SynblockClusterSummary {
        total_count,
        empty_count,
        allbut_count,
        has_spell: if spell_id > 0 { 1 } else { 0 },
        has_nospell: if nospell_id > 0 { 1 } else { 0 },
    }
}
