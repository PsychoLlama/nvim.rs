...
pub fn rs_should_search_rtp(flags: c_int) -> bool {
    (flags & dip::NORTP) == 0
}

/// Determine if we should search "after" directories.
pub fn rs_should_search_after(flags: c_int) -> bool {
...
pub fn rs_search_pack_start(flags: c_int) -> bool {
    (flags & dip::START) != 0
}

/// Check if searching pack/*/opt directories.
pub fn rs_search_pack_opt(flags: c_int) -> bool {
    (flags & dip::OPT) != 0
}
...
pub fn rs_pack_search_flags(want_start: bool, want_opt: bool, find_all: bool) -> c_int {
    let mut flags = 0;
    if want_start {
        flags |= dip::START;
    }
...
pub fn rs_get_pack_start_pattern() -> *const c_char {
    PACK_START_DIR.as_ptr().cast::<c_char>()
}

/// Get the "pack/*/opt/*" pattern.
pub fn rs_get_pack_opt_pattern() -> *const c_char {
    PACK_OPT_DIR.as_ptr().cast::<c_char>()
}

/// Get the "after" directory name.
pub fn rs_get_after_dir() -> *const c_char {
