...
/// Get VarType from raw value.
#[unsafe(no_mangle)]
pub extern "C" fn rs_vartype(value: c_int) -> c_int {
    VarType::from_raw(value).to_raw()
}

/// Check if type is numeric.
#[unsafe(no_mangle)]
pub extern "C" fn rs_vartype_is_numeric(value: c_int) -> c_int {
    c_int::from(VarType::from_raw(value).is_numeric())
}

/// Check if type is a container.
#[unsafe(no_mangle)]
pub extern "C" fn rs_vartype_is_container(value: c_int) -> c_int {
    c_int::from(VarType::from_raw(value).is_container())
}

/// Check if type is callable.
#[unsafe(no_mangle)]
pub extern "C" fn rs_vartype_is_callable(value: c_int) -> c_int {
    c_int::from(VarType::from_raw(value).is_callable())
}

/// Check if types can be coerced.
#[unsafe(no_mangle)]
pub extern "C" fn rs_can_coerce(from: c_int, to: c_int) -> c_int {
    c_int::from(can_coerce(VarType::from_raw(from), VarType::from_raw(to)))
}

/// Get common type for operation.
#[unsafe(no_mangle)]
pub extern "C" fn rs_common_type(a: c_int, b: c_int) -> c_int {
