...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_list_empty(qfl: QfListPtr) -> bool {
    if qfl.is_null() {
        return true;
    }
    (*qfl).qf_count <= 0
...
/// - If non-null, `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_list_has_valid_entries(qfl: QfListPtr) -> bool {
    if qfl.is_null() {
        return false;
    }
    // Has valid entries if list is not empty and not marked as nonevalid
    !rs_qf_list_empty(qfl) && !(*qfl).qf_nonevalid
}
...
pub unsafe extern "C" fn rs_qf_entry_after_pos(
    ...
    let qfp = qfp as *mut crate::ffi_types::QfLineRaw;
...
/// - If non-null, `qi` must be a valid pointer to a `qf_info_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_curlist(qi: QfInfoHandle) -> QfListPtr {
    if qi.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_list_at(qi: QfInfoHandle, idx: c_int) -> QfListPtr {
...
/// - If non-null, `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_size(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
    (*qfl).qf_count
...
pub unsafe extern "C" fn rs_qflist_valid(wp: WinHandle, qf_id: u32) -> bool {
    ...
    for i in 0..listcount {
        ...
        if !qfl.is_null() && (*qfl).qf_id == qf_id {
            return true;
        }
    }
    false
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_entry_present(qfl: QfListPtr, qf_ptr: QfLinePtr) -> bool {
...
pub unsafe extern "C" fn rs_qf_find_first_entry_on_line(
    ...
    let target_fnum = (*entry).qf_fnum;
    let target_line = (*entry).qf_lnum;

    loop {
        let prev = (*current).qf_prev;
        if prev.is_null() {
            break;
        }
        if (*prev).qf_fnum != target_fnum {
            break;
        }
        if (*prev).qf_lnum != target_line {
            break;
        }
        current = prev;
        current_idx -= 1;
...
pub unsafe extern "C" fn rs_qf_find_last_entry_on_line(
    ...
    loop {
        let next = (*current).qf_next;
        if next.is_null() {
            break;
        }
        if (*next).qf_fnum != target_fnum {
            break;
        }
        if (*next).qf_lnum != target_line {
            break;
        }
        current = next;
        current_idx += 1;
...
pub unsafe extern "C" fn rs_qf_find_entry_after_pos(
    ...
    if rs_qf_entry_after_pos(qfp.cast(), pos, linewise) {
        return qfp;
    }
    ...
    loop {
        ...
        if !rs_qf_entry_on_or_before_pos(next.cast(), pos, linewise) {
            break;
        }
        current = next;
        *errornr += 1;
...
pub unsafe extern "C" fn rs_qf_find_entry_before_pos(
    ...
    loop {
        ...
        if !rs_qf_entry_before_pos(next.cast(), pos, linewise) {
            break;
        }
        current = next;
        *errornr += 1;
    ...
    if rs_qf_entry_on_or_after_pos(current.cast(), pos, linewise) {
        return std::ptr::null_mut();
    }
...
pub unsafe extern "C" fn rs_qf_get_nth_below_entry(
    ...
    let mut current = entry;
    let mut remaining = n;
    let current_fnum = (*entry).qf_fnum;
    ...
    while remaining > 0 {
        ...
        let next = (*current).qf_next;
        if next.is_null() || (*next).qf_fnum != current_fnum {
            if linewise {
...
pub unsafe extern "C" fn rs_qf_get_nth_above_entry(
    ...
    while remaining > 0 {
        ...
        let prev = (*current).qf_prev;
        if prev.is_null() || (*prev).qf_fnum != current_fnum {
            break;
        }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_entry_count_in_file(qfl: QfListPtr, bnr: c_int) -> c_int {
    if qfl.is_null() {
        return 0;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_count_valid_entries(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_entry_at_idx(qfl: QfListPtr, idx: c_int) -> QfLinePtr {
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_iter_init(qfl: QfListPtr) -> QfIterator {
    if qfl.is_null() {
        return QfIterator {
...
#[no_mangle]
#[allow(clippy::missing_const_for_fn)] // extern "C" cannot be const
pub extern "C" fn rs_qf_iter_current(iter: &QfIterator) -> QfLinePtr {
    iter.current
}
...
pub unsafe extern "C" fn rs_qf_iter_next(iter: &mut QfIterator) {
    if !iter.current.is_null() {
        iter.current = (*iter.current).qf_next;
        iter.idx += 1;
    }
}
...
/// - `iter` must be a valid iterator obtained from `rs_qf_iter_init`
#[no_mangle]
pub unsafe extern "C" fn rs_qf_iter_next_valid(iter: &mut QfIterator) -> QfLinePtr {
    while !iter.current.is_null() && iter.idx <= iter.count {
        if (*iter.current).qf_valid != 0 {
...
/// - `iter` must be a valid iterator obtained from `rs_qf_iter_init`
#[no_mangle]
pub unsafe extern "C" fn rs_qf_iter_next_in_file(iter: &mut QfIterator, bnr: c_int) -> QfLinePtr {
    while !iter.current.is_null() && iter.idx <= iter.count {
        if (*iter.current).qf_fnum == bnr {
            let result = iter.current;
            rs_qf_iter_next(iter);
            return result;
        }
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_iter_reset(iter: &mut QfIterator, qfl: QfListPtr) {
    if qfl.is_null() {
        iter.current = std::ptr::null_mut();
        iter.idx = 0;
        iter.count = 0;
    } else {
        iter.current = (*qfl).qf_start;
        iter.idx = 1;
        iter.count = (*qfl).qf_count;
    }
}
...
pub unsafe extern "C" fn rs_qf_find_last_of_type(
    ...
    let mut iter = rs_qf_iter_init(qfl);
    let mut last_qfp: QfLinePtr = std::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_cmp_entries(a: QfLinePtr, b: QfLinePtr) -> c_int {
    if a.is_null() || b.is_null() {
        return 0;
    }

    // Compare by file number first
    let fnum_a = (*a).qf_fnum;
    let fnum_b = (*b).qf_fnum;
    if fnum_a != fnum_b {
        return fnum_a - fnum_b;
    }

    // Then by line number
    let lnum_a = (*a).qf_lnum;
    let lnum_b = (*b).qf_lnum;
    if lnum_a != lnum_b {
...
pub unsafe extern "C" fn rs_qf_entry_is_closer_to_target(
    ...
    let entry_fnum = (*entry).qf_fnum;
    let other_fnum = (*other_entry).qf_fnum;
    ...
    let entry_lnum = (*entry).qf_lnum;
    let other_lnum = (*other_entry).qf_lnum;
    ...
    let entry_col = (*entry).qf_col;
    let other_col = (*other_entry).qf_col;
...
/// - `qfp` must be a valid pointer to a `qfline_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_entry_in_file(qfp: QfLinePtr, bnr: c_int) -> bool {
    if qfp.is_null() {
        return false;
    }
    (*qfp).qf_fnum == bnr
...
/// - `qfp` must be a valid pointer to a `qfline_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_entry_is_active(qfp: QfLinePtr) -> bool {
    if qfp.is_null() {
        return false;
    }
    ((*qfp).qf_valid != 0) && ((*qfp).qf_cleared == 0)
}
...
/// - `qfp` must be a valid pointer to a `qfline_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_entry_has_type(qfp: QfLinePtr, type_char: c_char) -> bool {
    if qfp.is_null() {
        return false;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_id(qfl: QfListPtr) -> u32 {
    if qfl.is_null() {
        return 0;
    }
    (*qfl).qf_id
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_changedtick(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
    (*qfl).qf_changedtick
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_has_title(qfl: QfListPtr) -> bool {
    if qfl.is_null() {
        return false;
    }
    !(*qfl).qf_title.is_null()
}
...
/// - `qfp` must be a valid pointer to a `qfline_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_entry_covers_line(qfp: QfLinePtr, lnum: LinenrT) -> bool {
    if qfp.is_null() {
        return false;
    }
    let start = (*qfp).qf_lnum;
    let end = (*qfp).qf_end_lnum;
    if end == 0 {
...
/// - `qfp` must be a valid pointer to a `qfline_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_entry_covers_pos(qfp: QfLinePtr, lnum: LinenrT, col: c_int) -> bool {
    if qfp.is_null() {
        return false;
    }
    let start_lnum = (*qfp).qf_lnum;
    let end_lnum = (*qfp).qf_end_lnum;
    let start_col = (*qfp).qf_col;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_count_entries_of_type(qfl: QfListPtr, type_char: c_char) -> c_int {
    if qfl.is_null() {
        return 0;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_count_errors(qfl: QfListPtr) -> c_int {
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_count_warnings(qfl: QfListPtr) -> c_int {
...
pub unsafe extern "C" fn rs_qf_skip_to_valid(
    ...
    let mut current = (*qfp).qf_next;
    let mut current_idx = idx + 1;

    while !current.is_null() {
        if (*current).qf_valid != 0 {
            *out_idx = current_idx;
            return current;
        }
        current = (*current).qf_next;
        current_idx += 1;
    }
...
pub unsafe extern "C" fn rs_qf_skip_to_file(
    ...
    let current_fnum = (*qfp).qf_fnum;
    let mut current = (*qfp).qf_next;
    let mut current_idx = idx + 1;

    while !current.is_null() {
        if (*current).qf_fnum != current_fnum {
            *out_idx = current_idx;
            return current;
        }
        current = (*current).qf_next;
...
extern "C" {
    ...
    fn nvim_qf_set_title_var_for_list(qfl: QfListPtr);
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_last(qfl: QfListPtr) -> QfLinePtr {
    if qfl.is_null() {
        return std::ptr::null_mut();
    }
    (*qfl).qf_last
...
/// - `qfp` must be a valid pointer to a `qfline_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qfline_get_fname(qfp: QfLinePtr) -> *const c_char {
    if qfp.is_null() {
        return std::ptr::null_mut();
    }
    (*qfp).qf_fname
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_qfl_type(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return qfl_types::QFLT_QUICKFIX;
    }
    (*qfl).qfl_type as c_int
}
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_is_qf_list(qfl: QfListPtr) -> bool {
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_is_ll_list(qfl: QfListPtr) -> bool {
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_has_user_data(qfl: QfListPtr) -> bool {
    if qfl.is_null() {
        return false;
    }
    (*qfl).qf_has_user_data
}
...
pub unsafe extern "C" fn rs_qf_new_list(qi: QfInfoHandleMut, title: *const c_char) {
    ...
    (*qfl).qfl_type = ::std::mem::transmute::<i32, crate::ffi_types::QflTypeRaw>(qi_type);
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_free_list(qfl: QfListPtr) {
    if qfl.is_null() {
        return;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_free_items(qfl: QfListPtr) {
    if qfl.is_null() {
        return;
    }
    ...
    while count > 0 && !qfp.is_null() {
        ...
        // Free the entry: fields first, then the struct itself
        {
            ...
            (*qfp).qf_module = ::std::ptr::null_mut();
            xfree((*qfp).qf_fname.cast());
            (*qfp).qf_fname = ::std::ptr::null_mut();
            xfree((*qfp).qf_pattern.cast());
            (*qfp).qf_pattern = ::std::ptr::null_mut();
            xfree((*qfp).qf_text.cast());
            (*qfp).qf_text = ::std::ptr::null_mut();
            tv_clear((&raw mut (*qfp).qf_user_data).cast());
        };
        xfree(qfp.cast());
    ...
    // Reset list pointers
    (*qfl).qf_start = std::ptr::null_mut();
    (*qfl).qf_last = std::ptr::null_mut();
    (*qfl).qf_ptr = std::ptr::null_mut();
    (*qfl).qf_index = 0;
    (*qfl).qf_count = 0;
    (*qfl).qf_nonevalid = true;

    // Clean directory stacks using Rust implementation
    let dir_stack = (*qfl).qf_dir_stack;
    let mut stack = dir_stack;
    dirstack::clean_dir_stack_raw(&raw mut stack);
    (*qfl).qf_dir_stack = stack;
    (*qfl).qf_directory = std::ptr::null_mut();

    let file_stack = (*qfl).qf_file_stack;
    let mut stack = file_stack;
    dirstack::clean_dir_stack_raw(&raw mut stack);
    (*qfl).qf_file_stack = stack;
    (*qfl).qf_currfile = std::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_store_title(qfl: QfListPtr, title: *const c_char) {
    if qfl.is_null() {
        return;
    }
    {
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_start(qfl: QfListPtr, start: QfLinePtr) {
    if qfl.is_null() {
        return;
    }
    (*qfl).qf_start = start;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_last(qfl: QfListPtr, last: QfLinePtr) {
    if qfl.is_null() {
        return;
    }
    (*qfl).qf_last = last;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_count(qfl: QfListPtr, count: c_int) {
    if qfl.is_null() {
        return;
    }
    (*qfl).qf_count = count;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_increment_count(qfl: QfListPtr) {
    if qfl.is_null() {
        return;
    }
    (*qfl).qf_count += 1;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_nonevalid(qfl: QfListPtr, nonevalid: bool) {
    if qfl.is_null() {
        return;
    }
...
/// - `qfp` must be a valid pointer to a `qfline_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qfline_set_next(qfp: QfLinePtr, next: QfLinePtr) {
    if qfp.is_null() {
        return;
    }
    (*qfp).qf_next = next;
...
/// - `qfp` must be a valid pointer to a `qfline_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qfline_set_prev(qfp: QfLinePtr, prev: QfLinePtr) {
    if qfp.is_null() {
        return;
    }
    (*qfp).qf_prev = prev;
...
pub unsafe extern "C" fn rs_qf_add_entry(
    ...
    let qfp = xcalloc(1, ::std::mem::size_of::<crate::ffi_types::QfLineRaw>())
        .cast::<crate::ffi_types::QfLineRaw>();
    if qfp.is_null() {
        return QF_FAIL;
    }
    ...
    if !fname.is_null() {
        ...
        if !fixed_fname.is_null() {
            {
                xfree((*qfp).qf_fname.cast());
                (*qfp).qf_fname = if !fixed_fname.is_null() && *fixed_fname != 0 {
                    xstrdup(fixed_fname)
                } else {
                    ::std::ptr::null_mut()
                }
    ...
    {
        xfree((*qfp).qf_module.cast());
        (*qfp).qf_module = if !module.is_null() && *module != 0 {
            xstrdup(module)
        } else {
            ::std::ptr::null_mut()
        }
    ...
    {
        xfree((*qfp).qf_pattern.cast());
        (*qfp).qf_pattern = if !pattern.is_null() && *pattern != 0 {
            xstrdup(pattern)
        } else {
            ::std::ptr::null_mut()
        }
    ...
    if user_data.is_null() || nvim_tv_is_unknown(user_data) {
        (*qfp).qf_user_data.fill(0);
    } else {
        tv_copy(user_data, (&raw mut (*qfp).qf_user_data).cast());
        if !qfl.is_null() {
    ...
    if rs_qf_list_empty(qfl) {
        ...
        (*qfl).qf_ptr = qfp;
        (*qfl).qf_index = 0;
        (*qfp).qf_prev = std::ptr::null_mut();
    } else {
        // Append to the end
        (*qfp).qf_prev = last;
        (*last).qf_next = qfp;
    }
    (*qfp).qf_next = std::ptr::null_mut();
    (*qfl).qf_last = qfp;
    (*qfl).qf_count += 1;

    // If this is the first valid entry, update the current pointer
    let count = (*qfl).qf_count;
    let index = (*qfl).qf_index;
    if index == 0 && valid != 0 {
        (*qfl).qf_index = count;
        (*qfl).qf_ptr = qfp;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_dir_stack(qfl: QfListPtr) -> *mut c_void {
    if qfl.is_null() {
        return std::ptr::null_mut();
    }
    (*qfl).qf_dir_stack.cast()
}
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_dir_stack(qfl: QfListPtr, stack: *mut c_void) {
    if qfl.is_null() {
        return;
    }
    (*qfl).qf_dir_stack = stack.cast();
}
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_file_stack(qfl: QfListPtr) -> *mut c_void {
    if qfl.is_null() {
        return std::ptr::null_mut();
    }
    (*qfl).qf_file_stack.cast()
}
...
/// - `stack` must be a valid `dir_stack_T` pointer or null
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_file_stack(qfl: QfListPtr, stack: *mut c_void) {
    if qfl.is_null() {
        return;
    }
    (*qfl).qf_file_stack = stack.cast();
}
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_directory(qfl: QfListPtr) -> *const c_char {
    if qfl.is_null() {
        return std::ptr::null_mut();
    }
    (*qfl).qf_directory
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_directory(qfl: QfListPtr, dir: *mut c_char) {
    if qfl.is_null() {
        return;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_currfile(qfl: QfListPtr) -> *const c_char {
    if qfl.is_null() {
        return std::ptr::null_mut();
    }
    (*qfl).qf_currfile
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_currfile(qfl: QfListPtr, file: *mut c_char) {
    if qfl.is_null() {
        return;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_pop_dir(qfl: QfListPtr, is_file_stack: bool) -> *const c_char {
    if qfl.is_null() {
        return std::ptr::null_mut();
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_clean_dir_stack(qfl: QfListPtr, is_file_stack: bool) {
    if qfl.is_null() {
        return;
    }
...
pub unsafe extern "C" fn rs_qf_guess_filepath(
    ...
    let stack_ptr = (*qfl).qf_dir_stack;
...
pub unsafe extern "C" fn rs_qf_get_fnum(
    ...
    let is_qf = (*qfl.cast_const()).qfl_type == crate::ffi_types::QflTypeRaw::Quickfix;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_multiline(qfl: QfListPtr) -> bool {
    if qfl.is_null() {
        return false;
    }
    (*qfl).qf_multiline
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_multiline(qfl: QfListPtr, multiline: bool) {
    if qfl.is_null() {
        return;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_multiignore(qfl: QfListPtr) -> bool {
    if qfl.is_null() {
        return false;
    }
    (*qfl).qf_multiignore
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_multiignore(qfl: QfListPtr, multiignore: bool) {
    if qfl.is_null() {
        return;
    }
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_title_var(qfl: QfListPtr) {
    if qfl.is_null() {
        return;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_title(qfl: QfListPtr) -> *const c_char {
    if qfl.is_null() {
        return std::ptr::null_mut();
    }
    (*qfl).qf_title
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_get_ctx(qfl: QfListPtr) -> *mut c_void {
    if qfl.is_null() {
        return std::ptr::null_mut();
    }
    (*qfl).qf_ctx
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_incr_changedtick(qfl: QfListPtr) {
    if qfl.is_null() {
        return;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_index(qfl: QfListPtr, idx: c_int) {
    if qfl.is_null() {
        return;
    }
    (*qfl).qf_index = idx;
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_ptr(qfl: QfListPtr, ptr: QfLinePtr) {
    if qfl.is_null() {
        return;
    }
    (*qfl).qf_ptr = ptr;
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_set_cur_entry(qfl: QfListPtr, idx: c_int) {
    if qfl.is_null() || idx <= 0 {
        return;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_next_valid_entry(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
    ...
    let count = (*qfl).qf_count;
    let mut qfp = (*current_ptr).qf_next;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_prev_valid_entry(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
    ...
    let current_idx = (*qfl).qf_index;
    let mut qfp = (*current_ptr).qf_prev;
    let mut idx = current_idx - 1;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_first_valid_entry(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_last_valid_entry(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }

    let count = (*qfl).qf_count;
    let mut qfp = (*qfl).qf_last;
    let mut idx = count;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_goto_nth_valid(qfl: QfListPtr, count: c_int) -> c_int {
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_goto_entry(qfl: QfListPtr, target_idx: c_int) -> c_int {
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_next_entry_in_file(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
    ...
    let current_fnum = (*current_ptr).qf_fnum;
    let current_idx = (*qfl).qf_index;
    let count = (*qfl).qf_count;

    let qfp = (*current_ptr).qf_next;
    let idx = current_idx + 1;

    // Check if next entry exists and is in the same file
    if !qfp.is_null() && idx <= count && (*qfp).qf_fnum == current_fnum {
        (*qfl).qf_index = idx;
        (*qfl).qf_ptr = qfp;
        return idx;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_prev_entry_in_file(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
    ...
    let qfp = (*current_ptr).qf_prev;
    let idx = current_idx - 1;

    // Check if previous entry exists and is in the same file
    if !qfp.is_null() && idx >= 1 && (*qfp).qf_fnum == current_fnum {
        (*qfl).qf_index = idx;
        (*qfl).qf_ptr = qfp;
        return idx;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_next_file(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_prev_file(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
    ...
    while !qfp.is_null() && idx >= 1 {
        if (*qfp).qf_fnum != current_fnum {
            ...
            let new_fnum = (*qfp).qf_fnum;

            // Keep going back to find the first entry of this file
            let mut first_qfp = qfp;
            let mut first_idx = idx;

            let mut prev = (*qfp).qf_prev;
            while !prev.is_null() && (first_idx - 1) >= 1 {
                if (*prev).qf_fnum != new_fnum {
                    break;
                }
                first_qfp = prev;
                first_idx -= 1;
                prev = (*first_qfp).qf_prev;
            }

            (*qfl).qf_index = first_idx;
            (*qfl).qf_ptr = first_qfp;
            return first_idx;
        }
        qfp = (*qfp).qf_prev;
        idx -= 1;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_count_entries_in_file(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_count_valid_in_file(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
    ...
    while !qfp.is_null() && idx <= count {
        if (*qfp).qf_fnum == current_fnum && ((*qfp).qf_valid != 0) {
            valid_count += 1;
        }
        qfp = (*qfp).qf_next;
        idx += 1;
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_file_count(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct
#[no_mangle]
pub unsafe extern "C" fn rs_qf_current_file_idx(qfl: QfListPtr) -> c_int {
    if qfl.is_null() {
        return 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn rs_qf_list_summary(qfl: QfListPtr) -> *mut c_char {
    if qfl.is_null() {
...
/// - `qfl` must be a valid pointer to a `qf_list_T` struct or null
#[no_mangle]
pub unsafe extern "C" fn rs_qf_should_abort(qfl: QfListPtr) -> bool {
...
extern "C" {
    ...
    fn tv_free(tv: *mut ::std::ffi::c_void);
}
...
pub unsafe extern "C" fn rs_qf_update_buffer(qi: QfInfoHandleMut, old_last: QfLinePtr) {
    if qi.is_null() {
        return;
    }
...
mod phase79_tests {
    ...
    fn test_parse_line_col() {
        unsafe {
            ...
            assert_eq!((r.line, r.col), (0, 0));
            let r = rs_qf_parse_line_col(std::ptr::null_mut());
            assert_eq!((r.line, r.col), (0, 0));
        }
    }
...
mod tests {
    ...
    fn test_null_stack_is_empty() {
        unsafe {
            assert!(rs_qf_stack_empty(std::ptr::null_mut()));
        }
    }
    ...
    fn test_null_list_is_empty() {
        unsafe {
            assert!(rs_qf_list_empty(std::ptr::null_mut()));
        }
    }
    ...
    fn test_null_list_no_valid_entries() {
        ...
        unsafe {
            assert!(!rs_qf_list_has_valid_entries(std::ptr::null_mut()));
        }
    }
    ...
    fn test_null_index_validation() {
        unsafe {
            ...
            assert!(!rs_qf_valid_idx(std::ptr::null_mut(), 0));
            assert!(!rs_qf_valid_idx(std::ptr::null_mut(), 1));
        }
    }
    ...
    fn test_null_get_size() {
        unsafe {
            ...
            assert_eq!(rs_qf_get_size(std::ptr::null_mut()), 0);
        }
    }
    ...
    fn test_null_find_first_entry_in_buf() {
        unsafe {
            let mut idx = 99;
            let result = rs_qf_find_first_entry_in_buf(std::ptr::null_mut(), 1, &raw mut idx);
    ...
    fn test_null_entry_count_in_file() {
        unsafe {
            assert_eq!(rs_qf_entry_count_in_file(std::ptr::null_mut(), 1), 0);
        }
    }
    ...
    fn test_null_count_valid_entries() {
        unsafe {
            assert_eq!(rs_qf_count_valid_entries(std::ptr::null_mut()), 0);
        }
    }
    ...
    fn test_null_get_nth_valid_entry() {
        unsafe {
            let mut idx = 99;
            let result = rs_qf_get_nth_valid_entry(std::ptr::null_mut(), 1, &raw mut idx);
            assert!(result.is_null());
        }
    }
    ...
    fn test_null_get_entry_at_idx() {
        unsafe {
            assert!(rs_qf_get_entry_at_idx(std::ptr::null_mut(), 1).is_null());
        }
    }
    ...
    fn test_null_cmp_entries() {
        unsafe {
            assert_eq!(
                rs_qf_cmp_entries(std::ptr::null_mut(), std::ptr::null_mut()),
                0
            );
        }
    }
    ...
    fn test_null_entry_in_file() {
        unsafe {
            assert!(!rs_qf_entry_in_file(std::ptr::null_mut(), 1));
        }
    }
    ...
    fn test_null_entry_is_active() {
        unsafe {
            assert!(!rs_qf_entry_is_active(std::ptr::null_mut()));
        }
    }
    ...
    fn test_null_skip_to_valid() {
        unsafe {
            let mut idx = 99;
            let result = rs_qf_skip_to_valid(std::ptr::null_mut(), 1, &raw mut idx);
            assert!(result.is_null());
        }
    }
    ...
    fn test_null_skip_to_file() {
        unsafe {
            let mut idx = 99;
            let result = rs_qf_skip_to_file(std::ptr::null_mut(), 1, &raw mut idx);
            assert!(result.is_null());
        }
    }
    ...
    fn test_null_next_valid_in_file() {
        unsafe {
            let mut idx = 99;
            let result = rs_qf_next_valid_in_file(std::ptr::null_mut(), 1, &raw mut idx);
            assert!(result.is_null());
        }
    }
    ...
    fn test_null_prev_valid_in_file() {
        unsafe {
            let mut idx = 99;
            let result = rs_qf_prev_valid_in_file(std::ptr::null_mut(), 1, &raw mut idx);
            assert!(result.is_null());
        }
    }
    ...
    fn test_null_get_id() {
        unsafe {
            assert_eq!(rs_qf_get_id(std::ptr::null_mut()), 0);
        }
    }
    ...
    fn test_null_get_changedtick() {
        unsafe {
            assert_eq!(rs_qf_get_changedtick(std::ptr::null_mut()), 0);
        }
    }
    ...
    fn test_null_has_title() {
        unsafe {
            assert!(!rs_qf_has_title(std::ptr::null_mut()));
        }
    }
    ...
    fn test_null_get_maxcount() {
        unsafe {
            assert_eq!(rs_qf_get_maxcount(std::ptr::null_mut()), 0);
        }
    }
    ...
    fn test_null_stack_at_max() {
        unsafe {
            assert!(!rs_qf_stack_at_max(std::ptr::null_mut()));
        }
    }
    ...
    fn test_null_find_list_by_id() {
        unsafe {
            assert_eq!(rs_qf_find_list_by_id(std::ptr::null_mut(), 1), -1);
        }
    }
    ...
    fn test_find_list_by_id_zero() {
        unsafe {
            ...
            assert_eq!(rs_qf_find_list_by_id(std::ptr::null_mut(), 0), -1);
        }
    }
    ...
    fn test_null_id_exists() {
        unsafe {
            assert!(!rs_qf_id_exists(std::ptr::null_mut(), 1));
        }
    }
    ...
    fn test_null_entry_in_range() {
        unsafe {
            assert!(!rs_qf_entry_in_range(std::ptr::null_mut(), 1, 10));
        }
    }
    ...
    fn test_null_entry_covers_line() {
        unsafe {
            assert!(!rs_qf_entry_covers_line(std::ptr::null_mut(), 5));
        }
    }
    ...
    fn test_null_entry_covers_pos() {
        unsafe {
            assert!(!rs_qf_entry_covers_pos(std::ptr::null_mut(), 5, 10));
        }
    }
    ...
    fn test_null_count_entries_of_type() {
        unsafe {
            assert_eq!(
                rs_qf_count_entries_of_type(std::ptr::null_mut(), error_types::QF_TYPE_ERROR),
                0
            );
        }
    }
    ...
    fn test_null_count_errors() {
        unsafe {
            assert_eq!(rs_qf_count_errors(std::ptr::null_mut()), 0);
        }
    }
    ...
    fn test_null_count_warnings() {
        unsafe {
            assert_eq!(rs_qf_count_warnings(std::ptr::null_mut()), 0);
        }
    }
}
