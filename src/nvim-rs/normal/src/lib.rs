...
extern "C" {
    ...
    // VIsual_* globals (from globals.h)
    ...
    static mut resel_VIsual_mode: c_int;
    static mut resel_VIsual_line_count: c_int;
    static mut resel_VIsual_vcol: c_int;

    // Other globals (from globals.h / option_vars.h)
    ...
    fn nvim_check_cursor(wp: WinHandle);
    fn setmouse();
    ...
    fn ml_get_buf(buf: BufHandle, lnum: i32) -> *mut c_char;
...
/// Check if the 'selection' option is "exclusive" (p_sel[0] == 'e').
///
/// # Safety
/// `p_sel` must be a valid non-null pointer to a C string.
...
pub type CapHandle = *mut crate::types::CmdargT;
...
pub unsafe extern "C" fn rs_clearop(oap: OapHandle) {
    (*oap).op_type = OP_NOP;
    (*oap).regname = 0;
    (*oap).motion_force = NUL_CHAR;
    (*oap).use_reg_one = false;
    motion_force = NUL_CHAR;
}
...
pub unsafe extern "C" fn rs_nv_ctrlg(cap: CapHandle) {
    if VIsual_active {
        ...
        let select = VIsual_select;
        VIsual_select = !select;
...
pub unsafe extern "C" fn rs_nv_goto(cap: CapHandle) {
    ...
    (*oap).motion_type = K_MT_LINE_WISE;
    setpcmark();
    ...
    if (fdo_flags & K_OPT_FDO_FLAG_JUMP) != 0 && KeyTyped && (*oap).op_type == OP_NOP {
        rs_foldOpenCursor();
    }
}
...
pub unsafe extern "C" fn rs_nv_beginline(cap: CapHandle) {
    ...
    (*oap).motion_type = K_MT_CHAR_WISE;
    (*oap).inclusive = false;
    beginline(arg);

    if (fdo_flags & K_OPT_FDO_FLAG_HOR) != 0 && KeyTyped && (*oap).op_type == OP_NOP {
        rs_foldOpenCursor();
    }
    ins_at_eol = false;
}
...
pub unsafe extern "C" fn rs_nv_dollar(cap: CapHandle) {
    ...
    if !cursor_down(count1 - 1, (*oap).op_type == OP_NOP) {
        rs_clearopbeep(oap);
    } else if (fdo_flags & K_OPT_FDO_FLAG_HOR) != 0 && KeyTyped && (*oap).op_type == OP_NOP {
        rs_foldOpenCursor();
    }
}
...
pub unsafe extern "C" fn rs_nv_bck_word(cap: CapHandle) {
    ...
    if bck_word(count1, arg != 0, false) == FAIL {
        rs_clearopbeep(oap);
    } else if (fdo_flags & K_OPT_FDO_FLAG_HOR) != 0 && KeyTyped && (*oap).op_type == OP_NOP {
        rs_foldOpenCursor();
    }
...
pub unsafe extern "C" fn rs_nv_wordcmd(cap: CapHandle) {
    ...
    (*oap).inclusive = word_end;

    let mut flag = false;

    // "cw" and "cW" are a special case.
    if !word_end && (*oap).op_type == OP_CHANGE {
    ...
    let n = if word_end {
        ...
        fwd_word(count1, arg != 0, (*oap).op_type != OP_NOP)
    };
...
pub unsafe extern "C" fn rs_nv_findpar(cap: CapHandle) {
    ...
    if (fdo_flags & K_OPT_FDO_FLAG_BLOCK) != 0 && KeyTyped && (*oap).op_type == OP_NOP {
        rs_foldOpenCursor();
    }
}
...
pub unsafe extern "C" fn rs_nv_gomark(cap: CapHandle) {
    ...
    let mut flags: c_int = if (*oap).op_type != OP_NOP {
        0
    ...
    let old_key_typed = KeyTyped;

    let name: c_int;
    if cmdchar == c_int::from(b'g') {
        let extra_char = (*cap).extra_char;
        name = extra_char;
    ...
    if (*oap).op_type == OP_NOP
        ...
        && (fdo_flags & K_OPT_FDO_FLAG_MARK) != 0
        && old_key_typed
    {
        rs_foldOpenCursor();
    }
...
pub unsafe extern "C" fn rs_nv_visual(cap: CapHandle) {
    ...
    if (*oap).op_type != OP_NOP {
        (*oap).motion_force = cmdchar;
        motion_force = cmdchar;
    ...
    if VIsual_active {
        ...
        // start Visual mode
        let count0 = (*cap).count0;
        let resel_mode = resel_VIsual_mode;
        if count0 > 0 && resel_mode != 0 {
            // use previously selected part
            // VIsual = curwin->w_cursor
            VIsual = crate::types::PosT {
            ...
            let resel_line_count = resel_VIsual_line_count;
            let resel_vcol = resel_VIsual_vcol;
            ...
            if resel_mode != c_int::from(b'v') || resel_line_count > 1 {
                ...
                nvim_check_cursor(nvim_get_curwin());
            }
            VIsual_mode = resel_mode;
            ...
            if resel_vcol == MAXCOL {
                ...
                nvim_set_cursor_lnum(VIsual.lnum);
                update_curswant_force();
            ...
            if VIsual_mode != c_int::from(b'V') && sel_is_exclusive() {
...
pub unsafe extern "C" fn rs_nv_window(cap: CapHandle) {
    ...
    if nchar == c_int::from(b':') {
        ...
        (*cap).cmdchar = c_int::from(b':');
        (*cap).nchar = c_int::from(NUL);
        rs_nv_colon(cap);
...
pub unsafe extern "C" fn rs_nv_regname(cap: CapHandle) {
    ...
    if nchar == eq_char {
        nchar = get_expr_register();
    }
    if nchar != NUL_CHAR && valid_yank_reg(nchar, false) {
        (*oap).regname = nchar;
        let count0 = (*cap).count0;
        (*cap).opcount = count0; // remember count before '"'
        set_reg_var(nchar);
    } else {
        rs_clearopbeep(oap);
    }
...
unsafe fn nv_put_opt_impl(cap: CapHandle, fix_indent: bool) {
    ...
    if op_type != OP_NOP {
        let cmdchar = (*cap).cmdchar;
        let cap_opcount = (*cap).opcount;
        if op_type == OP_DELETE && cmdchar == b'p' as c_int {
            rs_clearop(oap);
            assert!(cap_opcount >= 0);
            nv_diffgetput(true, cap_opcount.unsigned_abs() as usize);
        } else {
            rs_clearopbeep(oap);
        }
        return;
    ...
    let save_fen = win_ref(nvim_get_curwin()).w_p_fen() != 0;
    ...
    if VIsual_active {
        ...
        let keep_registers = (*cap).cmdchar == b'P' as c_int;
        ...
        win_mut(nvim_get_curwin()).set_w_p_fen(0);
        if !VIsual_active || VIsual_mode == c_int::from(b'V') || regname != c_int::from(b'.') {
            (*cap).cmdchar = b'd' as c_int;
            (*cap).nchar = NUL_CHAR;
        ...
        dir = BACKWARD;
        if (vis_mode != b'V' as c_int
            && (win_ref(nvim_get_curwin()).w_cursor.col
                - buf_ref(nvim_get_curbuf()).b_op_start.col)
                < 0)
            || (vis_mode == b'V' as c_int
                && ((win_ref(nvim_get_curwin()).w_cursor.lnum
                    - buf_ref(nvim_get_curbuf()).b_op_start.lnum) as c_int)
                    < 0)
        {
            dir = FORWARD;
        }
    ...
    if was_visual {
        ...
        if save_fen {
            win_mut(nvim_get_curwin()).set_w_p_fen(1);
        }
        {
    ...
    if empty && *ml_get(nvim_get_line_count()).cast::<u8>() == 0 {
        ...
        if nvim_get_cursor_lnum()
            > buf_ref(win_ref(nvim_get_curwin()).w_buffer as BufHandle).ml_line_count() as c_int
        {
            nvim_set_cursor_lnum(
                buf_ref(win_ref(nvim_get_curwin()).w_buffer as BufHandle).ml_line_count() as c_int,
            );
            coladvance(nvim_get_curwin(), MAXCOL);
        }
...
pub unsafe extern "C" fn rs_nv_hat(cap: CapHandle) {
    ...
    if !rs_checkclearopq(oap) {
        ...
        buflist_getfile(count0, 0, flags, 0);
    }
}
...
unsafe fn invoke_edit_impl(cap: CapHandle, repl: bool, cmd: c_int, startln: bool) {
    ...
    let restart_edit_save = if repl || !stuff_empty() {
        restart_edit
    } else {
        0
    };
...
pub unsafe extern "C" fn rs_nv_esc(cap: CapHandle) {
    let oap = (*cap).oap;
    let no_reason = (*oap).op_type == OP_NOP
        && (*cap).opcount == 0
        && (*cap).count0 == 0
        && (*oap).regname == 0;
...
pub unsafe extern "C" fn rs_nv_search(cap: CapHandle) {
    ...
    if cmdchar == c_int::from(b'?') && (*oap).op_type == OP_ROT13 {
        // Translate "g??" to "g?g?"
        (*cap).cmdchar = c_int::from(b'g');
        (*cap).nchar = c_int::from(b'?');
        rs_nv_operator(cap);
        return;
    }
    ...
    unsafe {
        ...
        match cmdchar_byte {
            b'*' | b'#' => {
                ...
                let line_start = nvim_get_cursor_line_ptr();
        ...
        if cmdchar_byte == b'*' || cmdchar_byte == b'#' {
            ...
            if g_cmd == 0 && vim_iswordp(mb_prevptr(line_ptr, ptr)) {
                buf.extend_from_slice(b"\\>");
            }
            init_history();
...
unsafe fn nv_operator_impl(cap: CapHandle) {
    ...
    let cmdchar = (*cap).cmdchar;
    let nchar = (*cap).nchar;
    let op_type = get_op_type(cmdchar, nchar);
...
pub unsafe extern "C" fn rs_nv_tilde(cap: CapHandle) {
    let oap = (*cap).oap;
    if !p_to && !VIsual_active && (*oap).op_type != OP_TILDE {
        if nvim_bt_prompt_curbuf() && !prompt_curpos_editable() {
            rs_clearopbeep(oap);
            return;
        }
...
pub unsafe extern "C" fn rs_n_swapchar(cap: CapHandle) {
    ...
    if did_change {
        ...
        {
            let c = win_ref(nvim_get_curwin()).w_cursor;
            buf_mut(nvim_get_curbuf()).b_op_end = std::mem::transmute_copy(&c);
        }
        {
            let b = buf_mut(nvim_get_curbuf());
            if b.b_op_end.col > 0 {
...
pub unsafe extern "C" fn rs_nv_object(cap: CapHandle) {
    ...
    let flag = match nchar {
        ...
        n if n == c_int::from(b't') => {
            ...
            (*cap).retval |= CA_NO_ADJ_OP_END_P2;
...
pub unsafe extern "C" fn rs_nv_select(cap: CapHandle) {
    if VIsual_active {
        ...
        (*cap).nchar = c_int::from(b'v'); // fake "gv" command
        (*cap).arg = 1;
        rs_nv_g_cmd(cap);
    }
}
...
pub unsafe extern "C" fn rs_nv_brackets(cap: CapHandle) {
        ...
        || (cmdchar == b']' as c_int && !vim_strchr(c"})*/#mM".as_ptr(), nchar).is_null())
    {
        ...
        if found {
            ...
            if (fdo_flags & K_OPT_FDO_FLAG_BLOCK) != 0 && KeyTyped && op_type == OP_NOP {
                rs_foldOpenCursor();
            }
        } else {
            rs_clearopbeep(oap);
        ...
        do_mouse(oap, nchar, dir, (*cap).count1, false);
        ...
        if rs_foldMoveTo(false, dir, (*cap).count1) == 0 {
            rs_clearopbeep(oap);
        }
        ...
        if rs_diff_move_to(dir, (*cap).count1) == 0 {
            rs_clearopbeep(oap);
        }
        ...
        if op_type == OP_NOP && (fdo_flags & K_OPT_FDO_FLAG_SEARCH) != 0 && KeyTyped {
            rs_foldOpenCursor();
        }
    } else {
...
unsafe fn find_decl_search(
    ...
    loop {
        ...
        let valid = rs_is_ident(nvim_get_cursor_line_ptr(), cur_col);
...
pub unsafe extern "C" fn rs_nv_undo(cap: CapHandle) {
    let oap = (*cap).oap;
    if (*oap).op_type == OP_LOWER || VIsual_active {
        // translate "<Visual>u" to "<Visual>gu" and "guu" to "gugu"
        (*cap).cmdchar = c_int::from(b'g');
        (*cap).nchar = c_int::from(b'u');
        rs_nv_operator(cap);
    } else {
        rs_nv_kundo(cap);
    }
...
pub unsafe extern "C" fn rs_nv_Undo(cap: CapHandle) {
    let oap = (*cap).oap;
    if (*oap).op_type == OP_UPPER || VIsual_active {
...
pub unsafe extern "C" fn rs_nv_redo_or_register(cap: CapHandle) {
    if VIsual_select && VIsual_active {
        ...
        let valid = valid_yank_reg(reg, true);
...
pub unsafe extern "C" fn rs_nv_replace(cap: CapHandle) {
    ...
    let had_ctrl_v = {
        ...
        if nch == CTRL_V || nch == CTRL_Q_P3 {
            let new_nchar = get_literal(false);
            (*cap).nchar = new_nchar;
    ...
    if had_ctrl_v != CTRL_V && (nchar == c_int::from(b'\r') || nchar == c_int::from(b'\n')) {
        ...
        {
            let c = win_ref(nvim_get_curwin()).w_cursor;
            buf_mut(nvim_get_curbuf()).b_op_start = std::mem::transmute_copy(&c);
        }
        let old_state = State;

        let cap_typed = cap.cast_const();
...
pub unsafe extern "C" fn rs_nv_vreplace(cap: CapHandle) {
    if VIsual_active {
        ...
        let extra_char = (*cap).extra_char;
        (*cap).cmdchar = c_int::from(b'r');
        (*cap).nchar = extra_char;
        rs_nv_replace(cap);
        return;
    }
    ...
    if nvim_curbuf_modifiable() {
        let mut extra_char = (*cap).extra_char;
        if extra_char == CTRL_V || extra_char == CTRL_Q_P3 {
            // get another character
            extra_char = get_literal(false);
        }
...
unsafe fn nv_z_get_count_impl(cap: CapHandle, nchar_arg: &mut c_int) -> bool {
    ...
    loop {
        no_mapping += 1;
        allow_keys += 1; // no mapping for nchar, but allow key codes
        ...
        {
            let count1 = (*cap).count1;
            (*cap).count1 = if n != 0 { n * count1 } else { count1 };
            *nchar_arg = nchar;
            return true;
        } else {
            rs_clearopbeep(oap);
...
unsafe fn nv_zet_impl(cap: CapHandle) {
    ...
    let old_fen = win_ref(nvim_get_curwin()).w_p_fen() != 0;
    ...
    if !vim_strchr(c"+\r\nt.z^-b".as_ptr(), nchar).is_null()
        && (*cap).count0 != 0
        && (*cap).count0 != nvim_get_cursor_lnum()
    {
        setpcmark();
        let line_count = nvim_get_line_count();
        if (*cap).count0 > line_count {
            nvim_set_cursor_lnum(line_count);
        } else {
            nvim_set_cursor_lnum((*cap).count0);
        }
    ...
    match nchar {
        ...
        n if n == b'+' as c_int => {
            if (*cap).count0 == 0 {
                ...
                let botline = win_ref(nvim_get_curwin()).w_botline as c_int;
                let line_count = nvim_get_line_count();
        ...
        n if n == b'H' as c_int => {
            let half = win_ref(nvim_get_curwin()).w_view_width / 2;
            (*cap).count1 *= half;
            // FALLTHROUGH to 'h'
            if win_ref(nvim_get_curwin()).w_p_wrap() == 0 {
                let leftcol = win_ref(nvim_get_curwin()).w_leftcol as c_int;
                let count1 = (*cap).count1;
                if count1 > leftcol {
                    set_leftcol(0);
                } else {
        ...
        n if n == b'L' as c_int => {
            ...
            // FALLTHROUGH to 'l'
            if win_ref(nvim_get_curwin()).w_p_wrap() == 0 {
                set_leftcol((win_ref(nvim_get_curwin()).w_leftcol as c_int) + (*cap).count1);
            }
        }
        ...
        n if n == b's' as c_int => {
            if win_ref(nvim_get_curwin()).w_p_wrap() == 0 {
                ...
                if (win_ref(nvim_get_curwin()).w_leftcol as c_int) != col {
                    win_mut(nvim_get_curwin()).w_leftcol = col;
        ...
        n if n == b'e' as c_int => {
            if win_ref(nvim_get_curwin()).w_p_wrap() == 0 {
                ...
                let n_val =
                    win_ref(nvim_get_curwin()).w_view_width - win_col_off(nvim_get_curwin());
                if col + siso < n_val {
                    col = 0;
                } else {
        ...
        n if n == b'F' as c_int || n == b'f' as c_int => {
            if rs_foldManualAllowed(true) != 0 {
                (*cap).nchar = b'f' as c_int;
                rs_nv_operator(cap);
                win_mut(nvim_get_curwin()).set_w_p_fen(1);

                // "zF" is like "zfzf"
                if nchar == b'F' as c_int && (*oap).op_type == OP_FOLD {
                    rs_nv_operator(cap);
                    finish_op = true;
                }
            } else {
        ...
        n if n == b'i' as c_int => {
            win_mut(nvim_get_curwin())
                .set_w_p_fen(i32::from(win_ref(nvim_get_curwin()).w_p_fen() == 0));
        }
        ...
        n if n == b'a' as c_int => {
            ...
            if nvim_hasFolding_curwin(cursor_lnum) {
                rs_setFoldRepeat(cursor_lnum, (*cap).count1, true);
            } else {
                rs_setFoldRepeat(cursor_lnum, (*cap).count1, false);
                win_mut(nvim_get_curwin()).set_w_p_fen(1);
            }
        }
        ...
        n if n == b'o' as c_int => {
            if VIsual_active {
                ...
                rs_setFoldRepeat(nvim_get_cursor_lnum(), (*cap).count1, true);
            }
        }
        ...
        n if n == b'c' as c_int => {
            if VIsual_active {
                ...
                rs_setFoldRepeat(nvim_get_cursor_lnum(), (*cap).count1, false);
            }
            win_mut(nvim_get_curwin()).set_w_p_fen(1);
        }
        ...
        n if n == b'x' as c_int => {
            win_mut(nvim_get_curwin()).set_w_p_fen(1);
            win_mut(nvim_get_curwin()).w_foldinvalid = true;
            rs_newFoldLevel();
            rs_foldOpenCursor();
        }
        ...
        n if n == b'j' as c_int || n == b'k' as c_int => {
            ...
            if rs_foldMoveTo(true, dir, (*cap).count1) == 0 {
                rs_clearopbeep(oap);
            }
        }
    ...
    if old_fen != (win_ref(nvim_get_curwin()).w_p_fen() != 0) {
...
pub unsafe extern "C" fn rs_nv_right(cap: CapHandle) {
    ...
    if n != count1 && (fdo_flags & K_OPT_FDO_FLAG_HOR) != 0 && KeyTyped && (*oap).op_type == OP_NOP
    {
        rs_foldOpenCursor();
    }
}
    ...
    loop {
        ...
        if oneleft() == 0 {
            ...
            if wrap {
                ...
                if ((*oap).op_type == OP_DELETE || (*oap).op_type == OP_CHANGE)
                    && !*ml_get(nvim_get_cursor_lnum()) == 0
                {
                    ...
                    (*cap).retval |= CA_NO_ADJ_OP_END_P1;
                }
                n -= 1;
                continue;
            } else if (*oap).op_type == OP_NOP && n == count1 {
                // Only beep if not moved at all
                beep_flush();
            }
            break;
...
pub unsafe extern "C" fn rs_nv_up(cap: CapHandle) {
    ...
    (*oap).motion_type = K_MT_LINEWISE;
    let count1 = (*cap).count1;
    if cursor_up(count1, (*oap).op_type == OP_NOP) == 0 {
        rs_clearopbeep(oap);
    } else if (*cap).arg != 0 {
        beginline(BL_WHITE | BL_FIX);
    }
...
pub unsafe extern "C" fn rs_nv_down(cap: CapHandle) {
    ...
    if (mod_mask & MOD_MASK_SHIFT_P1) != 0 {
            ...
            && win_ref(nvim_get_curwin()).w_cursor.lnum == nvim_get_line_count()
        {
            ...
            (*oap).motion_type = K_MT_LINEWISE;
            let count1 = (*cap).count1;
            if !cursor_down(count1, (*oap).op_type == OP_NOP) {
                rs_clearopbeep(oap);
            } else if (*cap).arg != 0 {
                beginline(BL_WHITE | BL_FIX);
            }
...
pub unsafe extern "C" fn rs_nv_screengo(
    ...
    (*oap).motion_type = K_MT_CHARWISE;
    (*oap).inclusive = nvim_get_curswant() == MAXCOL;

    let col_off1 = win_col_off(nvim_get_curwin());
    let col_off2 = col_off1 - win_col_off2(nvim_get_curwin());
    let width1 = win_ref(nvim_get_curwin()).w_view_width - col_off1;
    let mut width2 = win_ref(nvim_get_curwin()).w_view_width - col_off2;

    if width2 == 0 {
        width2 = 1; // Avoid divide by zero.
    }

    if win_ref(nvim_get_curwin()).w_view_width != 0 {
        ...
        if nvim_get_curswant() == MAXCOL {
            ...
            if width1 <= 0 {
                ...
                if (win_ref(nvim_get_curwin()).w_virtcol as c_int) > nvim_get_curswant() {
                    let extra = (((win_ref(nvim_get_curwin()).w_virtcol as c_int)
                        - nvim_get_curswant()
                        - 1)
                        / width2
                        + 1)
        ...
        while remaining > 0 {
            ...
            if dir == BACKWARD {
                    ...
                    && !nvim_hasFolding_curwin(nvim_get_cursor_lnum())
                {
                    ...
                    // to next line
                    if nvim_get_cursor_lnum()
                        >= buf_ref(win_ref(nvim_get_curwin()).w_buffer as BufHandle).ml_line_count()
                    {
                        retval = false;
                        break;
                    }
    ...
    if nvim_get_cursor_col() > 0 && (win_ref(nvim_get_curwin()).w_p_wrap() != 0) {
        // Check for landing on a character that got split at the end of the
        // last line. We want to advance a screenline, not end up in the same
        // screenline or move two screenlines.
        nvim_validate_virtcol_curwin();
        let mut virtcol = win_ref(nvim_get_curwin()).w_virtcol as c_int;
...
pub unsafe extern "C" fn rs_nv_g_home_m_cmd(cap: CapHandle) {
    ...
    let mut i: c_int;
    if (win_ref(nvim_get_curwin()).w_p_wrap() != 0) && win_ref(nvim_get_curwin()).w_view_width != 0
    {
        let col_off1 = win_col_off(nvim_get_curwin());
        let col_off2 = col_off1 - win_col_off2(nvim_get_curwin());
        let width1 = win_ref(nvim_get_curwin()).w_view_width - col_off1;
        ...
        nvim_validate_virtcol_curwin();
        let virtcol = win_ref(nvim_get_curwin()).w_virtcol as c_int;
        i = 0;
        ...
        let skipcol = win_ref(nvim_get_curwin()).w_skipcol as c_int;
        if skipcol > 0 && nvim_get_cursor_lnum() == (win_ref(nvim_get_curwin()).w_topline as c_int)
        {
            let overlap = sms_marker_overlap(
                nvim_get_curwin(),
                win_ref(nvim_get_curwin()).w_view_width - col_off2,
            );
        ...
        i = win_ref(nvim_get_curwin()).w_leftcol as c_int;
    }
    ...
    if nchar == c_int::from(b'm') {
        ...
        let col_off2 = col_off1 - win_col_off2(nvim_get_curwin());
        i += i32::midpoint(
            win_ref(nvim_get_curwin()).w_view_width - col_off1,
            if (win_ref(nvim_get_curwin()).w_p_wrap() != 0) && i > 0 {
                col_off2
            } else {
                0
            },
    ...
    if flag {
        ...
        win_mut(nvim_get_curwin()).w_valid &= !0x02; // VALID_WCOL
    }
    ...
    if rs_hasAnyFolding(nvim_get_curwin()) != 0 {
        ...
        if win_ref(nvim_get_curwin()).w_cline_folded {
            update_curswant_force();
        }
    }
    adjust_skipcol();
...
pub unsafe extern "C" fn rs_nv_g_dollar_cmd(cap: CapHandle) {
    ...
    if (win_ref(nvim_get_curwin()).w_p_wrap() != 0) && win_ref(nvim_get_curwin()).w_view_width != 0
    {
        ...
        if count1 == 1 {
            ...
            let width1 = win_ref(nvim_get_curwin()).w_view_width - col_off;
            let width2 = width1 + col_off2;
            ...
            if nvim_get_cursor_col() > 0 && (win_ref(nvim_get_curwin()).w_p_wrap() != 0) {
                ...
                // the end of the line.
                nvim_validate_virtcol_curwin();
                if (win_ref(nvim_get_curwin()).w_virtcol as c_int) > i {
        ...
        let i = (win_ref(nvim_get_curwin()).w_leftcol as c_int)
            + win_ref(nvim_get_curwin()).w_view_width
            - col_off
            - 1;
        coladvance(nvim_get_curwin(), i);
        ...
        if nvim_get_cursor_col() > 0 && utf_ptr2cells(get_cursor_pos_ptr()) > 1 {
            ...
            if vcol
                >= (win_ref(nvim_get_curwin()).w_leftcol as c_int)
                    + win_ref(nvim_get_curwin()).w_view_width
                    - col_off
            {
                let col = nvim_get_cursor_col() - 1;
    ...
    if u_save(u_save_top, u_save_bot) != 0 {
        ...
        if open_line(dir, flags, 0, std::ptr::null_mut()) {
            if win_cursorline_standout(nvim_get_curwin()) {
                ...
                win_mut(nvim_get_curwin()).w_valid &= !0x10; // VALID_CROW
            }
    ...
    if coladd > 0 {
        ...
        if col > 0 {
            ...
            if lnum > 1 {
                let new_lnum = lnum - 1;
                nvim_set_cursor_lnum(new_lnum);
                nvim_set_cursor_col(ml_get_len(new_lnum));
                return true;
            }
        }
    }
...
pub unsafe extern "C" fn rs_unadjust_for_sel_inner_visual() -> bool {
    VIsual_select_exclu_adj = false;
    let coladd = VIsual.coladd;
    if coladd > 0 {
        VIsual.coladd = coladd - 1;
    } else {
        let col = VIsual.col;
        if col > 0 {
            VIsual.col = col - 1;
            ...
            if virtual_active(nvim_get_curwin()) {
                ...
                VIsual.coladd = coladd_new;
            }
        } else {
            let lnum = VIsual.lnum;
            if lnum > 1 {
                let new_lnum = lnum - 1;
                VIsual.lnum = new_lnum;
                VIsual.col = ml_get_len(new_lnum);
                return true;
            }
        }
    }
...
unsafe fn nv_g_cmd_impl(cap: CapHandle) {
    ...
    match nchar {
        ...
        n if n == K_BS => {
            (*cap).nchar = CTRL_H_KEY;
            // FALLTHROUGH
            (*cap).cmdchar = (*cap).nchar + (b'v' as c_int - b'h' as c_int);
            (*cap).arg = 1;
            rs_nv_visual(cap);
        }
        n if n == b'h' as c_int || n == b'H' as c_int || n == CTRL_H_KEY => {
            (*cap).cmdchar = nchar + (b'v' as c_int - b'h' as c_int);
            (*cap).arg = 1;
            rs_nv_visual(cap);
        }
        ...
        n if n == b'N' as c_int || n == b'n' as c_int => {
            if !current_search((*cap).count1, nchar == b'n' as c_int) {
                rs_clearopbeep(oap);
            }
        }
        ...
        n if n == b'j' as c_int || n == K_DOWN => {
            let ok = if win_ref(nvim_get_curwin()).w_p_wrap() != 0 {
                rs_nv_screengo(oap, FORWARD, (*cap).count1, false)
            } else {
                (*oap).motion_type = K_MT_LINEWISE;
                cursor_down((*cap).count1, (*oap).op_type == OP_NOP)
            };
            if !ok {
                rs_clearopbeep(oap);
            }
        ...
        n if n == b'k' as c_int || n == K_UP => {
            let ok = if win_ref(nvim_get_curwin()).w_p_wrap() != 0 {
                rs_nv_screengo(oap, BACKWARD, (*cap).count1, false)
            } else {
                (*oap).motion_type = K_MT_LINEWISE;
                cursor_up((*cap).count1, (*oap).op_type == OP_NOP) != 0
            };
            if !ok {
                rs_clearopbeep(oap);
            }
        ...
        n if n == b'e' as c_int || n == b'E' as c_int => {
            ...
            nvim_curwin_set_curswant(true);
            (*oap).inclusive = true;
            if bckend_word((*cap).count1, nchar == b'E' as c_int, false) == 0 {
                rs_clearopbeep(oap);
            }
        }
        ...
        n if n == b'd' as c_int || n == b'D' as c_int => {
                ...
                )
            {
                ...
                if (fdo_flags & K_OPT_FDO_FLAG_SEARCH) != 0 && KeyTyped && (*oap).op_type == OP_NOP
                {
                    rs_foldOpenCursor();
                }
            ...
            || n == K_X2RELEASE =>
        {
            ...
            do_mouse(oap, nchar, BACKWARD, (*cap).count1, false);
        }
...
pub unsafe extern "C" fn rs_nv_at(cap: CapHandle) {
    ...
    let nchar = (*cap).nchar;
    if nchar == c_int::from(b'=') && get_expr_register() == NUL_CHAR {
        return;
    }
    let mut count = (*cap).count1;
...
pub unsafe extern "C" fn rs_nv_join(cap: CapHandle) {
    ...
    let mut count0 = (*cap).count0.max(2);
    (*cap).count0 = count0;
...
pub unsafe extern "C" fn rs_nv_open(cap: CapHandle) {
    let oap = (*cap).oap;
    let cmdchar = (*cap).cmdchar;
    if (*oap).op_type == OP_DELETE && cmdchar == c_int::from(b'o') {
        // "do" is ":diffget"
        rs_clearop(oap);
        let cap_opcount = (*cap).opcount;
        debug_assert!(cap_opcount >= 0);
        #[allow(clippy::cast_sign_loss)]
        nv_diffgetput(false, cap_opcount as usize);
...
pub unsafe extern "C" fn rs_prep_redo_cmd(cap: CapHandle) {
    ...
    if (*cap).nchar_len > 0 {
        AppendToRedobuff((*cap).nchar_composing.as_ptr());
    } else {
        let nchar = (*cap).nchar;
        AppendCharToRedobuff(nchar);
    }
...
pub unsafe extern "C" fn rs_set_vcount_ca(cap: CapHandle, set_prevcount: *mut bool) {
    let count0 = i64::from((*cap).count0);
    let cap_opcount = i64::from((*cap).opcount);

    let count = if cap_opcount != 0 {
        cap_opcount * (if count0 == 0 { 1 } else { count0 })
    } else {
        count0
    };
...
pub unsafe extern "C" fn rs_nv_tagpop(cap: CapHandle) {
    let oap = (*cap).oap;
    if !rs_checkclearopq(oap) {
        rs_do_tag(c"".as_ptr().cast_mut(), DT_POP, (*cap).count1, 0, true);
    }
}
...
pub unsafe extern "C" fn rs_v_visop(cap: CapHandle) {
    ...
    if cmdchar >= c_int::from(b'A') && cmdchar <= c_int::from(b'Z') {
        if VIsual_mode != CTRL_V {
            VISUAL_MODE_ORIG.store(VIsual_mode, Ordering::Relaxed);
            VIsual_mode = c_int::from(b'V');
...
pub unsafe extern "C" fn rs_nv_lineop(cap: CapHandle) {
    ...
    if cursor_down(count1 - 1, op_type == OP_NOP) {
        let oap_motion_force = (*oap).motion_force;
        let is_linewise_delete = op_type == OP_DELETE
            && oap_motion_force != c_int::from(b'v')
            && oap_motion_force != CTRL_V;
...
pub extern "C" fn rs_may_start_select(c: c_int) {
    unsafe {
        let select = (c == c_int::from(b'o') || (stuff_empty() && typebuf_typed()))
...
pub unsafe extern "C" fn rs_nv_gv_cmd(cap: CapHandle) {
    ...
    if VIsual_active {
        ...
        let i = VIsual_mode;
        VIsual_mode = buf_ref(cur_buf).b_visual.vi_mode;
        {
        ...
        buf_mut(cur_buf).b_visual.vi_start = make_pos(VIsual.lnum, VIsual.col, VIsual.coladd);
    } else {
        VIsual_mode = buf_ref(cur_buf).b_visual.vi_mode;
        nvim_set_curswant(buf_ref(cur_buf).b_visual.vi_curswant);
...
pub unsafe extern "C" fn rs_v_swap_corners(cmdchar: c_int) {
    ...
    if cmdchar == c_int::from(b'O') && VIsual_mode == CTRL_V {
        ...
        if old_lnum >= VIsual.lnum && sel_is_exclusive() {
            nvim_set_curswant(nvim_get_curswant() + 1);
        }
        coladvance(nvim_get_curwin(), nvim_get_curswant());
        ...
        nvim_set_cursor_pos(VIsual.lnum, VIsual.col, VIsual.coladd);
        VIsual = crate::types::PosT {
            lnum: old_lnum,
            col: old_col,
            coladd: old_coladd,
...
pub unsafe extern "C" fn rs_unadjust_for_sel() -> bool {
    if sel_is_exclusive()
        && !(VIsual.lnum == nvim_get_cursor_lnum()
            && VIsual.col == nvim_get_cursor_col()
            && VIsual.coladd == nvim_get_cursor_coladd())
    {
        if lt_pos(
            VIsual.lnum,
            VIsual.col,
...
pub unsafe extern "C" fn rs_nv_percent(cap: CapHandle) {
    ...
    if (*oap).op_type == OP_NOP
        && lnum != nvim_get_cursor_lnum()
        && (fdo_flags & K_OPT_FDO_FLAG_PERCENT) != 0
        && KeyTyped
    {
        rs_foldOpenCursor();
    }
...
pub unsafe extern "C" fn rs_nv_addsub(cap: CapHandle) {
    ...
    if nvim_bt_prompt_curbuf() && !prompt_curpos_editable() {
        rs_clearopbeep(oap);
    } else if !VIsual_active && (*oap).op_type == OP_NOP {
        rs_prep_redo_cmd(cap);
        let cmdchar = (*cap).cmdchar;
        (*oap).op_type = if cmdchar == CTRL_A {
            OP_NR_ADD
        } else {
            OP_NR_SUB
        };
    ...
    if !cmd_result {
        ...
        && ({
            ...
            oap_start_lnum > nvim_get_line_count()
                || oap_start_col > ml_get_len(oap_start_lnum)
...
pub unsafe fn rs_normal_search(
    ...
    if i == 0 {
        ...
        if i == 2 {
            (*oap).motion_type = K_MT_LINE_WISE_P2;
        }
        nvim_set_cursor_coladd(0);
        if (*oap).op_type == OP_NOP && (fdo_flags & K_OPT_FDO_FLAG_SEARCH) != 0 && KeyTyped {
            rs_foldOpenCursor();
        }
    }
...
pub unsafe extern "C" fn rs_get_visual_text(
    ...
    if VIsual_mode != visual_mode_linewise {
        rs_unadjust_for_sel();
    }
    if VIsual.lnum != nvim_get_cursor_lnum() {
        if !cap.is_null() {
            let oap = (*cap).oap;
            rs_clearopbeep(oap);
        }
    ...
    if VIsual_mode == visual_mode_linewise {
        ...
        *lenp = nvim_get_cursor_line_len() as usize;
        ...
        if lt_pos(
            ...
            *lenp = (nvim_get_cursor_col() - VIsual.col) as usize + 1;
        } else {
            // text from cursor to VIsual
            *pp = nvim_ml_get_pos_cursor();
            *lenp = (VIsual.col - nvim_get_cursor_col()) as usize + 1;
        }
...
pub unsafe extern "C" fn rs_nv_mark_move_to(
    ...
    (*oap).motion_type = if (flags & K_MARK_BEGIN_LINE) != 0 {
        K_MT_LINE_WISE_P2
    } else {
        K_MT_CHARWISE
    };
    if (*cap).cmdchar == c_int::from(b'`') {
        (*oap).use_reg_one = true;
    }
    (*oap).inclusive = false;
    nvim_curwin_set_curswant(true);
...
extern "C" {
    ...
    fn get_extra_op_char(optype: c_int) -> c_int;
...
pub unsafe extern "C" fn rs_n_start_visual_mode(c: c_int) {
        ...
        && utf_ptr2char(get_cursor_pos_ptr()) == TAB_CHAR
    {
        ...
        coladvance(
            nvim_get_curwin(),
            win_ref(nvim_get_curwin()).w_virtcol as c_int,
        );
    }
    // VIsual = curwin->w_cursor
    let lnum = nvim_get_cursor_lnum();
    let col = nvim_get_cursor_col();
    let coladd = nvim_get_cursor_coladd();
    VIsual = crate::types::PosT { lnum, col, coladd };
    ...
    if win_ref(nvim_get_curwin()).w_redr_type < UPD_INVERTED {
        {
            let l = win_ref(nvim_get_curwin()).w_cursor.lnum;
            let w = win_mut(nvim_get_curwin());
            w.w_old_cursor_lnum = l;
            w.w_old_visual_lnum = l;
        }
    }
...
pub unsafe extern "C" fn rs_end_visual_mode() {
    ...
    let vis_mode = VIsual_mode;
    {
        ...
        b.b_visual.vi_start = make_pos(VIsual.lnum, VIsual.col, VIsual.coladd);
...
pub unsafe extern "C" fn rs_set_op_var(optype: c_int) {
    if optype == OP_NOP {
        ...
        let opchar0 = get_op_char(optype) as u8;
        let opchar1 = get_extra_op_char(optype) as u8;
