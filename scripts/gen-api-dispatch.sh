...
# Each is a root holding the shared support code and one child per API source
# file. Unlike the unit-test cdefs, the output is committed: it is ordinary
# crate source that has to compile, be formatted and be measured by the
